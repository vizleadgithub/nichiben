...
// 解答履歴の取得(テスト)
$sql = "SELECT * FROM exam_answer WHERE exam_id='".mysqli_real_escape_string($objDbConnect->connect,$eid)."' AND student_id='".mysqli_real_escape_string($objDbConnect->connect,$user_id)."' AND product_id='".mysqli_real_escape_string($objDbConnect->connect,$pid)."' AND contents_no='".mysqli_real_escape_string($objDbConnect->connect,$ccno)."'";
...
// 解答履歴の取得(アンケート)
if($qid!=''){
	$sql = "SELECT * FROM exam_answer WHERE exam_id='".mysqli_real_escape_string($objDbConnect->connect,$qid)."' AND student_id='".mysqli_real_escape_string($objDbConnect->connect,$user_id)."' AND product_id='".mysqli_real_escape_string($objDbConnect->connect,$pid)."' AND contents_no='".mysqli_real_escape_string($objDbConnect->connect,$ccno)."'";
	$res = $objDbConnect->query_fetch_arr($sql);
	if ($res){
		foreach($res as $val){
...
$template->layout_noside('exam/result2.tpl');
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
$objDbConnect->close();
exit();
?>