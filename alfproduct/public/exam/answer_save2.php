...
if ($res_exam_answer_retry){
	...
	if (!empty($insert_rows)) {
		...
		if (!$insert_ok) {
			$err_flg = true;
			exam_error_log('answer_save2.bulk_insert', array(
				'student_id'  => $user_id,
				'exam_id'     => $eid,
				'product_id'  => $pid,
				'contents_no' => $ccno,
	...
	if (!$err_flg && !empty($update_rows)) {
		foreach ($update_rows as $u) {
			...
			if (!$ret) {
				$err_flg = true;
				exam_error_log('answer_save2.update', array(
					'student_id'     => $user_id,
					'exam_id'        => $eid,
					'product_id'     => $pid,
					'contents_no'    => $ccno,
	...
	if (!$err_flg && !empty($exam_answer_id_list)) {
		...
		if($ret_exam){
			...
			foreach($exam_answer_id_list as $exam_answer_id){
				...
				if (!$ret){
					$err_flg = true;
					exam_error_log('answer_save2.update_passing', array(
						'student_id'     => $user_id,
						'exam_id'        => $eid,
	...
	if(!$err_flg){
		...
		if (!$ret){
			$err_flg = true;
			exam_error_log('answer_save2.delete_retry', array(
				'student_id'  => $user_id,
				'exam_id'     => $eid,
				'product_id'  => $pid,
				'contents_no' => $ccno,
	...
	if(!$err_flg && $qid!=''){
		...
		if ($res_exam_answer_retry_q){
			...
			if (!empty($insert_rows_q)) {
				...
				if (!$insert_ok_q) {
					$err_flg = true;
					exam_error_log('answer_save2.bulk_insert_q', array(
						'student_id'  => $user_id,
						'exam_id'     => $qid,
						'product_id'  => $pid,
						'contents_no' => $ccno,
			...
			if (!$err_flg && !empty($update_rows_q)) {
				foreach ($update_rows_q as $u) {
					...
					if (!$ret) {
						$err_flg = true;
						exam_error_log('answer_save2.update_q', array(
							'student_id'     => $user_id,
							'exam_id'        => $qid,
							'exam_answer_id' => $u['exam_answer_id'],
							'mysql_errno'    => mysqli_errno($conn),
			...
			if(!$err_flg && !empty($exam_answer_id_list_q)){
				...
				if($ret_exam){
					foreach($exam_answer_id_list_q as $exam_answer_id){
						...
						if (!$ret){
							$err_flg = true;
							exam_error_log('answer_save2.update_passing_q', array(
								'student_id'     => $user_id,
								'exam_id'        => $qid,
								'exam_answer_id' => $exam_answer_id,
								'mysql_errno'    => mysqli_errno($conn),
			...
			if(!$err_flg){
				...
				if (!$ret){
					$err_flg = true;
					exam_error_log('answer_save2.delete_retry_q', array(
						'student_id'  => $user_id,
						'exam_id'     => $qid,
						'product_id'  => $pid,
						'contents_no' => $ccno,
