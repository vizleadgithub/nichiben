...
if ($res){
	$sql = "SELECT * FROM exam_answer WHERE exam_id='".mysqli_real_escape_string($objDbConnect->connect,$eid_sql)."' AND exam_problem_id='".mysqli_real_escape_string($objDbConnect->connect,$res['exam_problem_id'])."' AND student_id='".mysqli_real_escape_string($objDbConnect->connect,$user_id)."' AND product_id='".mysqli_real_escape_string($objDbConnect->connect,$pid)."' AND contents_no='".mysqli_real_escape_string($objDbConnect->connect,$ccno)."'";
	$res = $objDbConnect->query_fetch($sql);
	if ($res){
		$answered_info = $res;
...
$template->layout_noside('exam/resubmit_index2.tpl');
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
$objDbConnect->close();
exit();
?>