...
if ($res){
	$sql = "SELECT * FROM exam_answer WHERE exam_id='".mysql_real_escape_string($eid_sql)."' AND exam_problem_id='".mysql_real_escape_string($res['exam_problem_id'])."' AND student_id='".mysql_real_escape_string($user_id)."' AND product_id='".mysql_real_escape_string($pid)."' AND contents_no='".mysql_real_escape_string($ccno)."'";
	$res = $objDbConnect->query_fetch($sql);
	if ($res){
		$answered_info = $res;
