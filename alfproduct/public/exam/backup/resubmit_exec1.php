...
if(!$err_flg){
	$sql = "SELECT * FROM exam_answer_retry WHERE exam_id='".mysql_real_escape_string($eid)."' AND student_id='".mysql_real_escape_string($user_id)."' AND product_id='".mysql_real_escape_string($pid)."' AND contents_no='".mysql_real_escape_string($ccno)."'";
	...
	// アンケート
	if($qid!=''){
		$sql = "SELECT * FROM exam_answer_retry WHERE exam_id='".mysql_real_escape_string($qid)."' AND student_id='".mysql_real_escape_string($user_id)."' AND product_id='".mysql_real_escape_string($pid)."' AND contents_no='".mysql_real_escape_string($ccno)."'";
		$res_exam_answer_retry = $objDbConnect->query_fetch_arr($sql);
		if ($res_exam_answer_retry){
			// 解答回数の取得
			$exam_answer_no = 1;
...
if(!$err_flg){
	...
	if ($res_rel_exam_problem){
		$sql = "SELECT * FROM exam_answer WHERE exam_id='".mysql_real_escape_string($eid_sql)."' AND exam_problem_id='".mysql_real_escape_string($res_rel_exam_problem['exam_problem_id'])."' AND student_id='".mysql_real_escape_string($user_id)."' AND product_id='".mysql_real_escape_string($pid)."' AND contents_no='".mysql_real_escape_string($ccno)."'";
