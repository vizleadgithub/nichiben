...
if ($res){
	...
	$sql.= "   TP.start_date,";
	$sql.= "   TP.end_date,";
	$sql.= "   ( SELECT DATE_FORMAT(MIN(regist_at), '%Y/%m/%d') FROM report_user_video_viewed WHERE student_id = '".$_SESSION['user']['id']."' AND video_id IN( 
	...
	$sql.= "   ( SELECT DATE_FORMAT(MAX(complete_date), '%Y/%m/%d') FROM report_user_video_viewed WHERE student_id = '".$_SESSION['user']['id']."' AND video_id IN( 
			TP.contents_contents1,
			TP.contents_contents2,
			TP.contents_contents3,
			TP.contents_contents4,
...
$template->layout_noside('mypage/lesson_list1.tpl');
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
