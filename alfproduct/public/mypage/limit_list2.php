...
$_SESSION['wp_page_head_title'] = '掲載終了間近の講座(受講中研修)';
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
$page = 1;
if( isset($_SESSION["limit_list2.page"]) && !empty($_SESSION["limit_list2.page"]) ){
	$page = $_SESSION["limit_list2.page"];
}
if( isset($_GET["page"]) && !empty($_GET["page"]) && is_numeric($_GET["page"]) ){
	$page = $_GET["page"];
	$_SESSION["limit_list2.page"] = $page;
}
...
$search_buy_date = '';
/*
if( isset($_SESSION["limit_list2.search_buy_date"]) && !empty($_SESSION["limit_list2.search_buy_date"]) ){
	$search_buy_date = $_SESSION["limit_list2.search_buy_date"];
}
if( isset($_GET["search_buy_date"]) && !empty($_GET["search_buy_date"]) ){
	$search_buy_date = $_GET["search_buy_date"];
	$_SESSION["limit_list2.search_buy_date"] = $search_buy_date;
}
if( isset($_GET["search_buy_date"]) && empty($_GET["search_buy_date"]) ){
	$search_buy_date = "";
	$_SESSION["limit_list2.search_buy_date"] = "";
}
*/
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
// 受講していて完了していない動画のvideo_idを取得
$in_video_id = '';
...
$sql = _get_sql("COUNT(*) AS c", '', $search_buy_date, $in_video_id);
...
$sql = _get_sql($select_col, $order, $search_buy_date, $in_video_id);
...
$template->layout_noside('mypage/limit_list2.tpl');
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

function _get_sql($select_col, $order='', $search_buy_date='', $in_video_id){
	$sql = "SELECT";
	$sql.= "   $select_col";
	$sql.= " FROM";
	$sql.= "   tbl_product AS T2";
	$sql.= "     INNER JOIN";
	$sql.= "   tbl_product_add AS T1";
	$sql.= "       ON T2.product_id = T1.product_id";
	$sql.= " WHERE";
	$sql.= "   T2.del_flg = '0'";
	...
	$sql.= "   AND T1.product_type_add = 1";
	// 受講中動画なしの場合
	if ($in_video_id==''){
	$sql.= "   AND 1=0";
	} else {
	$sql.= "   AND (
      T2.contents_contents1 IN($in_video_id)
      OR T2.contents_contents2 IN($in_video_id)
      OR T2.contents_contents3 IN($in_video_id)
      OR T2.contents_contents4 IN($in_video_id)
      OR T2.contents_contents5 IN($in_video_id)
      OR T2.contents_contents6 IN($in_video_id)
      OR T2.contents_contents7 IN($in_video_id)
      OR T2.contents_contents8 IN($in_video_id)
      OR T2.contents_contents9 IN($in_video_id)
      OR T2.contents_contents10 IN($in_video_id)
      OR T2.contents_contents11 IN($in_video_id)
      OR T2.contents_contents12 IN($in_video_id)
      OR T2.contents_contents13 IN($in_video_id)
      OR T2.contents_contents14 IN($in_video_id)
      OR T2.contents_contents15 IN($in_video_id)
      OR T2.contents_contents16 IN($in_video_id)
      OR T2.contents_contents17 IN($in_video_id)
      OR T2.contents_contents18 IN($in_video_id)
      OR T2.contents_contents19 IN($in_video_id)
      OR T2.contents_contents20 IN($in_video_id)
      OR T2.contents_contents21 IN($in_video_id)
      OR T2.contents_contents22 IN($in_video_id)
      OR T2.contents_contents23 IN($in_video_id)
      OR T2.contents_contents24 IN($in_video_id)
      OR T2.contents_contents25 IN($in_video_id)
    )";
	}
	if ($order!=''){
		$sql.= " ORDER BY $order";
