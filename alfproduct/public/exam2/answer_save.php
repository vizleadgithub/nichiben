...
for ($i = 0; $i < count(@$_POST["exam2_problem_id"]); $i++){
	...
	// 登録
	if(count($ret_exam2_answer)<=0){
		$sql  = "";
		$sql .= "INSERT INTO exam2_answer ( ";
		$sql .= "       exam2_id, exam2_problem_id, student_id, exam2_answer_no, exam2_answer_contents, exam2_answer_date, exam2_answer_mark, exam2_answer_point, marked_teacher_id, status, product_id, contents_no, question_flg ";
		$sql .= ") VALUES ('".$answer['exam2_id']."', '".$answer['exam2_problem_id']."', '".$answer['student_id']."', '".$answer['exam2_answer_no']."', '".$answer['exam2_answer_contents']."', '".$answer['exam2_answer_date']."', '".$answer['exam2_answer_mark']."', '".$answer['exam2_answer_point']."', '".$answer['marked_teacher_id']."', '".$answer['status']."', '".$pid."', '".$ccno."', '".$question_flg."') ";
		$ret = $objDbConnect->execute($sql);
		if (!$ret){
			$err_flg = true;
		} else {
			$exam2_answer_id_list[] = $objDbConnect->get_thread_id("exam2_answer");
		}
...
$template->layout_alfstream('exam2/answer_save.tpl');
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
$objDbConnect->close();
exit();
?>