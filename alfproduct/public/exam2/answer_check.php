...
if( empty(@$_POST["exam2_problem_id"]) ){
	$return_check = false;
} else {
	if( $exam2_check>=0 ){
		for ($i = 0; $i < count(@$_POST["exam2_problem_id"]); $i++){
			...
			$temp_check = array();
			if( isset($_POST["exam2_problem_id"][$i]) ){
				$temp_post_name = "exam2_problem_".$_POST["exam2_problem_id"][$i];
				if( is_array($_POST[$temp_post_name]) ){
...
$template->layout_pop('exam2/answer_check.tpl');
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
$objDbConnect->close();
exit();
?>