...
// 回答履歴の更新回数を取得しておく(管理者側の更新チェック用)
if(isset($_SESSION['exam2_answer_update_count'])){
	unset($_SESSION['exam2_answer_update_count']);
}
$exam2_answer_update_count = 0;
$sql  = "";
...
$res = $objDbConnect->query_fetch($sql);
if($res){
	$exam2_answer_update_count = $res['update_count'];
}
$_SESSION['exam2_answer_update_count'] = $exam2_answer_update_count;
//print("[exam2_answer_update_count:".$_SESSION['exam2_answer_update_count']."]");
...
// 判定基準ありかどうか
$hantei_ari = false;
$sql = "SELECT criteria_value FROM exam2 WHERE exam2_id='".mysql_real_escape_string($e2id)."'";
...
// 合格しているかどうか
$passing_flg = false;
$sql = "SELECT COUNT(exam2_answer_id) AS count FROM exam2_answer WHERE status=0 AND passing_flg=1 AND student_id='".mysql_real_escape_string($user_id)."' AND product_id='".mysql_real_escape_string($pid)."' ";
$ret = $objDbConnect->query_fetch($sql);
if($ret){
	if($ret['count']>0){
		$passing_flg = true;
