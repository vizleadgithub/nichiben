...
if($err_flg==0 ){
	...
	if( $access_id=="" && $access_pass=="" ){
		$ret = $objGMOPaymentProtocol->entry_tran_au( $order_id, $price, $tax );
	...
	if($err_flg==0 ){
		if( $access_id!="" && $access_pass!="" ){
			$ret = $objGMOPaymentProtocol->exec_tran_au( $access_id, $access_pass, $order_id, $item_name, $etc1, $etc2, $etc3 );
			if( isset($ret["ErrCode"]) ){
				if( $ret["ErrCode"]!="" ){
					$err_flg = 1;
					print("order err.7");
				...
				$objDbConnect->execute($sql);
				header("Location: end_au.php?".session_name ()."=".session_id() );
				exit;
			}
		}
	}
