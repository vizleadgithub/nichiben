...
if($_SERVER["REQUEST_METHOD"] == "POST" && $err_flg==0 ){
	...
	if($err_flg==0 ){
		if( $access_id!="" && $access_pass!="" ){
			...
			$ret = $objGMOPaymentProtocol->exec_tran_cvs( $access_id, $access_pass, $order_id, $convenience, $name, $kana, $tel, $mail_address, $reserve_no, $member_no, $etc1, $etc2, $etc3 );
			if( isset($ret["ErrCode"]) ){
				if( $ret["ErrCode"]!="" ){
					$err_flg = 1;
					print("order err.7");
				...
				header("Location: end_cvs.php?".session_name ()."=".session_id() );
				exit;
			}
		}
	}
