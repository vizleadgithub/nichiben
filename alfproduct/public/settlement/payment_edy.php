...
if($_SERVER["REQUEST_METHOD"] == "POST" && $err_flg==0 ){
	...
	if( $access_id=="" && $access_pass=="" ){
		$ret = $objGMOPaymentProtocol->entry_tran_edy( $order_id, $price, $tax );
	...
	if($err_flg==0 ){
		if( $access_id!="" && $access_pass!="" ){
			$ret = $objGMOPaymentProtocol->exec_tran_edy( $access_id, $access_pass, $order_id, $mail_address, $etc1, $etc2, $etc3 );
			if( isset($ret["ErrCode"]) ){
				if( $ret["ErrCode"]!="" ){
					$err_flg = 1;
					print("order err.7");
				...
				header("Location: end_edy.php?".session_name ()."=".session_id() );
				exit;
			}
		}
	}
