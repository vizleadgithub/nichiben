...
$arrTemp = $objDbConnect->query_fetch($sql);
...
if( !$arrTemp ) {
	...
	$ret_sub = $objDbConnect->query_fetch($sql);
	if( count($ret_sub)>0 ){
		$template->layout_noside('settlement/err.tpl');
		$objDbConnect->close();
		exit;
...
if( $err_flg==0 ){
	...
	//$objDbConnect->execute($sql);
	$sql = "update tbl_order set payment_status='2', claim_flg='0', web_flg='1', payment_date='$payment_date', order_date='$payment_date' where order_id='".$order_id."' ";
	$objDbConnect->execute($sql);
	
	// 注文詳細の支払フラグ更新・カート情報削除
	...
	//+++++++++++++++++++++++++++++++++++++++++++++++++++
	$str_mail = file_get_contents( dirname(__FILE__) ."./../../module/order9.mail" );
	...
	if( !$arr_order ){
		...
		if( !$arr_order_detail ){
			...
			if( !$arr_student ){
				...
				$product_count = count($arr_order_detail);
				...
				for($x=0;$x<$product_count;$x++){
					...
					if ($arr_order_detail[$x]["product_type_add"] == 2){
						...
						if ($arr_rel_product_bar_association_branch && isset($arr_rel_product_bar_association_branch[0]) && isset($arr_rel_product_bar_association_branch[0]["dates"]) ){
							$dates = '['.$arr_rel_product_bar_association_branch[0]["dates"].'開催]';
						} else {
							$dates = '';
						}
				...
				$temp_mail = str_replace( '@@product_count@@', $product_count, $temp_mail );
				...
				$to      = $tomail;
				$subject = str_replace('@@name@@',$name,'【日本弁護士連合会】ライブ実務研修：お申込みありがとうございます。');
				$body    = $temp_mail;
	...
	//+++++++++++++++++++++++++++++++++++++++++++++++++++
	header("Location: end_free.php?".session_name ()."=".session_id() );
