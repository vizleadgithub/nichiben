...
$arr_expire_y = array('' => ' -- ');
$now_year_val = date('y');
$now_year_disp = date('Y');
for ($i=0; $i<11; $i++){
	$year_val = $now_year_val + $i;
	$year_disp = $now_year_disp + $i;
	
	$arr_expire_y[$year_val] = $year_disp;
}
...
if( !$arrTemp ) {
	...
	if ( is_numeric($arrTemp["price"]) && is_numeric($arrTemp["tax"]) ) {
		$access_id = trim($arrTemp["access_id"]);
		$access_pass = trim($arrTemp["access_pass"]);
		$price = $arrTemp["price"];
		$tax = $arrTemp["tax"];
...
if($_SERVER["REQUEST_METHOD"] == "POST" && $err_flg==0 ){
	...
	if( $access_id=="" && $access_pass=="" ){
		...
		$sql = "update tbl_order_temp set entry_return='".serialize($ret)."' where order_id='".$order_id."' ";
		...
		if($err_flg==0 ){
			if( isset($ret["AccessID"]) && isset($ret["AccessPass"]) ){
				if( $ret["AccessID"]!="" && $ret["AccessPass"]!="" ){
					$access_id = trim($ret["AccessID"]);
					$access_pass = trim($ret["AccessPass"]);
					$sql = "update tbl_order_temp set access_id='".$access_id."', access_pass='".$access_pass."' where order_id='".$order_id."' ";
					$objDbConnect->execute($sql);
				} else {
					$err_flg = 1;
					//print("order err.6");
	...
	if($err_flg==0 ){
		if( $access_id!="" && $access_pass!="" ){
			$ret = $objGMOPaymentProtocol->exec_tran( $access_id, $access_pass, $order_id, $card_no, $expire, '', '', $etc1, $etc2, $etc3 );
			if( isset($ret["ErrCode"]) ){
				if( $ret["ErrCode"]!="" ){
					$err_flg = 1;
					//print("order err.8");
				...
				//$objDbConnect->execute($sql);
				$sql = "update tbl_order set payment_status='2', exec_return='".serialize($ret)."', payment_date='$payment_date', claim_flg='$claim_flg', web_flg='1', order_date='".date('Y-m-d H:i:s')."' where order_id='".$order_id."' ";
				$objDbConnect->execute($sql);
				
				// 注文詳細の支払フラグ更新
				$sql = "update tbl_order_detail set payment_status='2', payment_date='$payment_date' where order_id='".$order_id."' ";
				$objDbConnect->execute($sql);
				...
				if ($passport_flg){
					$now_date = date('Y-m-d');
					//--------------------------------------------
					$sql_passport = "SELECT exp_date_passport, presence_passport FROM student WHERE student_id = '".$_SESSION['user']['id']."' AND exp_date_passport>='".date('Y-m-d')."' ";
					...
					$sql = "UPDATE student SET presence_passport = '1', exp_date_passport = '".$exp_date_passport."' WHERE student_id = '".$_SESSION['user']['id']."'";
					$res = $objDbConnect->execute($sql);
					if ($res){
						$_SESSION['user']['presence_passport'] = 1;
						$_SESSION['user']['exp_date_passport'] = $exp_date_passport;
					}
				}
				...
				//+++++++++++++++++++++++++++++++++++++++++++++++++++
				// メールテンプレート(クレジットカード＋パスポート購入)
				if ($passport_flg){
					$str_mail = file_get_contents( dirname(__FILE__) ."./../../module/order5.mail" );
				// メールテンプレート(クレジットカード＋eラーニング・会場研修)
				} else {
					$str_mail = file_get_contents( dirname(__FILE__) ."./../../module/order1.mail" );
				}
...
//exit();
				//+++++++++++++++++++++++++++++++++++++++++++++++++++
				//$template->layout_noside('settlement/end.tpl');
				header("Location: end.php?".session_name ()."=".session_id() );
				$objDbConnect->close();
				exit;
				//+++++++++++++++++++++++++++++++++++++++++++++++++++
			}
