...
sso_log('login_in', array(
	'has_token_id'     => isset($_SESSION['user']['token_id']) ? 1 : 0,
	'has_login_result' => isset($_SESSION['user']['login_result']) ? 1 : 0,
));

if (isset($_SESSION['user']['token_id']) && isset($_SESSION['user']['login_result'])){
	$lawyer_number_raw = isset($_SESSION['user']['login_result']->lawyer_number)
		? $_SESSION['user']['login_result']->lawyer_number
		: null;
	...
	sso_log('login_student_lookup', array(
		'has_lawyer_no' => ($lawyer_number_raw !== null && $lawyer_number_raw !== '') ? 1 : 0,
	...
	//var_dump($ret);
	if ($ret){
		sso_log('login_student_found', array(
			'student_id'         => isset($ret['student_id']) ? $ret['student_id'] : '',
			'has_student_name'   => (isset($ret['student_name']) && $ret['student_name'] !== '') ? 1 : 0,
			'bar_association_id' => isset($ret['bar_association_id']) ? $ret['bar_association_id'] : '',
			'has_regist_date'    => (isset($ret['regist_date']) && $ret['regist_date'] !== '' && $ret['regist_date'] !== '0000-00-00') ? 1 : 0,
			'presence_passport'  => isset($ret['presence_passport']) ? $ret['presence_passport'] : '',
		));
		...
		$_SESSION['user']['sub_auth_ethic_training'] = 1;//倫理研修強制許可
		...
		sso_log('login_success', array(
			'student_id' => $_SESSION['user']['id'],
			'has_name'   => (isset($_SESSION['user']['name']) && $_SESSION['user']['name'] !== '') ? 1 : 0,
			'year'       => $_SESSION['user']['year'],
			'duty_year'  => ($_SESSION['user']['bar_association_duty_year'] === false) ? '' : $_SESSION['user']['bar_association_duty_year'],
		));

		unset($_SESSION['user']['token_id']);
		unset($_SESSION['user']['login_result']);
		unset($_SESSION['sso_log_trace_id']);

		// セッションIDの再生成（フィクセーション対策）
		session_regenerate_id(true);

		// SSOログイン判定ok情報を保持
		$_SESSION["sso_logged_in"] = 1;
	...
	sso_log('login_student_not_found', array(
		'lawyer_no' => ($lawyer_number_raw === null) ? '' : $lawyer_number_raw,
	));
...
sso_log('login_bad_request', array(
	'has_token_id'     => isset($_SESSION['user']['token_id']) ? 1 : 0,
	'has_login_result' => isset($_SESSION['user']['login_result']) ? 1 : 0,
));
