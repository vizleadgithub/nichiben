...
if ($_SESSION['user']['sub_auth_ethic_training'] == 1){
	// 履歴登録チェック
	$sql = "SELECT COUNT(*) AS c FROM tbl_ethic_question_history WHERE student_id = '$user_id' AND product_id = '$pid'";
	$res = $objDbConnect->query_fetch($sql);
	if ($res){
		// 未登録なら登録する
		if ($res['c']<=0){
			$sql = "INSERT INTO tbl_ethic_question_history (student_id,product_id,create_date) VALUES ('".$user_id."','".$pid."','".date('Y-m-d H:i:s')."')";
			$objDbConnect->execute($sql);
		}
	}
	
	// ステータスチェック(1次未受講と1次受講中ユーザーのみ受講可)
	$sql = "SELECT COUNT(*) AS c FROM tbl_ethic_question_history WHERE student_id = '$user_id' AND product_id = '$pid' AND status IN(0,1)";
	$res = $objDbConnect->query_fetch($sql);
	if ($res){
		if ($res['c']<=0){
			header("Location: /");
...
//var_dump($user_id);
//var_dump($val['video_id']);
//var_dump($history_view);
					if ($history_view){
...
$template->layout_noside('ethic_treaning/index.tpl');
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
$objDbConnect->close();
exit();
?>