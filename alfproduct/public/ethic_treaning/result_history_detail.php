...
if ($_SESSION['user']['sub_auth_ethic_training'] == 1){
	...
	// 設問の取得
	$sql = "SELECT question, failure_flg, `rank`, reference FROM tbl_ethic_question WHERE ethic_question_id = '$qid'";
	$res = $objDbConnect->query_fetch($sql);
	if ($res){
		$question = $res['question'];
		$reference = $res['reference'];
		if ($res['failure_flg']==1){
			$question_no = $res['rank'] + 10;
		} else {
			$question_no = $res['rank'];
		}
		
		// 回答履歴の取得
		$sql = "SELECT answer_ethic_branch_id$question_no, answer_date$question_no FROM tbl_ethic_question_history WHERE student_id = '$user_id' AND product_id = '$pid' AND answer_ethic_branch_id$question_no IS NOT NULL";
		$res = $objDbConnect->query_fetch($sql);
		if ($res){
