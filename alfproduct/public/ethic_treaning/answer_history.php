...
if ($_SESSION['user']['sub_auth_ethic_training'] == 1){
	// ステータスチェック
	$sql = "SELECT COUNT(*) AS c FROM tbl_ethic_question_history WHERE student_id = '$user_id' AND product_id = '$pid' AND status IN(1,4)";
	...
	// 設問の取得
	$sql = "SELECT question, `rank`, reference FROM tbl_ethic_question WHERE ethic_question_id = '$qid'";
	$res = $objDbConnect->query_fetch($sql);
	if ($res){
		...
		// 通常・追試問題の判断フラグ
		if (strpos($_SERVER['HTTP_REFERER'], '/ethic_treaning/retry.php') !== false){
			$retry_flg = true;
			$question_no += 10;
		} else {
			$retry_flg = false;
		...
		// 回答履歴の取得
		$sql = "SELECT answer_ethic_branch_id$question_no, answer_date$question_no FROM tbl_ethic_question_history WHERE student_id = '$user_id' AND product_id = '$pid'";
		$res = $objDbConnect->query_fetch($sql);
		if ($res){
			$history_answer['answer_ethic_branch_id'] = $res["answer_ethic_branch_id$question_no"];
			$history_answer['answer_date'] = date('Y年m月d日', strtotime($res["answer_date$question_no"]));
		}
	}
	
	// 問題の取得
	$sql = "SELECT ethic_branch_id, question_branch, answer_flg FROM tbl_ethic_branch WHERE ethic_question_id = '$qid' ORDER BY `rank` ASC";
	$res = $objDbConnect->query_fetch_arr($sql);
	if ($res){
		foreach ($res as $val){
			$arr_list[$val['ethic_branch_id']]['question_branch'] = $val['question_branch'];
			$arr_list[$val['ethic_branch_id']]['answer_flg'] = $val['answer_flg'];
		}
	}
