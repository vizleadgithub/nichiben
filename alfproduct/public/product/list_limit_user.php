...
$_SESSION['wp_page_head_title'] = '掲載終了間近の講座';
...
//$pcid = $_GET['pcid'];
$pagemax = 5;
if( isset($_SESSION["productlistlimituser.pagemax"]) && !empty($_SESSION["productlistlimituser.pagemax"]) ){
	$pagemax = $_SESSION["productlistlimituser.pagemax"];
}
...
if( $_SERVER["REQUEST_METHOD"] == "POST" ){
	$_SESSION["productlistlimituser.page"] = 1;
	$_SESSION["productlistlimituser.sort"] = 1;
}
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
$page = 1;
if( isset($_SESSION["productlistlimituser.page"]) && !empty($_SESSION["productlistlimituser.page"]) ){
	$page = $_SESSION["productlistlimituser.page"];
}
if( isset($_GET["page"]) && !empty($_GET["page"]) && is_numeric($_GET["page"]) ){
	$page = $_GET["page"];
	$_SESSION["productlistlimituser.page"] = $page;
}
if( isset($_GET["pagemax"]) && is_numeric($_GET["pagemax"]) ){
	$pagemax = $_GET["pagemax"];
	$_SESSION["productlistlimituser.pagemax"] = $pagemax;
}
$objPager->setNowPage( $page );
$objPager->setPageMax( $pagemax );
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
$sort = 1;
if( isset($_SESSION["productlistlimituser.sort"]) && !empty($_SESSION["productlistlimituser.sort"]) ){
	$sort = $_SESSION["productlistlimituser.sort"];
}
if( isset($_GET["sort"]) && !empty($_GET["sort"]) && is_numeric($_GET["sort"]) ){
	$sort = $_GET["sort"];
	$_SESSION["productlistlimituser.sort"] = $sort;
}
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
$where.= " AND DATE_SUB(tbl_product.end_date, INTERVAL 1 MONTH) <= '".date("Y-m-d")."'";
$where.= " AND tbl_product_add.product_type_add = '1'";
// パスポートなしの場合は購入商品で絞る
if ($_SESSION['user']['presence_passport']==0){
	$where.= " AND tbl_order_detail.member_id = '".$_SESSION['user']['id']."'";
	$where.= " AND tbl_order_detail.payment_status = '2'";
	$where.= " AND tbl_order_detail.product_type_add = '1'";
}
// 未ログインの場合
//if (!st_login_check()){
...
$template->layout('product/list_limit_user.tpl');
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
$objDbConnect->close();
exit();
?>