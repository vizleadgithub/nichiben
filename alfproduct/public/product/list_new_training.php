...
$_SESSION['wp_page_head_title'] = '新着eラーニング一覧';
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
$pagemax = 5;
if( isset($_SESSION["productlist.pagemax"]) && !empty($_SESSION["productlist.pagemax"]) ){
	$pagemax = $_SESSION["productlist.pagemax"];
}

if( $_SERVER["REQUEST_METHOD"] == "POST" ){
	$_SESSION["productlist.page"] = 1;
	$_SESSION["productlist.sort"] = 1;
}
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
$page = 1;
if( isset($_SESSION["productlist.page"]) && !empty($_SESSION["productlist.page"]) ){
	$page = $_SESSION["productlist.page"];
}
if( isset($_GET["page"]) && !empty($_GET["page"]) && is_numeric($_GET["page"]) ){
	$page = $_GET["page"];
	$_SESSION["productlist.page"] = $page;
}
if( isset($_GET["pagemax"]) && is_numeric($_GET["pagemax"]) ){
	$pagemax = $_GET["pagemax"];
	$_SESSION["productlist.pagemax"] = $pagemax;
}
$objPager->setNowPage( $page );
$objPager->setPageMax( $pagemax );
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
$sort = 1;
if( isset($_SESSION["productlist.sort"]) && !empty($_SESSION["productlist.sort"]) ){
	$sort = $_SESSION["productlist.sort"];
}
if( isset($_GET["sort"]) && !empty($_GET["sort"]) && is_numeric($_GET["sort"]) ){
	$sort = $_GET["sort"];
	$_SESSION["productlist.sort"] = $sort;
}
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
$where = " WHERE ";
...
//$where.= " AND ( DATE_ADD(tbl_product.regist_date, INTERVAL 1 MONTH) >= '".date("Y-m-d")."' OR DATE_ADD(tbl_product.update_date, INTERVAL 1 MONTH) >= '".date("Y-m-d")."') ";
$where.= " AND DATE_ADD(tbl_product.regist_date, INTERVAL 1 MONTH) >= '".date("Y-m-d")."' ";
