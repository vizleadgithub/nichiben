...
$_SESSION['wp_page_head_title'] = 'おすすめｅラーニング一覧';
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
$pagemax = 5;
if( isset($_SESSION["productlistrecommend.pagemax"]) && !empty($_SESSION["productlistrecommend.pagemax"]) ){
	$pagemax = $_SESSION["productlistrecommend.pagemax"];
}

if( $_SERVER["REQUEST_METHOD"] == "POST" ){
	$_SESSION["productlistrecommend.page"] = 1;
	$_SESSION["productlistrecommend.sort"] = 1;
}
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
$page = 1;
if( isset($_SESSION["productlistrecommend.page"]) && !empty($_SESSION["productlistrecommend.page"]) ){
	$page = $_SESSION["productlistrecommend.page"];
}
if( isset($_GET["page"]) && !empty($_GET["page"]) && is_numeric($_GET["page"]) ){
	$page = $_GET["page"];
	$_SESSION["productlistrecommend.page"] = $page;
}
if( isset($_GET["pagemax"]) && is_numeric($_GET["pagemax"]) ){
	$pagemax = intval($_GET["pagemax"]);
	$_SESSION["productlistrecommend.pagemax"] = $pagemax;
}
$objPager->setNowPage( $page );
$objPager->setPageMax( $pagemax );
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
$sort = 1;
if( isset($_SESSION["productlistrecommend.sort"]) && !empty($_SESSION["productlistrecommend.sort"]) ){
	$sort = $_SESSION["productlistrecommend.sort"];
}
if( isset($_GET["sort"]) && !empty($_GET["sort"]) && is_numeric($_GET["sort"]) ){
	$sort = $_GET["sort"];
	$_SESSION["productlistrecommend.sort"] = $sort;
}
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
$where = " WHERE tbl_product.del_flg='0'";
$where.= " AND tbl_product_elearning.product_flg LIKE '%|1|%'";
...
$sql = "SELECT COUNT(*) AS c FROM tbl_product INNER JOIN tbl_product_elearning ON tbl_product.product_id = tbl_product_elearning.product_id";
$sql = "SELECT";
$sql.= "  COUNT(*) AS c";
$sql.= " FROM";
$sql.= "  tbl_product";
...
$sql = "SELECT * FROM tbl_product INNER JOIN tbl_product_elearning ON tbl_product.product_id = tbl_product_elearning.product_id";
$sql = "SELECT";
$sql.= "  tbl_product.*,";
...
$template->layout('product/list_recommend.tpl');
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
$objDbConnect->close();
exit();
?>