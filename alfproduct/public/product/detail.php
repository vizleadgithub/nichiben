...
//error_log("[".$dtStr."][start][/srv/alfproduct/public/product/detail.php][".$sql."]\n", 3, '/alflearning-data/alfproduct/logs/page.log');
//LLLLLLLLLLLLLLLLLL
$student_make_complete_list = $objDbConnect->query_fetch_arr($sql);
//LLLLLLLLLLLLLLLLLL
//$dtStr = date("Y-m-d H:i:s") . "." . substr(explode(".", (microtime(true) . ""))[1], 0, 3);
//error_log("[".$dtStr."][end][/srv/alfproduct/public/product/detail.php][".$sql."]\n", 3, '/alflearning-data/alfproduct/logs/page.log');
//LLLLLLLLLLLLLLLLLL
...
//LLLLLLLLLLLLLLLLLL
//$dtStr = date("Y-m-d H:i:s") . "." . substr(explode(".", (microtime(true) . ""))[1], 0, 3);
//error_log("[".$dtStr."][start][/srv/alfproduct/public/product/detail.php][".$sql.$where."]\n", 3, '/alflearning-data/alfproduct/logs/page.log');
//LLLLLLLLLLLLLLLLLL
$product_list = $objDbConnect->query_fetch($sql.$where);
//LLLLLLLLLLLLLLLLLL
//$dtStr = date("Y-m-d H:i:s") . "." . substr(explode(".", (microtime(true) . ""))[1], 0, 3);
//error_log("[".$dtStr."][end][/srv/alfproduct/public/product/detail.php][".$sql.$where."]\n", 3, '/alflearning-data/alfproduct/logs/page.log');
//LLLLLLLLLLLLLLLLLL
if (!$product_list){
	//print($sql.$where);
	//exit();
...
if ($product_list['product_type_add'] == 1 ){
	...
	for($i=1; $i<=MAX_CONTENTS; $i++){
		...
		if( $product_list["contents_contents".$i."so"] != "" ){
		...
		if ($ret_bookmark_so){
			...
			$bookmark_time_sec = 0;
			$product_list["video_view_flg".$i."so"] = false;
			$product_list["video_reading".$i."so"] = 0;
		}
...
//var_dump( $product_list['target'] );
//var_dump( $bar_association );
//var_dump( "[bar_association_id:".$bar_association_id."]" );
...
//var_dump( "[bar_association_id:".$bar_association_id."]" );
//var_dump( $bar_association_branch );
		// 研修情報検索ボタン
		if ($_POST['act'] == 'info_check'){
			...
			if ($bar_association_branch_info){
				...
				//+++++++++++++++++++++++++++++++++++
				if( $bar_association_branch_info['receptionist_end_date']!=null && trim($bar_association_branch_info['receptionist_end_date'])!="" && strtotime($bar_association_branch_info['receptionist_end_date']." 23:59:59")<strtotime(date("Y-m-d 23:59:59")) ){
					$limittime_alert_flg = true;
				} elseif( $bar_association_branch_info['receptionist_start_date']!=null && trim($bar_association_branch_info['receptionist_start_date'])!="" && strtotime($bar_association_branch_info['receptionist_start_date']." 00:00:00")>strtotime(date("Y-m-d 00:00:00")) ){
					$limittime_alert_flg = true;
				} else {
					$limittime_alert_flg = false;
				}
				//+++++++++++++++++++++++++++++++++++
			}
...
for ($i=1; $i<=MAX_CONTENTS; $i++){
	...
	$product_list["contents_baisoku_flg$i"."x15"] = false;
	$product_list["contents_baisoku_flg$i"."so"] = false;
	$product_list["contents_baisoku_flg$i"."x15so"] = false;
	//+++++++++++++++++++++++++++++++++
	/*
	if ( $product_list["contents_open_flg$i"] ){
		$sql = "SELECT idkey, video_logic_name, type_x15 FROM video WHERE video_id='".$product_list["contents_contents$i"]."' limit 1";
		$arr_ckey = $objDbConnect->query_fetch($sql);
		$url = 'https://api.alfstream.com/v1/asset/'.$arr_ckey['idkey']."?authkey=".ALFSTREAM_AUTHKEY;
	...
	*/
	//+++++++++++++++++++++++++++++++++
	/*
	if ( $product_list["contents_open_flg$i"."so"] ){
		$sql = "SELECT idkey, video_logic_name, type_x15 FROM video WHERE video_id='".$product_list["contents_contents$i"."so"]."' limit 1";
		$arr_ckey_so = $objDbConnect->query_fetch($sql);
		$url = 'https://api.alfstream.com/v1/asset/'.$arr_ckey_so['idkey']."?authkey=".ALFSTREAM_AUTHKEY;
		$contents_so = file_get_contents($url,false);
		$res_dat_so = json_decode($contents);
		if($res_dat_so->stat == 200) {
		} else {
		}
	}
	*/
		...
		if( $arr_ckey['type_x15']==1 ){
			...
			if (!empty($get_video_data)){
				...
				if ($get_alf_player['err']==1){
					...
					$product_list["contents_baisoku_flg$i"."x15"] = true;
				} else {
					$product_list["contents_baisoku_flg$i"."x15"] = false;
				}
			}
			*/
			//---------------------------------
			/*
			if( $product_list["contents_baisoku_flg$i"."x15"] == false ){
				$get_video_data = array(
					"ckey"   => $arr_ckey['idkey'],
					"uid"    => $_SESSION['user']['id'],
					"coder"  => "180px15",
	...
	if ( $product_list["contents_open_flg$i"] && $product_list["contents_free_time$i"]!=0 && $product_list["contents_free_time$i"]!=''){
		//+++++++++++++++++++++++++++++++++
		if( $arr_ckey_so['type_x15']==1 ){
			//---------------------------------
			/*
			$get_video_data = array(
				"ckey"   => $arr_ckey_so['idkey'],
				"uid"    => $_SESSION['user']['id'],
				"coder"  => "aux15",
				"playrate" => "1.5",
			);
			...
			if (!empty($get_video_data)){
				...
				if ($get_alf_player['err']==1){
					...
					$product_list["contents_baisoku_flg$i"."x15so"] = true;
				} else {
					$product_list["contents_baisoku_flg$i"."x15so"] = false;
				}
			}
			*/
			...
			if (!empty($get_video_data)){
				...
				if ($get_alf_player['err']==1){
					...
					$product_list["contents_baisoku_flg$i"."so"] = true;
				} else {
					$product_list["contents_baisoku_flg$i"."so"] = false;
				}
			}
...
//print("[".$product_list["contents_baisoku_flg$i"].":".$product_list["contents_baisoku_flg$i"."x15"].":".$product_list["contents_baisoku_flg$i"."so"].":".$product_list["contents_baisoku_flg$i"."x15so"]."]");
...
$template->assign('limittime_alert_flg', $limittime_alert_flg);
...
$GLOBALS['meta_description'] = $product_list['memo_old'];
