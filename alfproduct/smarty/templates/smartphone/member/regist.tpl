...
<table style="width:100%;" class="member_table" cellspacing="0" cellpadding="0">
	...
	<tr>
		...
		<td style="padding:5px;">
			<table><tr>
				<td colspan="3">姓<input type="text" name="name1" id="name1" value="<!--{$arr_input.name1|escape}-->" <!--{$name1_style}--> style="width:40%" />
				&nbsp;
	...
	<tr>
		...
		<td style="padding:5px;">
			<table><tr>
				<td colspan="3">セイ<input type="text" name="kana1" id="kana1" value="<!--{$arr_input.kana1|escape}-->" <!--{$kana1_style}--> style="width:40%" />
				&nbsp;
...
<!--{*
	<tr>
		...
		<th rowspan="2" style="background-color:#e4f3fa;color:#666666;text-align:left;vertical-align:top;padding:5px;">住所</th>
		<td style="padding:5px;">
			<table><tr>
		...
		<td style="padding:5px;">
			<div id="japan">
				<table><tr>
					...
					<th>番地<span style="color:red;">※必須</span></th>
					<td>
		...
		<td style="padding:5px;">
			<!--{foreach name=media from=$mtb_media item=val}-->
				<!--{foreach name=media_child from=$val.media_child item=val1}-->
					...
					<!--{foreach name=loop from=$arr_input.arr_media item=val2}-->
						<!--{if $val2 == $val1.id}-->
						<!--{assign var=checked value='checked'}-->
						<!--{/if}-->
					<!--{/foreach}-->
					<div style="width:30%;float:left;"><label><input type="checkbox" name="media[]" value="<!--{$val1.id|escape}-->"  <!--{$checked}--> /><!--{$val1.name|escape}--></label></div>
				<!--{/foreach}-->
			<!--{/foreach}-->
		</td>
	</tr>
		...
		<tr>
			...
			<td style="padding:5px;">
				...
				<!--{foreach name=loop from=$arr_input.arr_media item=val2}-->
					...
					<!--{/if}-->
				<!--{/foreach}-->
				<label><input type="checkbox" name="media[]" value="<!--{$val1.id|escape}-->"  <!--{$checked}--> /><!--{$val1.name|escape}--></label>
				<!--{/foreach}-->
			</td>
		</tr>
	<!--{/foreach}-->
...
<table style="width:100%;" class="member_table" cellspacing="0" cellpadding="0">
	...
	<tr>
		...
		<td style="padding:5px;">
			<label <!--{$mail_magazine_flag_style}-->><input type="radio" name="mail_magazine_flag" value="1" <!--{if $arr_input.mail_magazine_flag=="1" && $arr_input.mail_magazine_flag!=""}-->checked<!--{/if}--> />受け取る</label>&nbsp;
