...
<form name="playerForm" action="/player/index_tkp.php?term=pc" method="post">
				...
				<!--{* else *}-->
					...
					<!--{elseif $view_btn=='bookmark'}-->
						<!--{if $bookmark_time_sec!=""}-->
							ak_playAndSeek(0, <!--{$bookmark_time_sec|escape}-->);
						<!--{else}-->
							ak_playAndSeek(0, 0);
						<!--{/if}-->
					<!--{/if}-->
...
<!--{else}-->
	<a href="/product/detail_tkp.php?pid=<!--{$pid|escape}-->">戻る</a>
<!--{/if}-->
<!--{if $buy_wait_flg}-->
<!--{else}-->
	<!--{if !$buy_flg}-->
