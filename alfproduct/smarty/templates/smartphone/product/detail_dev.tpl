<script type="text/javascript">
	...
	function playerOpenWindowSubmit(vid,ftn,ccno,view_btn,vid2,codec){
		...
		<!--{if !$is_sp}-->
			document.playerForm.action = "/player/index_dev.php?term=pc";
		<!--{else}-->
			document.playerForm.action = "/player/index_dev.php?term=sp";
		<!--{/if}-->
						...
						|| ($product_list.contents_contents25so!='' && $product_list.contents_view_flg25so) 
							...
							<div style="text-align:center;">
								<!--{section name=contents_contents loop=$section_max_contents start=1}-->
									...
									<!--{assign var=contents_contents_name_so_key value="contents_contents"|cat:$ccno|cat:"so_name"}-->
			...
			<div style="width:290px;float:right;">
				...
				<!--{foreach from=$arr_exam2 key=exam2_i item=exam2_row}-->
					<div style="text-align:left;">
						<a href="#exam2_btn_area" class="btn_gray">レビューを見る</a>
															...
															<!--{if $product_list.$submit_possible_flg_test_key}-->
																<!--{if !$product_list.$exam_test_all_answered_key || ($product_list.$exam_test_all_answered_key && $product_list.$exam_test_passing_flg_key=='0' && $product_list.$resubmit_flag_test_key=='1')}-->
																	...
																	<!--{elseif $product_list.$display_format_test_key==='1'}-->
																		...
																		<!--{else}-->
																			<a class="test_btn" href="/exam/inde×1.php?pid=<!--{$pid}-->&ccno=<!--{$ccno}-->&eid=<!--{$product_list.$exam_id_test_key}-->&qid=<!--{$product_list.$exam_id_question_key}-->&eno=1"><!--{if $product_list.$btn_type_key=='2'}-->アンケートに回答する<!--{else}-->テストを受ける<!--{/if}--></a>
																		<!--{/if}-->
																	<!--{* 一問一答形式（一括採点） *}-->
																	<!--{elseif $product_list.$display_format_test_key==='2'}-->
																		<!--{if $product_list.$exam_test_all_answered_key}-->
															...
															<!--{if $product_list.$submit_possible_flg_test_key}-->
																<!--{if !$product_list.$exam_test_all_answered_key || ($product_list.$exam_test_all_answered_key && $product_list.$exam_test_passing_flg_key=='0' && $product_list.$resubmit_flag_test_key=='1')}-->
																	...
																	<!--{elseif $product_list.$display_format_test_key==='1'}-->
																		...
																		<!--{else}-->
																			<a class="test_btn" href="/exam/inde×1.php?pid=<!--{$pid}-->&ccno=<!--{$ccno}-->&eid=<!--{$product_list.$exam_id_test_key}-->&eno=1"><!--{if $product_list.$btn_type_key=='2'}-->アンケートに回答する<!--{else}-->テストを受ける<!--{/if}--></a>
																		<!--{/if}-->
																	<!--{* 一問一答形式（一括採点） *}-->
																	<!--{elseif $product_list.$display_format_test_key==='2'}-->
																		<!--{if $product_list.$exam_test_all_answered_key}-->
																...
																<!--{if $product_list.$submit_possible_flg_test_key}-->
																	<!--{if !$product_list.$exam_test_all_answered_key || ($product_list.$exam_test_all_answered_key && $product_list.$exam_test_passing_flg_key=='0' && $product_list.$resubmit_flag_test_key=='1')}-->
																		...
																		<!--{elseif $product_list.$display_format_test_key==='1'}-->
																			...
																			<!--{else}-->
																				<a href="/exam/inde×1.php?pid=<!--{$pid}-->&ccno=<!--{$ccno}-->&eid=<!--{$product_list.$exam_id_test_key}-->&qid=<!--{$product_list.$exam_id_question_key}-->&eno=1"><!--{if $product_list.$btn_type_key=='2'}-->アンケートに回答する<!--{else}-->テストを受ける<!--{/if}--></a>
																			<!--{/if}-->
																		<!--{* 一問一答形式（一括採点） *}-->
																		<!--{elseif $product_list.$display_format_test_key==='2'}-->
																			<!--{if $product_list.$exam_test_all_answered_key}-->
...
<!--{foreach from=$arr_exam2 key='exam2_i' item='exam2_row'}-->
	本レビューは，みなさまからいただいたアンケートを基に掲載しております。<br>
	レビュー一覧　（レビュー　<!--{$exam2_row.student|@count}-->件）<br>
	<br>
