		...
		<div style="float:left;font-size:16px;line-height: 22px;color:#525252;width:650px;margin-left:15px;margin-top:20px;border: none;margin-bottom:20px;">
			...
			<table style="width:100%;" class="member_table" cellspacing="0" cellpadding="0">
				...
				<tr style="<!--{if $cycle_bg=="1"}-->background-color: #F5F5F5;<!--{else}-->background-color: #FFFFFF;<!--{/if}-->">
					<td style="<!--{if $cycle_bg=="1"}-->background-color: #F5F5F5;<!--{else}-->background-color: #FFFFFF;<!--{/if}-->border:solid 1px #AB9983;padding:5px;text-align:center;vertical-align:middle;">研修日<br /><!--{$val.dates|escape}--></td>
					...
					<td rowspan="2" style="<!--{if $cycle_bg=="1"}-->background-color: #F5F5F5;<!--{else}-->background-color: #FFFFFF;<!--{/if}-->border:solid 1px #AB9983;padding:5px;text-align:right;vertical-align:middle;"><!--{$val.price|escape|number_format}-->円</td>
					<td rowspan="2" style="<!--{if $cycle_bg=="1"}-->background-color: #F5F5F5;<!--{else}-->background-color: #FFFFFF;<!--{/if}-->border:solid 1px #AB9983;padding:5px;text-align:center;vertical-align:middle;"><!--{if $val.del_flg==='0'}--><a href="/product/detail.php?pid=<!--{$val.product_id|escape}-->" style="color:#796A57;">詳細</a><!--{/if}--></td>
					<td rowspan="2" style="<!--{if $cycle_bg=="1"}-->background-color: #F5F5F5;<!--{else}-->background-color: #FFFFFF;<!--{/if}-->border:solid 1px #AB9983;padding:5px;text-align:center;vertical-align:middle;">
						<!--{if $val.download_flg == 1}-->
							<!--{if $val.ticket_flg == 1}-->
								<img src="/img/mypage/issue_btn_comp.png">
							<!--{else}-->
								<script type="text/javascript">
									function non_download<!--{$val.order_detail_id|escape}-->() {
										//document.getElementById('download_btn<!--{$val.order_detail_id|escape}-->').disabled = true;
										//document.getElementById('download_div<!--{$val.order_detail_id|escape}-->').innerHTML = '<img src="/img/mypage/issue_btn_comp.png">';
										window.open("ticket_download.php?odid=<!--{$val.order_detail_id|escape}-->", "", "width=10,height=10");
									}
								</script>
								<div id="download_div<!--{$val.order_detail_id|escape}-->">
									<a id="download_btn<!--{$val.order_detail_id|escape}-->" href="javascript: void(0);" onclick="non_download<!--{$val.order_detail_id|escape}-->();" ><img src="/img/mypage/issue_btn.png"></a>
								</div>
							<!--{/if}-->
						<!--{else}-->
						<!--{/if}-->
				...
				<tr style="<!--{if $cycle_bg=="1"}-->background-color: #F5F5F5;<!--{else}-->background-color: #FFFFFF;<!--{/if}-->">
					<td style="<!--{if $cycle_bg=="1"}-->background-color: #F5F5F5;<!--{else}-->background-color: #FFFFFF;<!--{/if}-->border:solid 1px #AB9983;padding:5px;text-align:center;vertical-align:middle;">購入手続完了日<br /><!--{if $val.payment_status=='2'}--><!--{$val.payment_date|escape}--><!--{else}-->-<!--{/if}--></td>
					<td style="<!--{if $cycle_bg=="1"}-->background-color: #F5F5F5;<!--{else}-->background-color: #FFFFFF;<!--{/if}-->border:solid 1px #AB9983;padding:5px;text-align:left;vertical-align:middle;">会名：<!--{$val.name|escape}--><br />会場：<!--{$val.hall|escape}--></td>
				</tr>
				<!--{/foreach}-->
			</table>
