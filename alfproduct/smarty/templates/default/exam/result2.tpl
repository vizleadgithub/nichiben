			...
			<!--{assign var=row_no value=$smarty.foreach.loop.iteration}-->
				<div class="problem">
					...
					<div id="btn_area_<!--{$row.exam_problem_id}-->" class="btn_area">
						<a class="btn" href="/exam/resubmit_index2.php?pid=<!--{$pid|escape}-->&ccno=<!--{$ccno|escape}-->&eid=<!--{$eid|escape}-->&eno=<!--{$row_no}-->&eflg=1<!--{if $qid!=''}-->&qid=<!--{$qid|escape}--><!--{/if}-->"><!--{if $btn_type=='2'}-->回答修正画面へ<!--{else}-->解答修正画面へ<!--{/if}--></a>
					</div>
				</div>
				<br>
			<!--{/foreach}-->
			...
			<!--{assign var=row_no_q value=$smarty.foreach.loop_q.iteration}-->
				<div class="problem">
						...
						<div class="problem_contents">
							...
							<!--{if $row_q.answer_kind==1}-->
								...
								<!--{assign var=row_no1_q value=$smarty.foreach.loop1_q.iteration}-->
									<div class="problem_content">
										<div class="content1"><input type="radio" id="exam_problem_q_<!--{$row_q.exam_problem_id}-->_<!--{$row_no1_q}-->" name="exam_problem_q_<!--{$row_q.exam_problem_id}-->[]" value="<!--{$row1_q.no}-->" disabled <!--{if isset($answered_info_q) && isset($answered_info_q[$row1_q.exam_problem_id]) && isset($answered_info_q[$row1_q.exam_problem_id].exam_answer_contents) && $row1_q.no==$answered_info_q[$row_q.exam_problem_id].exam_answer_contents[0]}-->checked<!--{/if}-->><!--{$row_no1_q}-->.</div>
							...
							<!--{elseif $row_q.answer_kind==2}-->
								...
								<!--{assign var=row_no1_q value=$smarty.foreach.loop1_q.iteration}-->
									<div class="problem_content">
										<div class="content1"><input type="checkbox" id="exam_problem_q_<!--{$row_q.exam_problem_id}-->_<!--{$row_no1_q}-->" name="exam_problem_q_<!--{$row_q.exam_problem_id}-->[]" value="<!--{$row1_q.no}-->" disabled <!--{if isset($answered_info_q) && isset($answered_info_q[$row1_q.exam_problem_id]) && isset($answered_info_q[$row1_q.exam_problem_id].exam_answer_contents) && array_search($row1_q.no, $answered_info_q[$row_q.exam_problem_id].exam_answer_contents)!==false}-->checked<!--{/if}-->><!--{$row_no1_q}-->.</div>
										<div class="content2"><label for="exam_problem_q_<!--{$row_q.exam_problem_id}-->_<!--{$row_no1_q}-->"><!--{$row1_q.word|escape|nl2br}--></label></div>
									</div>
									<br style="clear:both;">
								<!--{/foreach}-->
					...
					<div id="btn_area_<!--{$row_q.exam_problem_id}-->" class="btn_area">
						<a class="btn" href="/exam/resubmit_index2.php?pid=<!--{$pid|escape}-->&ccno=<!--{$ccno|escape}-->&eid=<!--{$eid|escape}-->&eno=<!--{$row_no_q+$row_no}-->&eflg=1<!--{if $qid!=''}-->&qid=<!--{$qid|escape}--><!--{/if}-->"><!--{if $btn_type_q=='2'}-->回答修正画面へ<!--{else}-->解答修正画面へ<!--{/if}--></a>
					</div>
				</div>
				<br>
			<!--{/foreach}-->
