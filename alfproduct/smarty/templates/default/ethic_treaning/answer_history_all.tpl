...
<!--{foreach name=loop from=$arr_list key=key item=item}-->
<!--{assign var=loop_cnt value=$smarty.foreach.loop.iteration}-->
<!--{assign var=answer_date value='answer_date'|cat:$loop_cnt}-->
<!--{assign var=question value='question'|cat:$loop_cnt}-->
		...
		<div style="float:left;font-size:16px;line-height: 22px;color:#525252;width:880px;margin-left:15px;margin-top:20px;border: none;margin-bottom:20px;">
			<div style="width:100%;">
				...
				<table style="width:100%;text-align:left;">
					<!--{foreach from=$item.branch_info key=key1 item=item1}-->
					<!--{assign var=answer_ethic_branch_id value='answer_ethic_branch_id'|cat:$loop_cnt}-->
						<!--{if $history_answer.$answer_ethic_branch_id==$key1 && $item1.answer_flg==1}-->
						...
						<tr style="border-bottom:solid 1px #000000;<!--{$style_background}-->">
							...
							<td style="vertical-align:middle;"><!--{$item1.question_branch|escape}--></td>
						</tr>
					<!--{/foreach}-->
				</table>
			</div>
