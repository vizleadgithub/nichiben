...
*}-->
	<tr>
		...
		<td>
			...
			<!--{if $arr_input.product_kind_flg=='2'}-->
			e-ライブ<br />
			<!--{$arr_input.live_training_product_name|escape}-->
			<!--{/if}-->
	...
	<tr>
		...
		<td>
			...
			<!--{foreach from=$arr_category item="row"}-->
				<!--{if in_array($row.term_id,$arr_input.arr_term_id)}--><li><!--{$row.name|escape}-->&nbsp;&nbsp;</li><!--{/if}-->
				<ul style="margin-left:15px;list-style-type:none;">
				<!--{foreach from=$row.categorys item="row2"}-->
					<!--{if in_array($row2.term_id,$arr_input.arr_term_id)}--><li>→<!--{$row2.name|escape}-->&nbsp;&nbsp;</li><!--{/if}-->
					<ul style="margin-left:15px;list-style-type:none;">
					<!--{foreach from=$row2.categorys item="row3"}-->
						<!--{if in_array($row3.term_id,$arr_input.arr_term_id)}--><li>→→<!--{$row3.name|escape}-->&nbsp;&nbsp;</li><!--{/if}-->
						<ul style="margin-left:15px;list-style-type:none;">
						<!--{foreach from=$row3.categorys item="row4"}-->
							<!--{if in_array($row4.term_id,$arr_input.arr_term_id)}--><li>→→→<!--{$row4.name|escape}-->&nbsp;&nbsp;</li><!--{/if}-->
							<ul style="margin-left:15px;list-style-type:none;">
							<!--{foreach from=$row4.categorys item="row5"}-->
								<!--{if in_array($row5.term_id,$arr_input.arr_term_id)}--><li>→→→→<!--{$row5.name|escape}-->&nbsp;&nbsp;</li><!--{/if}-->
							<!--{/foreach}-->
							</ul>
						<!--{/foreach}-->
						</ul>
...
<!--{*if $arr_input.$contents_thumbnail_key!='' && $arr_input.$contents_contents_name_key!='' && $arr_input.$contents_free_time_key!=''*}-->
	...
	<tr>
		...
		<td>
			...
			<!--{if $arr_input.$exam_id_question_key==''}-->
				未設定
			<!--{else}-->
				<!--{foreach from=$exam_list item=exam}-->
	...
	<!--{section name=contents_download loop=$section_contents_download start=1}-->
		<tr>
			...
			<td>
				<!--{assign var=contents_download_name_key value="contents_download_before"|cat:$smarty.section.contents_loop.index|cat:"_"|cat:$smarty.section.contents_download.index}-->
				<!--{if $arr_input.$contents_download_name_key==''}-->
					未設定
				<!--{else}-->
					<!--{$arr_input.$contents_download_name_key|escape}-->
				<!--{/if}-->
			</td>
		</tr>
	<!--{/section}-->
