...
*}-->
	<tr>
		...
		<td>
			<label><input type="radio" name="publish_flg" id="publish_flg0" value="0" <!--{if $arr_input.publish_flg==0}-->checked<!--{/if}--> />しない</label>
			<label><input type="radio" name="publish_flg" id="publish_flg1" value="1" <!--{if $arr_input.publish_flg==1}-->checked<!--{/if}--> />する</label>
		</td>
	</tr>
	<tr>
...
*}-->
	<tr>
		...
		<td>
			<!--{html_options name=bar_association_year options=$bar_association_year selected=$arr_input.bar_association_year}-->
		</td>
	</tr>
	<tr>
		<th style="vertical-align:middle;">倫理研修問題<span style="color:red;">※</span></th>
		<td>
			<!--{html_options name=ethic_group_id options=$ethic_group selected=$arr_input.ethic_group_id}-->
		</td>
	</tr>
	<tr>
		<th style="vertical-align:middle;">一括ダウンロード用資料</th>
...
<table id="contentsTable<!--{$smarty.section.contents_loop.index}-->" style="display:none;" />
	...
	<tr>
		...
		<td>
			...
			<!--<span id="spa_<!--{$contents_contents_key}-->"><!--{$arr_input.$contents_contents_name_key|escape}--></span>-->
