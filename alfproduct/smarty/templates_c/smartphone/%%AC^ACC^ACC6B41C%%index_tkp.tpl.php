<?php /* Smarty version 2.6.27, created on 2020-11-11 19:00:43
         compiled from /srv/alfproduct/smarty/templates/smartphone/player/index_tkp.tpl */ ?>
<?php require_once(SMARTY_CORE_DIR . 'core.load_plugins.php');
smarty_core_load_plugins(array('plugins' => array(array('modifier', 'escape', '/srv/alfproduct/smarty/templates/smartphone/player/index_tkp.tpl', 3, false),)), $this); ?>
<?php if ($this->_tpl_vars['next_contents_flg']): ?>
<form name="playerForm" action="/player/index_tkp.php?term=sp" method="post">
...
/<?php if ($this->_tpl_vars['codec'] == 'baisoku'): ?>2<?php elseif ($this->_tpl_vars['codec'] == 'x11'): ?>1.1<?php elseif ($this->_tpl_vars['codec'] == 'x12'): ?>1.2<?php elseif ($this->_tpl_vars['codec'] == 'x13'): ?>1.3<?php elseif ($this->_tpl_vars['codec'] == 'x14'): ?>1.4<?php elseif ($this->_tpl_vars['codec'] == 'x15'): ?>1.5<?php elseif ($this->_tpl_vars['codec'] == 'x16'): ?>1.6<?php elseif ($this->_tpl_vars['codec'] == 'x17'): ?>1.7<?php elseif ($this->_tpl_vars['codec'] == 'x18'): ?>1.8<?php elseif ($this->_tpl_vars['codec'] == 'x19'): ?>1.9<?php elseif ($this->_tpl_vars['codec'] == 'x20'): ?>2<?php elseif ($this->_tpl_vars['codec'] == 'aux11'): ?>1.1<?php elseif ($this->_tpl_vars['codec'] == 'aux12'): ?>1.2<?php elseif ($this->_tpl_vars['codec'] == 'aux13'): ?>1.3<?php elseif ($this->_tpl_vars['codec'] == 'aux14'): ?>1.4<?php elseif ($this->_tpl_vars['codec'] == 'aux15'): ?>1.5<?php elseif ($this->_tpl_vars['codec'] == 'aux16'): ?>1.6<?php elseif ($this->_tpl_vars['codec'] == 'aux17'): ?>1.7<?php elseif ($this->_tpl_vars['codec'] == 'aux18'): ?>1.8<?php elseif ($this->_tpl_vars['codec'] == 'aux19'): ?>1.9<?php elseif ($this->_tpl_vars['codec'] == 'aux20'): ?>2<?php else: ?>1<?php endif; ?>));//bookmark_time_sec
						}
	...
	function ak_initialized_on() {
		if (typeof(jwplayer) == "function") {
					...
					if (timeSeek == 0) {
						if (evt.position < evt.offset) {
							if( timeBeforeFlg == false ){
								...
								timeBefore = evt.position;
								evt.offset = evt.position;
								jwplayer().seek(timeBefore);
							}
						}
					}
				<?php endif; ?>
			});
			jwplayer().on("seeked", function(){//jw8
				<?php if ($this->_tpl_vars['set_btn_flg'] && $this->_tpl_vars['video_id2'] == ''): ?>
					if( timeBeforeFlg == true ){
						jwplayer().seek(timeBefore);
			...
			$(".player_area").show();//スマフォ版はプレイヤーの準備できるまで操作してほしくない
		}
		...
		if (ak_eventTime != null) { clearTimeout(ak_eventTime); }
			...
			if (time >= 5){
				<?php if ($this->_tpl_vars['video_popup'] == 1): ?>
					//ポップアップが有効な動画の場合
					//+++++++++++++++++++++++++++
					if( video_pop_time >=pop_up_time  && !video_pop_ok && timeBeforeFlg==false ){
						pause_time = time-1;
						video_pop_ok = true;
						video_check_pop();
					}
					if( video_pop_time >=pop_stop_time  && video_pop_ok && timeBeforeFlg==false ){
						video_check_pop();
						video_check_stop();
					}
					//+++++++++++++++++++++++++++
...
";
 ...
 -->
	...
	<div class="player_area" style="display:none;">
...
<?php else: ?>
	<font size="2">再生速度は，映像右下の歯車ボタン　<img src="/img/haguruma.png" alt="歯車ボタン" width="20" height="20" />　をクリックすると調整できます。</font>
	<a href="/product/detail_tkp.php?pid=<?php echo ((is_array($_tmp=$this->_tpl_vars['pid'])) ? $this->_run_mod_handler('escape', true, $_tmp) : smarty_modifier_escape($_tmp)); ?>
">戻る</a>
<?php endif; ?>
<?php if ($this->_tpl_vars['buy_wait_flg']): ?>
<?php else: ?>
