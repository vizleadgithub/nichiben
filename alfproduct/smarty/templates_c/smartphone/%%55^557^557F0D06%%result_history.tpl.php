<?php /* Smarty version 2.6.27, created on 2014-01-24 17:23:30
         compiled from /srv/alfproduct/smarty/templates/smartphone/ethic_treaning/result_history.tpl */ ?>
<?php require_once(SMARTY_CORE_DIR . 'core.load_plugins.php');
smarty_core_load_plugins(array('plugins' => array(array('modifier', 'escape', '/srv/alfproduct/smarty/templates/smartphone/ethic_treaning/result_history.tpl', 22, false),array('modifier', 'string_format', '/srv/alfproduct/smarty/templates/smartphone/ethic_treaning/result_history.tpl', 51, false),)), $this); ?>
							...
							<td style="background-color: #FFFFFF;border:solid 1px #AB9983;padding:5px;text-align:center;">
								<table><tr>
									<td style="text-align:center;width:280px;">
										...
										<a href="javascript: void(0);" ontouchmove="touchMoveFlag=true;" ontouchend="playerEthicCommentaryFormSubmit('<?php echo ((is_array($_tmp=$this->_tpl_vars['row']['video_id'])) ? $this->_run_mod_handler('escape', true, $_tmp) : smarty_modifier_escape($_tmp)); ?>
');return false;"><img src="/img/lecture/commentary_btn.png"></a>
			...
			<?php if (! empty ( $this->_tpl_vars['arr_list_add'] )): ?>
				...
				<table style="width:100%;margin-top:20px;" class="member_table" cellspacing="0" cellpadding="0">
					...
					<?php $_from = $this->_tpl_vars['arr_list_add']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }$this->_foreach['loop'] = array('total' => count($_from), 'iteration' => 0);
if ($this->_foreach['loop']['total'] > 0):
    foreach ($_from as $this->_tpl_vars['key'] => $this->_tpl_vars['row']):
        $this->_foreach['loop']['iteration']++;
?>
...
function playerEthicCommentaryFormSubmit(vid,ftn,ccno,view_btn){
    ...
    //}, 1000);
    if (!touchMoveFlag){
        playerEthicCommentaryOpenWindowSubmit(vid,ftn,ccno,view_btn);
    }
    touchMoveFlag = false;
}
function playerEthicCommentaryOpenWindowSubmit(vid,ftn,ccno,view_btn){
    ...
    document.playerForm.action = "/player/player_ethic_commentary.php?term=sp";
