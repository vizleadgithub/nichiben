...
/* ステージングでは外す　ここから */
		// SSOログインチェック(トークンチェック)
		$api_url = SSO_TOKEN_CHECK_API . "?subjectid=".$_COOKIE['iPlanetDirectoryPro'];
		exec('curl -d client_ip='.urlencode ($_SERVER["REMOTE_ADDR"]).' -d client_ua='.urlencode ($_SERVER["HTTP_USER_AGENT"]).' '.$api_url, $output);
		$res_content = $output[0];
		if (strpos($res_content, 'exception.name') !== false){
...
//var_dump($arr_term_id);
		...
		$arr_pan_cat = array();
		for($i=0;$i<count($arr_category);$i++){
			if(  in_array( $arr_category[$i]["term_id"], $arr_term_id )  &&  count($arr_pan_cat[0])==0  ){
				$arr_pan_cat[0]["id"] = $arr_category[$i]["term_id"];
				$arr_pan_cat[0]["name"] = $arr_category[$i]["name"];
				for($n=0;$n<count($arr_category[$i]["categorys"]);$n++){
					if(  in_array( $arr_category[$i]["categorys"][$n]["term_id"], $arr_term_id )  &&  count($arr_pan_cat[1])==0  ){
						$arr_pan_cat[1]["id"] = $arr_category[$i]["categorys"][$n]["term_id"];
						$arr_pan_cat[1]["name"] = $arr_category[$i]["categorys"][$n]["name"];
						for($m=0;$m<count($arr_category[$i]["categorys"][$n]["categorys"]);$m++){
							if(  in_array( $arr_category[$i]["categorys"][$n]["categorys"][$m]["term_id"], $arr_term_id )  &&  count($arr_pan_cat[2])==0  ){
								$arr_pan_cat[2]["id"] = $arr_category[$i]["categorys"][$n]["categorys"][$m]["term_id"];
								$arr_pan_cat[2]["name"] = $arr_category[$i]["categorys"][$n]["categorys"][$m]["name"];
								for($x=0;$x<count($arr_category[$i]["categorys"][$n]["categorys"][$m]["categorys"]);$x++){
									if(  in_array( $arr_category[$i]["categorys"][$n]["categorys"][$m]["categorys"][$x]["term_id"], $arr_term_id )  &&  count($arr_pan_cat[3])==0  ){
										$arr_pan_cat[3]["id"] = $arr_category[$i]["categorys"][$n]["categorys"][$m]["categorys"][$x]["term_id"];
										$arr_pan_cat[3]["name"] = $arr_category[$i]["categorys"][$n]["categorys"][$m]["categorys"][$x]["name"];
										for($y=0;$y<count($arr_category[$i]["categorys"][$n]["categorys"][$m]["categorys"][$x]["categorys"]);$y++){
											if(  in_array( $arr_category[$i]["categorys"][$n]["categorys"][$m]["categorys"][$x]["categorys"][$y]["term_id"], $arr_term_id )  &&  count($arr_pan_cat[4])==0  ){
												$arr_pan_cat[4]["id"] = $arr_category[$i]["categorys"][$n]["categorys"][$m]["categorys"][$x]["categorys"][$y]["term_id"];
												$arr_pan_cat[4]["name"] = $arr_category[$i]["categorys"][$n]["categorys"][$m]["categorys"][$x]["categorys"][$y]["name"];
												for($z=0;$z<count($arr_category[$i]["categorys"][$n]["categorys"][$m]["categorys"][$x]["categorys"][$y]["categorys"]);$z++){
													if(  in_array( $arr_category[$i]["categorys"][$n]["categorys"][$m]["categorys"][$x]["categorys"][$y]["categorys"][$z]["term_id"], $arr_term_id )  &&  count($arr_pan_cat[5])==0  ){
														$arr_pan_cat[5]["id"] = $arr_category[$i]["categorys"][$n]["categorys"][$m]["categorys"][$x]["categorys"][$y]["categorys"][$z]["term_id"];
														$arr_pan_cat[5]["name"] = $arr_category[$i]["categorys"][$n]["categorys"][$m]["categorys"][$x]["categorys"][$y]["categorys"][$z]["name"];
													}
												}
											}
										}
...
function get_bar_association_info($bar_association_branch_id){
	...
	WHERE
	  T2.bar_association_branch_id = '".mysql_real_escape_string($bar_association_branch_id)."'
	";
	$res = $objDbConnect->query_fetch($sql);
	if ($res){
		return $res;
...
function insert_wp_posted_article_product($product_id, $status=0){
	...
	WHERE
	  T1.product_id = '".mysql_real_escape_string($product_id)."'
	";
	$res = $objDbConnect->query_fetch($sql);
	if ($res){
		// スラッグ名(新着情報：固定)
		...
		(
		  '1',
		  '".mysql_real_escape_string($posttime)."',
		  '".mysql_real_escape_string($content)."',
		  '".mysql_real_escape_string($title)."',
		  'publish',
		  'open',
		  'open',
		  '".mysql_real_escape_string($slug.'-product')."',
		  '".mysql_real_escape_string($posttime)."',
		  '0',
		  'post',
		  '".mysql_real_escape_string($res['product_type_add'])."',
		  '$status',
		  '".mysql_real_escape_string($res['product_id'])."',
		  '".mysql_real_escape_string($res['target'])."'
		)
		";
...
function is_sp () {
	...
	if(eregi("Android", $agent)){//android
		$ret = true;
	} else {
		$useragents = array(
			'iPhone', // Apple iPhone
...
function get_passport_price($objDbConnect, $bar_association_year){
	...
	$sql.= " WHERE";
	$sql.= "  tbl_product_passport.passport_target LIKE '%|".mysql_real_escape_string($passport_target)."|%'";
...
function get_exam_list($bar_association_id){
	...
	($bar_association_id > 1 ?
		" AND teacher.bar_association_id = '".mysql_real_escape_string($bar_association_id)."' "
		: ''
	).
