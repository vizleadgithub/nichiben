...
//	private $admin_name        = "管理者";		//管理者名（common_lang.phpから取得に変更）
	...
	//	$sql .= "   AND status <> 9 ";
		...
		if($param['cource_id'] != ''){
		//	$sql .= "   AND book_library_id IN (SELECT book_library_id FROM book_library_lecture WHERE cource_id IN ({$this->db->escape($param['cource_id'])}) )";
		//	[2013/01/07]修正
			$sql .= "   AND book_library_id IN (SELECT book_library_id FROM book_library_lecture WHERE cource_id IN ({$param['cource_id']}) )";
		}else{
