...
if( $pid=="" ){
	...
	$sql.= " (  SELECT COUNT(*) AS c FROM tbl_order_detail WHERE tbl_order_detail.create_date<'2016-07-01 00:00:00' AND tbl_order_detail.payment_status=2 AND tbl_order_detail.product_type_add=1 AND tbl_order_detail.product_id=tbl_product.product_id $product_type_add_where ) AS product_type_add1_count, ";
	$sql.= " (  SELECT COUNT(*) AS c FROM tbl_order_detail WHERE tbl_order_detail.create_date<'2016-07-01 00:00:00' AND tbl_order_detail.payment_status=2 AND tbl_order_detail.product_type_add=1 AND tbl_order_detail.product_id=tbl_product.product_id AND tbl_order_detail.video_complete_flg=1 $product_type_add_where ) AS product_type_add1_end_count, ";

	// [NBR-239] payment_status=3（仮払い等）はフロント受講履歴(lesson_list2.php)と揃えて対象外にする。create_date条件(過去データ集計用)はそのまま維持
	$sql.= " (  SELECT COUNT(*) AS c FROM tbl_order_detail WHERE tbl_order_detail.create_date<'2016-07-01 00:00:00' AND ( tbl_order_detail.payment_status = 1 OR tbl_order_detail.payment_status = 2 ) AND tbl_order_detail.product_type_add=2 AND tbl_order_detail.bar_association_branch_id=rel_product_bar_association_branch_bar_association_branch_id AND tbl_order_detail.product_id=tbl_product.product_id $product_type_add_where ) AS product_type_add2_count, ";
	$sql.= " (  SELECT COUNT(*) AS c FROM tbl_order_detail WHERE tbl_order_detail.create_date<'2016-07-01 00:00:00' AND ( tbl_order_detail.payment_status = 1 OR tbl_order_detail.payment_status = 2 ) AND tbl_order_detail.product_type_add=2 AND tbl_order_detail.bar_association_branch_id=rel_product_bar_association_branch_bar_association_branch_id AND tbl_order_detail.product_id=tbl_product.product_id AND tbl_order_detail.participation_flg=1 $product_type_add_where ) AS product_type_add2_end_count, ";
	$sql.= " (  SELECT COUNT(*) AS c FROM tbl_order_detail WHERE tbl_order_detail.create_date<'2016-07-01 00:00:00' AND ( tbl_order_detail.payment_status = 1 OR tbl_order_detail.payment_status = 2 ) AND tbl_order_detail.product_type_add=2 AND tbl_order_detail.product_id=tbl_product.product_id ) AS product_type_add2_count_kako, ";
	...
	$sql.= " (  SELECT COUNT(*) AS c FROM tbl_ethic_question_history INNER JOIN student ON tbl_ethic_question_history.student_id = student.student_id WHERE tbl_ethic_question_history.create_date<'2016-07-01 00:00:00' AND tbl_ethic_question_history.product_id=tbl_product.product_id $product_type_add3_where ) AS product_type_add3_count, ";
	$sql.= " (  SELECT COUNT(*) AS c FROM tbl_ethic_question_history INNER JOIN student ON tbl_ethic_question_history.student_id = student.student_id WHERE tbl_ethic_question_history.create_date<'2016-07-01 00:00:00' AND tbl_ethic_question_history.product_id=tbl_product.product_id AND (tbl_ethic_question_history.status=2 OR tbl_ethic_question_history.status=5 OR tbl_ethic_question_history.status=7) $product_type_add3_where ) AS product_type_add3_end_count ";
	/*
...
//exit();
	...
	for($i=0;$i<count($ret);$i++){
		...
		$sql2.= " WHERE tbl_order_detail.create_date<'2016-07-01 00:00:00' AND tbl_order_detail.product_id ='".$pid."' ";
		...
		$sql2.= "           WHERE A.create_date<'2016-07-01 00:00:00' AND A.product_id  ='".$pid."' ";
		...
		if($ret[$i]["product_type_add1_count"]){
			...
			$sql2_where.= " AND tbl_order_detail.create_date<'2016-07-01 00:00:00' ";
...
if( $pid=="" ){
	...
	//----------------------------------------------------------
	fwrite($filepoint,  mb_convert_encoding("商品ID,商品名,商品コード,弁護士会,実施日,総受講者,受講完了者,受付期間(開始),受付期間(終了),\r\n", "SJIS", "UTF-8"));
	...
	for($i=0;$i<count($ret);$i++){
		...
		$temp .= mb_convert_encoding('"'. trim($ret[$i]["product_id"] )     . '",',  "SJIS",  "UTF-8");
		$temp .= mb_convert_encoding('"'. trim($ret[$i]["product_name"] )   . '",',  "SJIS",  "UTF-8");
		$temp .= mb_convert_encoding('"'. trim($ret[$i]["product_code"] )   . '",',  "SJIS",  "UTF-8");
		$temp .= mb_convert_encoding('"'. trim($ret[$i]["bar_association_branch_name"] ) . '",',  "SJIS",  "UTF-8");
		$temp .= mb_convert_encoding('"'. trim($ret[$i]["dates"] )          . '",',  "SJIS",  "UTF-8");

		if( $ret[$i]["product_type_add"]=="1" ){
			$temp .= mb_convert_encoding('"'. trim($ret[$i]["product_type_add1_count"] )     . '",',  "SJIS",  "UTF-8");
			$temp .= mb_convert_encoding('"'. trim($ret[$i]["product_type_add1_end_count"] )   . '",',  "SJIS",  "UTF-8");
		} elseif( $ret[$i]["product_type_add"]=="2" ){
			$temp .= mb_convert_encoding('"'. trim($ret[$i]["product_type_add2_count"] )     . '",',  "SJIS",  "UTF-8");
			$temp .= mb_convert_encoding('"'. trim($ret[$i]["product_type_add2_end_count"] )   . '",',  "SJIS",  "UTF-8");
		} elseif( $ret[$i]["product_type_add"]=="3" ){
			$temp .= mb_convert_encoding('"'. trim($ret[$i]["product_type_add3_count"] )     . '",',  "SJIS",  "UTF-8");
			$temp .= mb_convert_encoding('"'. trim($ret[$i]["product_type_add3_end_count"] )   . '",',  "SJIS",  "UTF-8");
		} elseif( $ret[$i]["product_type_add"]=="4" ){
			$temp .= mb_convert_encoding('"",',  "SJIS",  "UTF-8");
			$temp .= mb_convert_encoding('"",',  "SJIS",  "UTF-8");
		}

		$temp .= mb_convert_encoding('"'. trim($ret[$i]["start_date"] )     . '",',  "SJIS",  "UTF-8");
		$temp .= mb_convert_encoding('"'. trim($ret[$i]["end_date"] )       . '",',  "SJIS",  "UTF-8");

		$temp .= mb_convert_encoding("\r\n", "SJIS", "UTF-8");
	...
	echo mb_convert_encoding('商品名,',  "SJIS", "UTF-8");
	echo mb_convert_encoding('"' . $arr_product["product_name"] . '",',  "SJIS", "UTF-8");
	echo mb_convert_encoding("\r\n", "SJIS", "UTF-8");

	echo mb_convert_encoding('商品コード,',  "SJIS", "UTF-8");
	echo mb_convert_encoding('"' . $arr_product["product_code"] . '",',  "SJIS", "UTF-8");
	echo mb_convert_encoding("\r\n", "SJIS", "UTF-8");

	echo mb_convert_encoding('商品種別,',  "SJIS", "UTF-8");
	if( $arr_product["product_type_add"]=="1" ){
		if ($arr_product["product_kind_flg"]=="1" ){
			echo mb_convert_encoding('e-ラーニング,',  "SJIS", "UTF-8");
		} elseif ($arr_product["product_kind_flg"]=="2" ){
			echo mb_convert_encoding('e-ライブ,',  "SJIS", "UTF-8");
		} else {
			echo mb_convert_encoding('e-ラーニング,',  "SJIS", "UTF-8");
		}
	} elseif( $arr_product["product_type_add"]=="2" ){
		if ($arr_product["training_kind_flg"]=="2" ){
			echo mb_convert_encoding('夏季研修,',  "SJIS", "UTF-8");
		} elseif ($arr_product["training_kind_flg"]=="3" ){
			echo mb_convert_encoding('新規登録弁護士研修,',  "SJIS", "UTF-8");
		} else {
			echo mb_convert_encoding('ライブ実務研修,',  "SJIS", "UTF-8");
		}
	} elseif( $arr_product["product_type_add"]=="3" ){
		echo mb_convert_encoding('代替倫理研修,',  "SJIS", "UTF-8");
	} elseif( $arr_product["product_type_add"]=="4" ){
		echo mb_convert_encoding('パスポート,',  "SJIS", "UTF-8");
	}
	echo mb_convert_encoding("\r\n", "SJIS", "UTF-8");

	echo mb_convert_encoding('弁護士会,',  "SJIS", "UTF-8");
	echo mb_convert_encoding('"' . $arr_product["bar_association_branch_name"] . '",',  "SJIS", "UTF-8");
	echo mb_convert_encoding("\r\n", "SJIS", "UTF-8");

	echo mb_convert_encoding('実施日,',  "SJIS", "UTF-8");
	echo mb_convert_encoding('"' . $arr_product["dates"] . '",',  "SJIS", "UTF-8");
	echo mb_convert_encoding("\r\n", "SJIS", "UTF-8");
	...
	echo mb_convert_encoding('総受講者,',  "SJIS", "UTF-8");
	if ($arr_product["product_type_add"]=="1"){
		echo mb_convert_encoding('"' . $arr_product["product_type_add1_count"] . '",',  "SJIS", "UTF-8");
		echo mb_convert_encoding('受講完了者,',  "SJIS", "UTF-8");
		echo mb_convert_encoding('"' . $arr_product["product_type_add1_end_count"] . '",',  "SJIS", "UTF-8");
	} elseif ($arr_product["product_type_add"]=="2"){
		echo mb_convert_encoding('"' . $arr_product["product_type_add2_count"] . '",',  "SJIS", "UTF-8");
		echo mb_convert_encoding('受講完了者,',  "SJIS", "UTF-8");
		echo mb_convert_encoding('"' . $arr_product["product_type_add2_end_count"] . '",',  "SJIS", "UTF-8");
	} elseif ($arr_product["product_type_add"]=="3"){
		echo mb_convert_encoding('"' . $arr_product["product_type_add3_count"] . '",',  "SJIS", "UTF-8");
		echo mb_convert_encoding('受講完了者,',  "SJIS", "UTF-8");
		echo mb_convert_encoding('"' . $arr_product["product_type_add3_end_count"] . '",',  "SJIS", "UTF-8");
	} elseif ($arr_product["product_type_add"]=="4"){
	}
	echo mb_convert_encoding("\r\n", "SJIS", "UTF-8");

	echo mb_convert_encoding('公開期間,',  "SJIS", "UTF-8");
	echo mb_convert_encoding('"' . $arr_product["start_date"] . '",',  "SJIS", "UTF-8");
	echo mb_convert_encoding('"' . $arr_product["end_date"] . '",',  "SJIS", "UTF-8");
	echo mb_convert_encoding("\r\n", "SJIS", "UTF-8");

	echo mb_convert_encoding("\r\n", "SJIS", "UTF-8");

	if ($arr_product["product_type_add"]=="4"){
		echo mb_convert_encoding('登録番号,氏名,メールアドレス,弁護士会,利用開始日,有効期限,',  "SJIS", "UTF-8");
	} else {
		echo mb_convert_encoding('登録番号,氏名,メールアドレス,弁護士会,FP,開始日,最終受講日,受講状況,',  "SJIS", "UTF-8");
	}
	echo mb_convert_encoding("\r\n", "SJIS", "UTF-8");
	for($i=0;$i<count($arr_order);$i++){
		//+++++++++++++++++++++++++++++++++++++++++++++
		echo mb_convert_encoding('"' . $arr_order[$i]["lawyer_number"] . '",',                "SJIS", "UTF-8");
		//+++++++++++++++++++++++++++++++++++++++++++++
		echo mb_convert_encoding('"' . $arr_order[$i]["student_name"] . '",',                "SJIS", "UTF-8");
		//+++++++++++++++++++++++++++++++++++++++++++++
		echo mb_convert_encoding('"' . $arr_order[$i]["student_email"] . '",',                "SJIS", "UTF-8");
		//+++++++++++++++++++++++++++++++++++++++++++++
		echo mb_convert_encoding('"' . $arr_order[$i]["association_name"] . '",',                "SJIS", "UTF-8");
		//+++++++++++++++++++++++++++++++++++++++++++++
		
		if ($arr_product["product_type_add"]=="4"){
			//+++++++++++++++++++++++++++++++++++++++++++++
			echo mb_convert_encoding('"' . $arr_order[$i]["payment_date"] . '",',                "SJIS", "UTF-8");
			//+++++++++++++++++++++++++++++++++++++++++++++
			echo mb_convert_encoding('"' . $arr_order[$i]["exp_date_passport"] . '",',                "SJIS", "UTF-8");
			//+++++++++++++++++++++++++++++++++++++++++++++
			...
			if ($arr_order[$i]["product_type_add"]=="1"){
				echo mb_convert_encoding('"' . $arr_order[$i]["start_view_date"] . '",',                "SJIS", "UTF-8");
			} elseif ($arr_order[$i]["product_type_add"]=="2"){
				echo mb_convert_encoding('"' . $arr_order[$i]["dates"] . '",',                "SJIS", "UTF-8");
			} elseif ($arr_order[$i]["product_type_add"]=="3"){
				if ($arr_order[$i]["start_date1"]!="" && $arr_order[$i]["start_date1"]!="0000-00-00 00:00:00"){
					echo mb_convert_encoding('"' . $arr_order[$i]["start_date1"] . '",',                "SJIS", "UTF-8");
				} else {
					echo mb_convert_encoding('"-",',                "SJIS", "UTF-8");
				}
			...
			//+++++++++++++++++++++++++++++++++++++++++++++
			if ($arr_order[$i]["product_type_add"]=="1"){
				echo mb_convert_encoding('"' . $arr_order[$i]["end_view_date"] . '",',                "SJIS", "UTF-8");
			} elseif ($arr_order[$i]["product_type_add"]=="2"){
				echo mb_convert_encoding('"",',                "SJIS", "UTF-8");
			} elseif ($arr_order[$i]["product_type_add"]=="3"){
				if ($arr_order[$i]["judge_date2"]!="" && $arr_order[$i]["judge_date2"]!="0000-00-00 00:00:00"){
					echo mb_convert_encoding('"' . $arr_order[$i]["judge_date2"] . '",',                "SJIS", "UTF-8");
				} elseif ($arr_order[$i]["judge_date1"]!="" && $arr_order[$i]["judge_date1"]!="0000-00-00 00:00:00"){
					echo mb_convert_encoding('"' . $arr_order[$i]["judge_date1"] . '",',                "SJIS", "UTF-8");
				} else {
					echo mb_convert_encoding('"-",',                "SJIS", "UTF-8");
				}
			} elseif ($arr_order[$i]["product_type_add"]=="4"){
			...
			//+++++++++++++++++++++++++++++++++++++++++++++
			if ($arr_order[$i]["product_type_add"]=="1"){
				echo mb_convert_encoding('"' . $arr_order[$i]["percent"] . '%",',                "SJIS", "UTF-8");
				...
				if ($arr_order[$i]["status"]=="0" || $arr_order[$i]["status"]==""){
					echo mb_convert_encoding('"未受講",',                "SJIS", "UTF-8");
				} elseif ($arr_order[$i]["status"]=="1"){
					echo mb_convert_encoding('"受講中",',                "SJIS", "UTF-8");
				} elseif ($arr_order[$i]["status"]=="2"){
					echo mb_convert_encoding('"一次○",',                "SJIS", "UTF-8");
				} elseif ($arr_order[$i]["status"]=="3"){
					echo mb_convert_encoding('"一次×",',                "SJIS", "UTF-8");
				} elseif ($arr_order[$i]["status"]=="4"){
					echo mb_convert_encoding('"一次×",',                "SJIS", "UTF-8");
				} elseif ($arr_order[$i]["status"]=="5"){
					echo mb_convert_encoding('"追試○",',                "SJIS", "UTF-8");
				} elseif ($arr_order[$i]["status"]=="6"){
					echo mb_convert_encoding('"追試×",',                "SJIS", "UTF-8");
				} elseif ($arr_order[$i]["status"]=="7"){
					echo mb_convert_encoding('"レポート",',                "SJIS", "UTF-8");
				} elseif ($arr_order[$i]["status"]=="8"){
					echo mb_convert_encoding('"会場",',                "SJIS", "UTF-8");
				} else {
					echo mb_convert_encoding('"",',                "SJIS", "UTF-8");
				}
			} elseif ($arr_order[$i]["product_type_add"]=="4"){
