...
if( isset($_GET["search"]) && $_GET["search"]=="new" ){
	...
	if( isset($_SESSION["product_live_branch.search_product_name"]) && !empty($_SESSION["product_live_branch.search_product_name"]) ){
		$search_product_name = $_SESSION["product_live_branch.search_product_name"];
	}
	if( isset($_SESSION["product_live_branch.search_product_code"]) && !empty($_SESSION["product_live_branch.search_product_code"]) ){
		$search_product_code = $_SESSION["product_live_branch.search_product_code"];
	}
	if( isset($_SESSION["product_live_branch.search_start_date"]) && !empty($_SESSION["product_live_branch.search_start_date"]) ){
		$search_start_date = $_SESSION["product_live_branch.search_start_date"];
	}
	if( isset($_SESSION["product_live_branch.search_end_date"]) && !empty($_SESSION["product_live_branch.search_end_date"]) ){
		$search_end_date = $_SESSION["product_live_branch.search_end_date"];
	}
	if( isset($_SESSION["product_live_branch.search_category"]) && !empty($_SESSION["product_live_branch.search_category"]) ){
		$search_category = $_SESSION["product_live_branch.search_category"];
	}
	if( isset($_SESSION["product_live_branch.search_open"]) && !empty($_SESSION["product_live_branch.search_open"]) ){
		$search_open = $_SESSION["product_live_branch.search_open"];
	}
	if( isset($_SESSION["product_live_branch.search_teacher"]) && !empty($_SESSION["product_live_branch.search_teacher"]) ){
		$search_teacher = $_SESSION["product_live_branch.search_teacher"];
	}
	if( isset($_SESSION["product_live_branch.search_free"]) && !empty($_SESSION["product_live_branch.search_free"]) ){
		$search_free = $_SESSION["product_live_branch.search_free"];
	}
	if( isset($_SESSION["product_live_branch.search_word"]) && !empty($_SESSION["product_live_branch.search_word"]) ){
		$search_word = $_SESSION["product_live_branch.search_word"];
	}
	if( isset($_SESSION["product_live_branch.search_dates_start"]) && !empty($_SESSION["product_live_branch.search_dates_start"]) ){
		$search_dates_start = $_SESSION["product_live_branch.search_dates_start"];
	}
	if( isset($_SESSION["product_live_branch.search_dates_end"]) && !empty($_SESSION["product_live_branch.search_dates_end"]) ){
		$search_dates_end = $_SESSION["product_live_branch.search_dates_end"];
	}
	if( isset($_SESSION["product_live_branch.search_bar_association"]) && !empty($_SESSION["product_live_branch.search_bar_association"]) ){
		$search_bar_association = $_SESSION["product_live_branch.search_bar_association"];
	}
	if( isset($_SESSION["product_live_branch.search_orderby"]) && !empty($_SESSION["product_live_branch.search_orderby"]) ){
		$search_orderby = $_SESSION["product_live_branch.search_orderby"];
	}
}
if( $_SERVER["REQUEST_METHOD"] == "POST" ){
	...
	$_SESSION["product_live_branch.search_product_name"] = $search_product_name;
	$_SESSION["product_live_branch.search_product_code"] = $search_product_code;
	$_SESSION["product_live_branch.search_start_date"] = $search_start_date;
	$_SESSION["product_live_branch.search_end_date"] = $search_end_date;
	$_SESSION["product_live_branch.search_category"] = $search_category;
	$_SESSION["product_live_branch.search_open"] = $search_open;
	$_SESSION["product_live_branch.search_teacher"] = $search_teacher;
	$_SESSION["product_live_branch.search_free"] = $search_free;
	$_SESSION["product_live_branch.search_word"] = $search_word;
	$_SESSION["product_live_branch.search_dates_start"] = $search_dates_start;
	$_SESSION["product_live_branch.search_dates_end"] = $search_dates_end;
	$_SESSION["product_live_branch.search_bar_association"] = $search_bar_association;

	$_SESSION["product_live_branch.page"] = 1;
	$_GET["page"] = 1;
}
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
$page = 1;
if( isset($_SESSION["product_live_branch.page"]) && !empty($_SESSION["product_live_branch.page"]) ){
	$page = $_SESSION["product_live_branch.page"];
}
if( isset($_GET["page"]) && !empty($_GET["page"]) && is_numeric($_GET["page"]) ){
	$page = $_GET["page"];
	$_SESSION["product_live_branch.page"] = $page;
}
...
WHERE
  ...
  AND RPBA.bar_association_id <> '$login_bar_association_id'
";
...
// ログインユーザーが受講対象
if (!$nichibenren_flg){
	$where.= " and ( tbl_product_live_training.target like '%|".mysqli_real_escape_string($objDbConnect->connect, $login_bar_association_id)."|%' ) ";
}
...
$arr_list = $objDbConnect->query_fetch_arr($sql.$where.$order.$offset);
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
$template->admin_layout('product_live_branch/index.tpl');
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
?>
