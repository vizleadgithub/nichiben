...
WHERE
 ...
 ";
if (!$nichibenren_flg){
	$sql.= " AND RPBA.bar_association_id =".mysqli_real_escape_string($objDbConnect->connect,$login_bar_association_id)."";
}
	...
	if ($arr_input){
		...
		for($i=1; $i<=MAX_RELATED_PRODUCTS; $i++){
			if ($arr_input["related_products$i"] != ''){
				$sql = "select product_name from tbl_product where product_id='".mysqli_real_escape_string($objDbConnect->connect,$arr_input["related_products$i"])."'";
		...
		// 実施弁護士会
		if ($nichibenren_flg){
			$bar_association_branch = get_rel_product_bar_association_branch($mid);
			//var_dump($bar_association_branch);
			foreach ($bar_association_branch as $val){
				/*
				// 旧データの確認
				if ($mid <= 19233) {
					$product_id_old = $mid - 10000;
					$sql = "
					SELECT
					  capacity
					FROM
				...
				*/
				...
				if ($val['capacity'] == 0) {
					...
					$arr_input['entry_number'.$bar_association_branch_id] = 0;
				}
				else {
					...
					$arr_input['entry_number'.$bar_association_branch_id] = get_entry_number($bar_association_branch_id, $mid);
				}
			}
		}
	} else {
