...
if(!isset($_POST['act'])){
	...
	$template->admin_layout('product_live/add.tpl');
...
*/
	...
	if (isset($_POST["bar_association_sponsor"])){
		foreach ($_POST["bar_association_sponsor"] as $val){
			if ($val != $login_bar_association_id){
	...
	// 受講対象(select)
	if (isset($_POST["bar_association_target"]) && is_array($_POST["bar_association_target"]) ){
	...
	// 受講対象(unselect)
	if (isset($_POST["bar_association_target_unselect"]) && is_array($_POST["bar_association_target_unselect"]) ){
	...
	switch($_POST['act']){
		...
		case 'confirm':
			...
			// 入力エラーなし
			if(empty($err_msg)){
				$template->admin_layout('product_live/add_confirm.tpl');
...
//exit();
					if(!$ret){
						...
						//} else {
							...
							$str_sponsor = '|'.$login_bar_association_id.'|'; // 主催
							foreach ($arr_input["bar_association_sponsor"] as $key => $val){
								$str_sponsor.= $key.'|';
							}
							$str_target = '|'.$login_bar_association_id.'|'; // 受講対象
							foreach ($bar_association_target as $key => $val){
								$str_target.= $key.'|';
							}
//print("1.UPDATE tbl_product_live_training[".$nichibenren_flg."]\n");
...
//print("2.UPDATE rel_product_bar_association[".$login_bar_association_id."]\n");
...
//print("3.DELETE rel_product_bar_association[".$arr_input["mid"]."]\n");
									...
									if(!$ret){
										...
										foreach ($bar_association_target as $key => $val){
//print("4.INSERT rel_product_bar_association[".$key."]\n");
...
//print("5.SELECT rel_product_bar_association_branch[".$bar_association_branch_id."]\n");
													 	$sql = "SELECT";
														$sql.= "  count(*) AS c";
														$sql.= " FROM";
														...
														$sql.= "  AND bar_association_branch_id = '".$bar_association_branch_id."'";
													 	...
													 	// 登録
													 	if ($ret["c"] == 0){
//print("6.INSERT rel_product_bar_association_branch[".$bar_association_branch_id."]\n");
...
//print("7.UPDATE rel_product_bar_association_branch[".$bar_association_branch_id."]\n");
//print("7-u.[capacity:".$arr_input["capacity".$bar_association_branch_id]."]\n");
//print("7-u.[hall:".$arr_input["hall".$bar_association_branch_id]."]\n");
//print("7-u.[receptionist_start_date:".$arr_input["receptionist_start_date".$bar_association_branch_id]."]\n");
//print("7-u.[receptionist_end_date:".$arr_input["receptionist_end_date".$bar_association_branch_id]."]\n");
//print("7-u.[contents:".$arr_input["contents".$bar_association_branch_id]."]\n");
//print("7-u.[dates:".$arr_input["dates".$bar_association_branch_id]."]\n");
//print("7-u.[product_id:".$arr_input["mid"]."]\n");
//print("7-u.[bar_association_branch_id:".$bar_association_branch_id."]\n");
...
//exit();
				...
				*/
					...
					if(!$ret){
						...
						$sql.= " VALUES";
						$sql.= "  (";
						$sql.= "   '".mysqli_real_escape_string($objDbConnect->connect,$product_id)."',";
						$sql.= "   '2'";
						$sql.= "  )";
						$ret = $objDbConnect->execute($sql);
						if(!$ret){
							...
							$str_sponsor = '|'.$login_bar_association_id.'|'; // 主催
							foreach ($arr_input["bar_association_sponsor"] as $key => $val){
								$str_sponsor.= mysqli_real_escape_string($objDbConnect->connect,$key).'|';
							}
							$str_target = '|'.$login_bar_association_id.'|'; // 受講対象
							foreach ($bar_association_target as $key => $val){
								$str_target.= mysqli_real_escape_string($objDbConnect->connect,$key).'|';
							}
							...
							if(!$ret){
								...
								$sql.= "  (";
								$sql.= "   '".mysqli_real_escape_string($objDbConnect->connect,$product_id)."',";
								$sql.= "   '".mysqli_real_escape_string($objDbConnect->connect,$login_bar_association_id)."',";
								$sql.= "   '1',";
								...
								if(!$ret){
									...
									foreach ($bar_association_target as $key => $val){
										...
										$sql.= "  (";
										$sql.= "   '".mysqli_real_escape_string($objDbConnect->connect,$product_id)."',";
										$sql.= "   '".mysqli_real_escape_string($objDbConnect->connect,$key)."',";
										$sql.= "   '3',";
										$sql.= "   '".date('Y-m-d H:i:s')."'";
										$sql.= "  )";
										$ret = $objDbConnect->execute($sql);
											...
											foreach ($mtb_bar_association_branch as $val){
												...
												if (($arr_input["capacity".$bar_association_branch_id] !== ""
												 ...
												 && $arr_input["web_flg".$bar_association_branch_id] !== ""
													...
													$sql.= "  (";
													$sql.= "   '".mysqli_real_escape_string($objDbConnect->connect,$product_id)."',";
													$sql.= "   '".mysqli_real_escape_string($objDbConnect->connect,$bar_association_branch_id)."',";
													$sql.= "   '".mysqli_real_escape_string($objDbConnect->connect,$arr_input["capacity".$bar_association_branch_id])."',";
													$sql.= "   '".mysqli_real_escape_string($objDbConnect->connect,$arr_input["hall".$bar_association_branch_id])."',";
													$sql.= "   '".mysqli_real_escape_string($objDbConnect->connect,$arr_input["receptionist_start_date".$bar_association_branch_id])."',";
													$sql.= "   '".mysqli_real_escape_string($objDbConnect->connect,$arr_input["receptionist_end_date".$bar_association_branch_id])."',";
...
//exit();
			...
			$template->admin_layout('product_live/add_complete.tpl');
			break;
	...
	// 確認時のエラー
		...
		*/
			...
			if(cmCheckInput($arr_input['price'], 'CK_KARA')){
				$err_msg['price'] = '料金(テキスト代含む総額)は必須です。';
			} else {
				if(cmCheckInput($arr_input['price'], 'CK_NUM')){
					$err_msg['price'] = '料金(テキスト代含む総額)は半角数字で入力してください。';
				}
			}
