...
$tmp = get_ethic_group();
$ethic_group = array('' => '選択してください');
foreach ($tmp as $key => $val){
	$ethic_group[$key] = $val;
}
$template->assign('ethic_group', $ethic_group);
...
if(!isset($_POST['act'])){
	...
	$template->admin_layout('product_ethics/add.tpl');
...
*/
	...
	switch($_POST['act']){
		...
		case 'confirm':
			...
			if(empty($err_msg)){
				//var_dump($arr_input);
				$template->admin_layout('product_ethics/add_confirm.tpl');
			// 入力エラーあり
			} else {
		...
		case 'complete':
				...
				*/
					...
					if(!$ret){
						...
						if(!$ret){
							...
							// tbl_product_ethic_trainingにデータ登録
						/*
							$str_product_flg = '';
						...
						*/
							$sql = "UPDATE";
							$sql.= "  tbl_product_ethic_training";
							$sql.= " SET";
							$sql.= "  ethic_group_id = '".mysqli_real_escape_string($objDbConnect->connect,$arr_input["ethic_group_id"])."',";
							$sql.= "  bar_association_year = '".mysqli_real_escape_string($objDbConnect->connect,$arr_input["bar_association_year"])."',";
							$sql.= "  publish_flg = '".mysqli_real_escape_string($objDbConnect->connect,$arr_input["publish_flg"])."'";
							$sql.= " WHERE";
							$sql.= "  product_id = '".mysqli_real_escape_string($objDbConnect->connect,$arr_input["mid"])."'";
							$ret = $objDbConnect->execute($sql);
							if(!$ret){
				...
				*/
					...
					if(!$ret){
						...
						*/
							...
							$sql = "INSERT INTO tbl_product_ethic_training";
							$sql.= "  (";
							$sql.= "   product_id,";
							$sql.= "   ethic_group_id,";
							$sql.= "   bar_association_year,";
							$sql.= "   publish_flg";
							$sql.= "  )";
							$sql.= " VALUES";
							$sql.= "  (";
							$sql.= "   '".$product_id."',";
							$sql.= "   '".mysqli_real_escape_string($objDbConnect->connect,$arr_input["ethic_group_id"])."',";
							$sql.= "   '".mysqli_real_escape_string($objDbConnect->connect,$arr_input["bar_association_year"])."',";
							$sql.= "   '".mysqli_real_escape_string($objDbConnect->connect,$arr_input["publish_flg"])."'";
							$sql.= "  )";
							$ret = $objDbConnect->execute($sql);
							if(!$ret){
								$err_flag = 1;
			...
			$template->admin_layout('product_ethics/add_complete.tpl');
			break;
...
function err_check($template, $arr_input){
	...
	// 確認時のエラー
		...
		*/
			...
			if(cmCheckInput($arr_input['publish_flg'], 'CK_NUM')){
				$err_msg['publish_flg'] = '公開は必須です。';
			}
			...
			if(cmCheckInput($arr_input['bar_association_year'], 'CK_NUM')){
				$err_msg['bar_association_year'] = '年数は必須です。';
			}
			if(cmCheckInput($arr_input['ethic_group_id'], 'CK_NUM')){
				$err_msg['ethic_group_id'] = '倫理研修問題は必須です。';
			}
