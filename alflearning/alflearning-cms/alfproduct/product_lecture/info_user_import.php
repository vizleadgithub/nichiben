			...
			while (($data = fgetcsv($fp, 0, ",")) !== FALSE) {
				...
				if ($result){
					...
					if ($payment_date === NULL) {
						$payment_date_sql = "NULL";
					} else {
						$payment_date_sql = "'" . mysqli_real_escape_string($objDbConnect->connect, $payment_date) . "'";
					}
					$tran_started = false;
					...
					if ($err_flg){
						...
						if ($sql_error !== '') {
							@error_log(
								date('Y-m-d H:i:s').': product_lecture info_user_import regist failed: '.$sql_error."\n",
								3,
								'/alflearning-data/alfproduct/logs/product_lecture_info_user_import.log'
							);
						}
...
$template->admin_layout('product_lecture/info_user_import.tpl');
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
?>
