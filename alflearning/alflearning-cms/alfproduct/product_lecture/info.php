...
if ($res){
	...
	if ($pid <= 19233) {
		...
		if ($res) {
			foreach ($res as $k1 => $v1) {
				$association_branch_id[] = $v1["bar_association_branch_id"];
			}
		}
		...
		$sql.= "  ( SELECT * FROM rel_product_bar_association_branch WHERE product_id = '".mysqli_real_escape_string($objDbConnect->connect,$pid)."' AND bar_association_branch_id IN ('".implode(",", $association_branch_id)."' )) AS T3";
	...
	if ($res){
		foreach ($res as $key => $val){
			...
			$arr_list[$bar_association_branch_id]['entry_number'] = get_entry_number2($bar_association_branch_id, $pid);
//print_r("<!--[\n");
//var_dump($bar_association_branch_id);
//print_r("\n]-->\n");
//print_r("<!--[\n");
//var_dump($pid);
//print_r("\n]-->\n");
...
$template->admin_layout('product_lecture/info.tpl');
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
?>
