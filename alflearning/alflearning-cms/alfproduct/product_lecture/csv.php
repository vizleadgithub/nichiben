...
if ($type == 'info') {
	...
	$ret = _get_product_lecture_info_user($objDbConnect, $pid, $aid, $arr_session, $product_info);
	...
	//echo mb_convert_encoding("受付日,ステイタス,受講,WEB,登録番号,氏名,FP,価格\r\n", "SJIS", "UTF-8");
	echo mb_convert_encoding("受付日,ステイタス,受講,登録番号,氏名,所属弁護士会,申込FP,$fp_title,価格\r\n", "SJIS", "UTF-8");
	foreach ($ret as $val){
		// 各項目の整形
		$disp_participation = '';
		...
		// 一行ずつ書き込み
		$write_data = '';
		//$write_data = '"'.$val['create_date'].'",'.'"'.$disp_payment.'",'.'"'.$disp_participation.'",'.'"'.$val['disp_web'].'",'.'"'.$val['lawyer_number'].'",'.'"'.$val['student_name'].'",'.'"'.$val['disp_fp'].'",'.'"'.$val['pay_total'].'"';
		$write_data = '"'.$val['create_date'].'",'.'"'.$disp_payment.'",'.'"'.$disp_participation.'",'.'"'.$val['lawyer_number'].'",'.'"'.$val['student_name'].'",'.'"'.$mtb_bar_association[$val['bar_association_id']].'",'.'"'.$val['disp_fp'].'",'.'"'.$disp_fp2.'",'.'"'.$val['pay_total'].'"';
		$write_data.= "\r\n";
		echo mb_convert_encoding($write_data, "SJIS", "UTF-8");
	}
	...
	echo mb_convert_encoding("チェック,登録番号,氏名,申込FP,$fp_title,ステイタス\r\n", "SJIS", "UTF-8");
	foreach ($ret as $val){
		// 各項目の整形
		if ($val['payment_status_reserv'] != ''){
			$disp_payment = $val['disp_payment_reserv'];
		...
		// 一行ずつ書き込み
		$write_data = '';
		$write_data = '"",'.'"'.$val['lawyer_number'].'","'.$val['student_name'].'","'.$val['disp_fp'].'","'.$disp_fp2.'","'.$disp_payment.'"';
		$write_data.= "\r\n";
		echo mb_convert_encoding($write_data, "SJIS", "UTF-8");
	}
}
 ...
 */
function _get_product_lecture_info_user($objDbConnect, $pid, $aid, $arr_session, $product_info){
	$arr_list = array();
	$sql = "";
	$sql.= "SELECT ";
	...
	if ($return){
		$u_kenshu_dates = strtotime($product_info['kenshu_dates']);
