...
class Bat_update_student_from_csv extends CI_Controller {
	//----------------------------------------------
	// 定数（各ID増加値）
	//----------------------------------------------
...
//	public $_convert_log_file         = '/alflearning-data/__import_data__/LOG/MS_USER_to_student.txt';
  ...
  //public $_target_path              = '/alflearning-data/data_import/takasumi_test/';
	...
	//----------------------------------------------
	public function update_student_from_csv(){
		exec('ps auxw | grep update_student_from_csv | grep -v " grep " | grep -v "/bin/sh" ', $outputs);
		if(count($outputs) > 1){
		...
		for ($i=0 ; $i < count($_target_file_name_array) ; $i++) {
			...
			if($i>0){
				if(is_file($_target_path.'/'.$_target_file_name)){
					print "[".date('Y-m-d H:i:s')."]"."ファイル：".$_target_path.'/'.$_target_file_name." 削除開始"."\n";
					unlink($_target_path.'/'.$_target_file_name);
					print "[".date('Y-m-d H:i:s')."]"."ファイル：".$_target_path.'/'.$_target_file_name." 削除終了"."\n";
				}
				continue;
			}
			  ...
			  //$_convert['bar_association_id'] = $this->_get_bar_association_id($_convert['bar_association_id']);
				...
				try{
					...
					$res_student_update = $this->db->query($this->db->update_string($this->_table_student, array(
						...
						),"lawyer_number='".$_convert['lawyer_number']."' AND lawyer_division='".$_convert['lawyer_division']."'"
					));
					...
					//	// 受講者講座テーブルの新規登録（更新失敗時）
					//	$res_student_lecture = $this->db->query($this->db->insert_string($this->_table_student_lecture, array(
