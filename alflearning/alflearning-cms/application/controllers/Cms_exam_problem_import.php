...
class Cms_exam_problem_import extends CI_Controller {
	//----------------------------------------------
	//プライベート変数宣言
	//----------------------------------------------
...
//		}
	...
	function edit(){
		...
		$data['exam_problem']['exam_problem_groups']   = array();  // 設問グループ
		...
		//ビュー設定引数設定
		$disp_param = array(
						'view_name'   => 'cms_exam_problem_import/edit',
						'submenu_idx' => 2,
						'view_data'   => $data,
					);
		//ビュー設定
...
//					if(in_array($lecture['cource_id'], $check_exam_problem_lectures)) {
//						$check_flag = $check_flag + 1;
//						continue;
//					}
//				}
//				if($check_flag != $check_exam_problem_lectures_count){
...
//			}
			...
			if($this->form_validation->run() == FALSE || (isset($_FILES['local_file']) && $_FILES['local_file']['error'] != 0) || ($lecture_error_msg != "") || ($local_file_error_msg != "")){
				...
				$this->load->library('upload', array(
					...
					'overwrite'     => TRUE,
					'remove_spaces' => TRUE,
					'file_name'     => 'school_'.$this->libauth->get_school_id().'_exam_problem',
				));
				...
				$data['exam_problem']['exam_problem_groups']   = $this->input->post('exam_problem_groups')  ?$this->input->post('exam_problem_groups')  :array();
				$data['exam_problem']['csv_file_name']         = '/tmp/'.$upload_data['file_name'];
				...
				// 設問グループ名の取得
				$data['exam_problem']['exam_problem_groups_name'] = array();
				$this->load->model('model_exam_problem_group');
				if($data['exam_problem']['exam_problem_groups']){
					foreach($data['exam_problem']['exam_problem_groups'] as $idx => $exam_problem_group_id){
						$name_param = array(
										'exam_problem_group_id' => $exam_problem_group_id,
									);
						$exam_problem_group = $this->model_exam_problem_group->get_exam_problem_group($name_param);
						if($exam_problem_group){
							$data['exam_problem']['exam_problem_groups_name'][$idx] = $exam_problem_group['exam_problem_group_name'];
						}
					}
					//連想キーと要素との関係を維持しつつ配列をソート
					asort($data['exam_problem']['exam_problem_groups_name']);
				}
				
				//ビュー設定引数設定
				$disp_param = array(
								'view_name'   => 'cms_exam_problem_import/confirm',
								'submenu_idx' => 3,
								'view_data'   => $data,
							);
				//編集フォーム再表示
	...
	function commit(){
		...
		if(isset($edit_form_data['school_id']) && $edit_form_data['school_id'] == $select_school_id){
			...
			$exam_problem_lectures = $edit_form_data['exam_problem_lectures'];
			$exam_problem_groups   = $edit_form_data['exam_problem_groups'];
			...
			while(!feof($fp)){
				...
				if( count($arrayRecord) < 2 ){
					...
					$temp_import_result['message'] = $this->lang->line_or_def('error_exam_problem_import_blank_line','空白行');
				}elseif( count($arrayRecord) < 10 ){
					$temp_import_result['no']      = $counter;
					$temp_import_result['value']   = 'NG';
					$temp_import_result['message'] = $this->lang->line_or_def('error_exam_problem_import_items_lack','項目数が足りません');
				}
				...
				if( count($temp_import_result)==0 ){
					if( !empty($arrayRecord[$arrRecordIndex]) ){
						$exam_problem['exam_problem_name'] = $arrayRecord[$arrRecordIndex];
					}else{
						$temp_import_result['no']      = $counter;
						$temp_import_result['value']   = 'NG';
						$temp_import_result['message'] = $this->lang->line_or_def('error_exam_problem_import_no_exam_problem_name','設問名は必須入力です');
					}
				}
				...
				if( count($temp_import_result)==0 ){
					...
					if( ($arrayRecord[$arrRecordIndex]==1) ){
						$exam_problem['problem_kind'] = $arrayRecord[$arrRecordIndex];
					}else{
						$temp_import_result['no']      = $counter;
						$temp_import_result['value']   = 'NG';
						//$temp_import_result['message'] = $this->lang->line_or_def('error_exam_problem_import_ng_problem_kind','設問種類は1、2、3、いづれかが必要です');
						$temp_import_result['message'] = $this->lang->line_or_def('error_exam_problem_import_ng_problem_kind','設問種類は1、が必要です');
					}
				}
				...
				if( count($temp_import_result)==0 ){
					if( empty($arrayRecord[$arrRecordIndex]) ){
						$temp_import_result['no']      = $counter;
						$temp_import_result['value']   = 'NG';
						$temp_import_result['message'] = $this->lang->line_or_def('error_exam_problem_import_no_problem_contents','設問内容は必須入力です');
					}else{
						$exam_problem['problem_contents_text']         = '';
						$exam_problem['problem_contents_video']        = 0;
						$exam_problem['problem_contents_book_library'] = 0;
						
						if( ($exam_problem['problem_kind']==2) ){
							// ビデオID
							$check_param = array(
											'video_id'              => (int)$arrayRecord[$arrRecordIndex],
											'exam_problem_lectures' => $exam_problem_lectures,
											'school_id'             => $school_id,
							);
							$check_result = $this->model_exam_problem->check_exam_problem_video($check_param);
							
							if( empty($check_result) ){
								$temp_import_result['no']      = $counter;
								$temp_import_result['value']   = 'NG';
								$temp_import_result['message'] = $this->lang->line_or_def('error_exam_problem_import_no_problem_contents_video_id','設問内容のビデオIDが見つかりません');
							}else{
								$exam_problem['problem_contents_video'] = $arrayRecord[$arrRecordIndex];
							}
						}elseif( ($exam_problem['problem_kind']==3) ){
							// 図書室ID
							$check_param = array(
							...
							$check_result = $this->model_exam_problem->check_exam_problem_book_library($check_param);
							
							if( empty($check_result) ){
								$temp_import_result['no']      = $counter;
								$temp_import_result['value']   = 'NG';
								$temp_import_result['message'] = $this->lang->line_or_def('error_exam_problem_import_no_problem_contents_book_library_id','設問内容の図書室IDが見つかりません');
							}else{
								$exam_problem['problem_contents_book_library'] = $arrayRecord[$arrRecordIndex];
							}
						}else{
							// テキスト
							$exam_problem['problem_contents_text'] = $arrayRecord[$arrRecordIndex];
						}
					}
				}
				...
				$arrRecordIndex = 3;
				if( count($temp_import_result)==0 ){
					$exam_problem['problem_note'] = $arrayRecord[$arrRecordIndex];
				}
				...
				if( count($temp_import_result)==0 ){
					if( ($arrayRecord[$arrRecordIndex]==1) || ($arrayRecord[$arrRecordIndex]==2) || ($arrayRecord[$arrRecordIndex]==3) ){
						$exam_problem['answer_kind'] = $arrayRecord[$arrRecordIndex];
					}else{
						$temp_import_result['no']      = $counter;
						$temp_import_result['value']   = 'NG';
						$temp_import_result['message'] = $this->lang->line_or_def('error_exam_problem_import_ng_answer_kind','解答種類は1、2、3、いづれかが必要です');
					}
				}
				...
				if( count($temp_import_result)==0 ){
					if( (is_numeric($arrayRecord[$arrRecordIndex])) && (intval($arrayRecord[$arrRecordIndex])>-1) ) {
						$exam_problem['answer_point'] = intval($arrayRecord[$arrRecordIndex]);
					}else{
						$temp_import_result['no']      = $counter;
						$temp_import_result['value']   = 'NG';
						$temp_import_result['message'] = $this->lang->line_or_def('error_exam_problem_import_ng_answer_point','解答配点はゼロ以上の数字入力です');
					}
				}
				...
				if( count($temp_import_result)==0 ){
					...
					if( ($arrayRecord[$arrRecordIndex]==1) || ($arrayRecord[$arrRecordIndex]==9) ){
						$exam_problem['answer_explain_kind'] = $arrayRecord[$arrRecordIndex];
					}else{
						$temp_import_result['no']      = $counter;
						$temp_import_result['value']   = 'NG';
						//$temp_import_result['message'] = $this->lang->line_or_def('error_exam_problem_import_ng_answer_explain_kind','解答解説種類は1、2、3、9、いづれかが必要です');
						$temp_import_result['message'] = $this->lang->line_or_def('error_exam_problem_import_ng_answer_explain_kind','解答解説種類は1、9、いづれかが必要です');
					}
				}
				...
				$arrRecordIndex = 7;
				if( count($temp_import_result)==0 ){
					$exam_problem['answer_explain_contents_text']         = '';
					$exam_problem['answer_explain_contents_video']        = 0;
					$exam_problem['answer_explain_contents_book_library'] = 0;
					
					if( empty($arrayRecord[$arrRecordIndex]) ){
						if( ($exam_problem['answer_explain_kind']==9) ){
							...
							$temp_import_result['no']      = $counter;
							$temp_import_result['value']   = 'NG';
							$temp_import_result['message'] = $this->lang->line_or_def('error_exam_problem_import_no_answer_explain_contents','解答解説種類に指定がある場合、解答解説内容は必須入力です');
						}
					}else{
						if( ($exam_problem['answer_explain_kind']==2) ){
							// ビデオID
							$check_param = array(
											'video_id'              => (int)$arrayRecord[$arrRecordIndex],
											'exam_problem_lectures' => $exam_problem_lectures,
							...
							if( empty($check_result) ){
								$temp_import_result['no']      = $counter;
								$temp_import_result['value']   = 'NG';
								$temp_import_result['message'] = $this->lang->line_or_def('error_exam_problem_import_no_answer_explain_contents_video_id','解答解説内容のビデオIDが見つかりません');
							}else{
								$exam_problem['answer_explain_contents_video'] = $arrayRecord[$arrRecordIndex];
							}
						}elseif( ($exam_problem['answer_explain_kind']==3) ){
							// 図書室ID
							$check_param = array(
											'book_library_id'       => (int)$arrayRecord[$arrRecordIndex],
											'exam_problem_lectures' => $exam_problem_lectures,
							...
							if( empty($check_result) ){
								$temp_import_result['no']      = $counter;
								$temp_import_result['value']   = 'NG';
								$temp_import_result['message'] = $this->lang->line_or_def('error_exam_problem_import_no_answer_explain_contents_book_library_id','解答解説内容の図書室IDが見つかりません');
							}else{
								$exam_problem['answer_explain_contents_book_library'] = $arrayRecord[$arrRecordIndex];
							}
						}elseif( ($exam_problem['answer_explain_kind']==9) ){
							// なし（強制文字列ゼロ）
							$exam_problem['answer_explain_contents_text'] = '';
						}else{
							// テキスト
							$exam_problem['answer_explain_contents_text'] = $arrayRecord[$arrRecordIndex];
						}
					}
				}
				...
				if( count($temp_import_result)==0 ){
					if( ($exam_problem['answer_explain_kind']==9) ){
						...
						// 解答解説種類 ≠ 9 の場合、文字列格納
						$exam_problem['answer_explain_note'] = $arrayRecord[$arrRecordIndex];
					}
				}
				...
				if( count($temp_import_result)==0 ){
					...
					$exam_problem['answer_contents_no']      = array();  // 選択肢連番（1～）
					$exam_problem['answer_contents_word']    = array();
					$exam_problem['answer_contents_correct'] = array();
					$exam_problem['answer_contents_text']    = '';
					
					if( ($exam_problem['answer_kind']==1) || ($exam_problem['answer_kind']==2) ){
						// 解答種類 ＝ 単一形式、複数形式
						...
						for($index=$arrRecordIndex + 1; $index<count($arrayRecord); $index = $index + 2){
							...
							if( (array_key_exists($index, $arrayRecord)) && (array_key_exists($index+1, $arrayRecord)) ){
								...
								//	$temp_import_result['no']      = $counter;
								//	$temp_import_result['value']   = 'NG';
								//	$temp_import_result['message'] = $this->lang->line_or_def('error_exam_problem_import_no_answer_contents_choice','解答内容の選択肢内容がありません');
									...
									$exam_problem['answer_contents_no'][$select_count - 1]      = $select_count;
									$exam_problem['answer_contents_word'][$select_count - 1]    = $arrayRecord[$index+1];
									$exam_problem['answer_contents_correct'][$select_count - 1] = $arrayRecord[$index];
						...
						if($contents_error_flag == 0){
							if($select_count == 0){
								...
								$temp_import_result['no']      = $counter;
								$temp_import_result['value']   = 'NG';
								$temp_import_result['message'] = $this->lang->line_or_def('error_exam_problem_import_no_answer_contents','解答内容の選択肢がありません');
							}elseif($correct_count == 0){
								// 単一形式・複数形式なのに、正解がない
								$temp_import_result['no']      = $counter;
								$temp_import_result['value']   = 'NG';
								$temp_import_result['message'] = $this->lang->line_or_def('error_exam_problem_import_no_answer_contents_correct_answer','解答内容の正解がありません');
							}elseif( ($exam_problem['answer_kind']==1) && ($correct_count!=1) ) {
								// 単一形式なのに、正解が 1つでない
								$temp_import_result['no']      = $counter;
								$temp_import_result['value']   = 'NG';
								$temp_import_result['message'] = $this->lang->line_or_def('error_exam_problem_import_over_answer_contents_correct_answer','単一形式の解答内容の正解は1つです');
							}
						}else{
							// 正誤があって、内容なし
							$temp_import_result['no']      = $counter;
						...
						if( (array_key_exists($arrRecordIndex, $arrayRecord)) && (!empty($arrayRecord[$arrRecordIndex])) ){
							$exam_problem['answer_contents_text'] = $arrayRecord[$arrRecordIndex];
						}
					}
				}
				...
				if( count($temp_import_result)==0 ){
				    $exam_problem['update_flg'] = 0;                                  // 新規登録
				//  $exam_problem['exam_problem_name']                                // 設問名
				//  $exam_problem['problem_kind']                                     // 設問種類
				//  $exam_problem['problem_contents_text']                            // 設問内容 - テキスト
				//  $exam_problem['problem_contents_video']                           // 設問内容 - ビデオ
				//  $exam_problem['problem_contents_book_library']                    // 設問内容 - 図書室
				//	$exam_problem['problem_note']                                     // 設問備考 
				//	$exam_problem['answer_kind']                                      // 解答種類 
				//	$exam_problem['answer_contents_no']                               // 解答内容 - 選択肢連番（1～）
				//	$exam_problem['answer_contents_word']                             // 解答内容 - 選択肢内容
				//	$exam_problem['answer_contents_correct']                          // 解答内容 - 選択肢正誤
				//	$exam_problem['answer_point']                                     // 解答配点
				//  $exam_problem['answer_explain_kind']                              // 解答解説種類
				//  $exam_problem['answer_explain_contents_text']                     // 解答解説内容 - テキスト
				//  $exam_problem['answer_explain_contents_video']                    // 解答解説内容 - ビデオ
				//  $exam_problem['answer_explain_contents_book_library']             // 解答解説内容 - 図書室
				//	$exam_problem['answer_explain_note']                              // 解答解説備考 
					$exam_problem['school_id']             = $school_id;              // 学校ID
					$exam_problem['teacher_id']            = $teacher_id;             // 講師ID
					$exam_problem['exam_problem_lectures'] = $exam_problem_lectures;  // 所属講座
					$exam_problem['exam_problem_groups']   = $exam_problem_groups;    // 設問グループ（任意）
					...
					// 設問グループ関係テーブル新規登録
					$this->model_exam_problem_group->import_relations_exam_problem_group($data_param);

					// 成功情報の記載
					$exam_problem_id = $exam_problem_result['lastInsertId'];
					
					$message = $this->lang->line_or_def('msg_exam_problem_import_success' , '設問ID:{exam_problem_id} で登録されました');
					$message = str_replace("{exam_problem_id}" , $exam_problem_id , $message);
					...
					if($counter==1){
						$temp_import_result['no']      = $counter;
						$temp_import_result['value']   = '--';
						$temp_import_result['message'] = $this->lang->line_or_def('error_exam_problem_import_header_line','ヘッダー行');
					}
				}
	...
	function _display_view($param) {
		...
		$this->load->model('model_exam_problem_group');
		$param['view_data']['exam_problem_groups'] = $this->model_exam_problem_group->get_exam_problem_checkbox_list($drop_param);
	...
	function export(){
		...
		$fileName = 'exam_problem_'.date('YmdHis').'.csv';
		...
		foreach($export_data as $val){
			$data = array();
			$data[] = $val['exam_problem_name']; // 設問名
