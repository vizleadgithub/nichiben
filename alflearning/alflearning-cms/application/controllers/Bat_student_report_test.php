...
class Bat_student_report_test extends CI_Controller {
	...
	public function student_report($start_date = '', $end_date = ''){
		...
		$test_flg = 1;
		$one_day_flg = 1;
		$add_history_recode_flg = 1;
		$add_history_recode_sub_flg = 1;
		...
		if( $add_history_recode_flg == 1 ){
			...
			if( $one_day_flg == 1 ){
				$sql.= " AND update_date>='".date("Y-m-d H:i:s",strtotime(" -61 day "))."' ";
			}
		...
		if( $add_history_recode_sub_flg == 1 ){
			...
			if( $test_flg == 1 ){
				//$sql.= " AND student_id=36366 ";
				//$sql.= " AND student_id>17083 ";
				//$sql.= " AND student_id=54427 ";
				//$sql.= " AND student_id>0 ";
				//$sql.= " AND student_id<=15000 ";
			}
			if( $one_day_flg == 1 ){
				$sql.= " AND update_at>'".date("Y-m-d H:i:s",strtotime(" -61 day "))."' ";
			}
			$sql.= "GROUP BY  ";
			$sql.= "  student_id ";
			...
			if ($query_student->num_rows() > 0) {
				foreach($query_student->result_array() as $student_row){
					...
					if( $one_day_flg == 1 ){
						$sql.= " AND report_user_video_viewed.update_at>'".date("Y-m-d H:i:s",strtotime(" -61 day "))."' ";
					}
					$sql.= "   GROUP BY ";
					$sql.= "    tbl_product.product_id ";
		...
		if( $update_product_flg==1 ){
			...
			$sql.= "WHERE 1=1 ";
			if( $one_day_flg == 1 ){
				$sql.= " AND tbl_product.update_date>='".date("Y-m-d H:i:s",strtotime(" -61 day "))."' ";
			}
			$query = $this->db->query($sql);
		...
		if( $update_exam_answer_flg == 1 ){
			...
			if( $one_day_flg == 1 ){
				$sql.= " AND exam_answer_date>='".date("Y-m-d H:i:s",strtotime(" -61 day "))."' ";
			}
			$sql.= "GROUP BY ";
			$sql.= "  student_id ";
			$sql.= " ,product_id ";
		...
		if( $update_video_history_flg == 1 ){
			...
			if ($query_student->num_rows() > 0) {
				foreach($query_student->result_array() as $student_row){
					...
					if( $one_day_flg == 1 ){
						$sql.= " AND ( ";
						$sql.= "    complete_date >'".date("Y-m-d H:i:s",strtotime(" -61 day "))."' ";
						$sql.= " OR regist_at >'".date("Y-m-d H:i:s",strtotime(" -61 day "))."' ";
						$sql.= " OR update_at>'".date("Y-m-d H:i:s",strtotime(" -61 day "))."' ";
						$sql.= " ) ";
					}
					$sql.= "GROUP BY  ";
					$sql.= "  student_id ";
					...
					if ($query_product->num_rows() > 0) {
						foreach($query_product->result_array() as $product_row){
							...
							if( trim($video_ids)!="" ){
								...
								if( $one_day_flg == 1 ){
									$sql.= " AND ( ";
									$sql.= "    regist_at>'".date("Y-m-d H:i:s",strtotime(" -61 day "))."' ";
									$sql.= " OR complete_date>'".date("Y-m-d H:i:s",strtotime(" -61 day "))."' ";
									$sql.= " OR reading_date>'".date("Y-m-d H:i:s",strtotime(" -61 day "))."' ";
									$sql.= " ) ";
								}
								$query1 = $this->db->query($sql, array($student_row["student_id"]));
								//視聴完了したビデオが含まれる商品毎の履歴
