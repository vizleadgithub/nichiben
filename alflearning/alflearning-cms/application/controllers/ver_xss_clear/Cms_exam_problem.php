...
class Cms_exam_problem extends CI_Controller {
	//----------------------------------------------
	//プライベート変数宣言
	//----------------------------------------------
...
//		}
	...
	function index($offset=0){
		...
		if ( !$this->input->post() ){
			$data = $this->session->userdata('exam_problem_search_cond') ?: array(
				's_cource' => '',
				's_free_word' => '',
				's_exam_problem_group' => '',
			);
		} else {
			//データ取得
			$data['s_cource']             = ($this->input->post('s_cource', TRUE) ?? '');
			$data['s_free_word']          = ($this->input->post('s_free_word', TRUE) ?? '');
			$data['s_exam_problem_group'] = ($this->input->post('s_exam_problem_group', TRUE) ?? '');
			$this->session->set_userdata('exam_problem_search_cond', $data);
		}

		$exam_problem_list = $this->model_exam_problem->get_exam_problem_list(array(
			...
			's_exam_problem_group' => $data['s_exam_problem_group'],  //search
		));
		$data['exam_problem_list'] = $exam_problem_list['items'];
		
		//ページネーション設定
		$config['base_url']   = base_url().'/cms_exam_problem/index';
		$config['per_page']   = $per_page;
		$config['total_rows'] = $exam_problem_list['cnt'];
		$config['first_link'] = '&lt;&lt;';
		$config['last_link']  = '&gt;&gt;';
		...
		//ビュー設定引数設定
		$disp_param = array(
						'view_name'   => 'cms_exam_problem/index',
						'submenu_idx' => 2,
						'view_data'   => $data,
					);
		//ビュー設定
	...
	//新規フォーム表示
	//----------------------------------------------
	function newdata($exam_problem_id = 0){
		//表示用変数の初期化
		$data = array();
		...
		//初期表示設定
		$data['exam_problem']['update_flg']             = 0;
		$data['exam_problem']['exam_problem_id']        = 0;
		$data['exam_problem']['exam_problem_name']      = '';
		
		$data['exam_problem']['problem_kind']                   = 1;   // 設問種類 1:テキスト、2:動画、3:図書室
	  //$data['exam_problem']['problem_contents']               = '';  // 設問内容
		$data['exam_problem']['problem_contents_text']          = '';  // 設問内容 テキスト
		$data['exam_problem']['problem_contents_book_library']  = -1;  // 設問内容 図書室ID
		$data['exam_problem']['problem_contents_video']         = -1;  // 設問内容 ビデオID
		$data['exam_problem']['problem_note']                   = '';  // 設問備考
		
		$data['exam_problem']['answer_kind']             = 1;             // 解答種類 1:単一形式、2:複数形式、3:フリー回答
	  //$data['exam_problem']['answer_contents']         = array();       // 解答内容  JSON型
		$data['exam_problem']['answer_contents_no']      = array(1,2);    // 解答内容  選択肢順番（1,2,3,…）
		$data['exam_problem']['answer_contents_word']    = array('','');  // 解答内容  選択肢文言
		$data['exam_problem']['answer_contents_correct'] = array( 0,0);   // 解答内容  正誤フラグ（1:正解、0:不正解）
		$data['exam_problem']['answer_contents_text']    = '';            // 解答内容  選択肢フリー回答
		
		$data['exam_problem']['answer_point']                          = 0;   // 解答配点
		$data['exam_problem']['answer_explain_kind']                   = 9;   // 解答解説種類 1:テキスト、2:動画、3:図書室、9:なし
		$data['exam_problem']['answer_explain_contents_text']          = '';  // 解答解説内容 - テキスト
		$data['exam_problem']['answer_explain_contents_video']         = -1;  // 解答解説内容 - ビデオ
		$data['exam_problem']['answer_explain_contents_book_library']  = -1;  // 解答解説内容 - 図書室
		$data['exam_problem']['answer_explain_note']                   = '';  // 解答解説備考

		$data['exam_problem']['teacher_id']             = $this->libauth->get_teacher_id();
		$data['exam_problem']['teacher_name']           = '';
		$data['exam_problem']['exam_problem_lectures']  = array();  // 所属講座群

		// 設問IDがある場合、情報取得
		if($exam_problem_id > 0){
			$copy_data = $this->detail($exam_problem_id, 1);
			
			if(!empty($copy_data)){
				$data['exam_problem']['update_flg']             = 0;
				$data['exam_problem']['exam_problem_id']        = 0;
				$data['exam_problem']['exam_problem_name']      = $copy_data['exam_problem']['exam_problem_name'];
				
				$data['exam_problem']['problem_kind']                   = $copy_data['exam_problem']['problem_kind'];
				$data['exam_problem']['problem_contents_text']          = $copy_data['exam_problem']['problem_contents_text'];
				$data['exam_problem']['problem_contents_book_library']  = $copy_data['exam_problem']['problem_contents_book_library'];
				$data['exam_problem']['problem_contents_video']         = $copy_data['exam_problem']['problem_contents_video'];
				$data['exam_problem']['problem_note']                   = $copy_data['exam_problem']['problem_note'];
				
				$data['exam_problem']['answer_kind']             = $copy_data['exam_problem']['answer_kind'];
				$data['exam_problem']['answer_contents_no']      = $copy_data['exam_problem']['answer_contents_no'];
				$data['exam_problem']['answer_contents_word']    = $copy_data['exam_problem']['answer_contents_word'];
				$data['exam_problem']['answer_contents_correct'] = $copy_data['exam_problem']['answer_contents_correct'];
				$data['exam_problem']['answer_contents_text']    = $copy_data['exam_problem']['answer_contents_text'];

				$data['exam_problem']['answer_point']                          = $copy_data['exam_problem']['answer_point'];
				$data['exam_problem']['answer_explain_kind']                   = $copy_data['exam_problem']['answer_explain_kind'];
				$data['exam_problem']['answer_explain_contents_text']          = $copy_data['exam_problem']['answer_explain_contents_text'];
				$data['exam_problem']['answer_explain_contents_video']         = $copy_data['exam_problem']['answer_explain_contents_video'];
				$data['exam_problem']['answer_explain_contents_book_library']  = $copy_data['exam_problem']['answer_explain_contents_book_library'];
				$data['exam_problem']['answer_explain_note']                   = $copy_data['exam_problem']['answer_explain_note'];

				$data['exam_problem']['teacher_id']              = $copy_data['exam_problem']['teacher_id'];
				$data['exam_problem']['teacher_name']            = '';
				$data['exam_problem']['exam_problem_lectures']   = $copy_data['exam_problem']['exam_problem_lectures'];
			}
		}
		...
		if( getenv('URL_SERVICE')=='mitemo' ){
			...
			$data['exam_problem']['teacher_id'] = $this->model_teacher->get_teacher_representative_school($this->libauth->get_school_id());
		}

		//ビュー設定引数設定
		$disp_param = array(
						'view_name'   => 'cms_exam_problem/edit',
						'submenu_idx' => 2,
						'view_data'   => $data,
					);
		//ビュー設定
	...
	function edit(){
		...
		if( $edit_form_data['update_flg']!=0){
			$auth_exam_problem = $this->_get_auth_exam_problem($edit_form_data['exam_problem_id']);
		}

		// 権限を持たない場合、エラーを返す
		if($auth_exam_problem == 0){
			//戻り先設定
			$data['returnurl']       = site_url('cms_exam_problem');  //.'/detail/'.$edit_form_data['exam_id'].'/';
			$data['error_message']   = $this->lang->line_or_def('error_edit_auth','修正権限がありません<br />ログインし直してください');
			$data['select_callview'] = 'exam_problem';
		...
		elseif(isset($edit_form_data['exam_problem_id']) &&  $edit_form_data['exam_problem_id'] <> ''){
			//画面表示用データ設定
			$data['exam_problem'] = $edit_form_data;
		...
		//	$this->load->model('model_exam_problem');
			...
			$disp_param = array(
							'view_name'   => 'cms_exam_problem/edit',
							'submenu_idx' => ($data['exam_problem']['update_flg']==0 ? 2 : 3),
							'view_data'   => $data,
						);
			//確認フォーム表示
			$this->_display_view($disp_param);
			...
			//戻り先設定
			$data['returnurl'] = site_url('exam_problem');
	...
	//      フラグありの場合、返却値を変更
	//----------------------------------------------
	function detail($exam_problem_id, $copy_newdata_flag = 0){
		...
		if(count($db_data) > 0){
			...
			//表示用データ設定
			$data['exam_problem']['update_flg']                     = 1;
			$data['exam_problem']['exam_problem_id']                = $db_data['exam_problem_id'];
			$data['exam_problem']['exam_problem_name']              = $db_data['exam_problem_name'];
			$data['exam_problem']['teacher_id']                     = $db_data['teacher_id'];
			$data['exam_problem']['teacher_name']                   = $db_data['teacher_name'];
		//	$data['exam_problem']['exam_problem_lectures']          = $this->input->post('exam_problem_lectures')?$this->input->post('exam_problem_lectures'):array();
			$data['exam_problem']['problem_kind']                   = $db_data['problem_kind'];
			$data['exam_problem']['problem_contents_text']          = $problem_contents_text;
			$data['exam_problem']['problem_contents_video']         = $problem_contents_video;
			$data['exam_problem']['problem_contents_book_library']  = $problem_contents_book_library;
			$data['exam_problem']['problem_note']                   = $db_data['problem_note'];
			$data['exam_problem']['answer_kind']                    = $db_data['answer_kind'];
			$data['exam_problem']['answer_contents_no']             = $answer_contents_no;
			$data['exam_problem']['answer_contents_word']           = $answer_contents_word;
			$data['exam_problem']['answer_contents_correct']        = $answer_contents_correct;
			$data['exam_problem']['answer_contents_text']           = $answer_contents_text;
			$data['exam_problem']['answer_point']                   = $db_data['answer_point'];
			
			$data['exam_problem']['answer_explain_kind']                   = $db_data['answer_explain_kind'];
			$data['exam_problem']['answer_explain_contents_text']          = $answer_explain_contents_text;
			$data['exam_problem']['answer_explain_contents_video']         = $answer_explain_contents_video;
			$data['exam_problem']['answer_explain_contents_book_library']  = $answer_explain_contents_book_library;
			$data['exam_problem']['answer_explain_note']                   = $db_data['answer_explain_note'];
			
			$data['exam_problem']['school_id']                      = $db_data['school_id'];
			
			//所属講座取得
			$data['exam']['exam_problem_lectures']  = array();
			$lectures = $this->model_exam_problem->get_exam_problem_lectures($data_param);
			$idx = -1;
			if(isset($lectures)){
				foreach($lectures as $lecture){
					$idx++;
					$data['exam_problem']['exam_problem_lectures'][$idx] = $lecture['cource_id'];
				}
			}
			
			
			//セッションへDB取得データを書き込み
			$this->session->set_userdata('edit_form_data',serialize($data['exam_problem']));
			
			//所属講座名設定
			$data['exam_problem']['exam_problem_lectures_name'] = array();
			$this->load->model('model_cource');
			if($data['exam_problem']['exam_problem_lectures']){
				foreach($data['exam_problem']['exam_problem_lectures'] as $idx => $lecture){
					//データ取得用引数設定
					$name_param = array(
									'cource_id' => $lecture,
								);
				//	$data['student']['student_lectures_name'][$idx] = $this->model_cource->get_name($name_param);
					if($this->model_cource->get_name($name_param)){
						$data['exam_problem']['exam_problem_lectures_name'][$idx] = $this->model_cource->get_name($name_param);
					}
				}
				//連想キーと要素との関係を維持しつつ配列をソート
				asort($data['exam_problem']['exam_problem_lectures_name']);
			}
			
			// 設問IDより所属する設問グループIDを取得
			$this->load->model('model_exam_problem_group');
			$exam_problem_group_id = array();
			$exam_problem_groups = $this->model_exam_problem_group->get_exam_problem_group_id_list(array(
				"school_id"               => $this->libauth->get_school_id(),
				"select_exam_problem_id"  => $db_data['exam_problem_id'],
				"cource_flag"             => 1,
			));
			if($exam_problem_groups){
				foreach($exam_problem_groups as $index => $value){
					$exam_problem_group_id[] = $value;
				}
			}
			$data['exam_problem_group_id'] = $exam_problem_group_id;
			
			// 設問内容 - ビデオ名設定
			$data['exam_problem']['problem_contents_video_name'] = '';
			if($data['exam_problem']['problem_kind']==2){
				$this->load->model('model_video');
				$temp_video = $this->model_video->get_material(array(
					'video_id' => $data['exam_problem']['problem_contents_video'],
				));
				if($temp_video){
					$data['exam_problem']['problem_contents_video_name'] = '[No'.$temp_video['video_id'].'] '.$temp_video['video_logic_name'];
				}
			}
			
			// 設問内容 - 図書室名設定
			$data['exam_problem']['problem_contents_book_library_name'] = '';
			if($data['exam_problem']['problem_kind']==3){
				$this->load->model('model_book_library');
				$temp_book_library = $this->model_book_library->get_data(array(
					"school_id"        => $this->libauth->get_school_id(),
					'book_library_id'  => $data['exam_problem']['problem_contents_book_library'],
				));
				if($temp_book_library){
					$data['exam_problem']['problem_contents_book_library_name'] = '[No'.$temp_book_library['book_library_id'].'] '.$temp_book_library['book_library_logic_name'];
				}
			}
			
			// 解答解説内容 - ビデオ名設定
			$data['exam_problem']['answer_explain_contents_video_name'] = '';
			if($data['exam_problem']['answer_explain_kind']==2){
				$this->load->model('model_video');
				$temp_video = $this->model_video->get_material(array(
					'video_id' => $data['exam_problem']['answer_explain_contents_video'],
				));
				if($temp_video){
					$data['exam_problem']['answer_explain_contents_video_name'] = '[No'.$temp_video['video_id'].'] '.$temp_video['video_logic_name'];
				}
			}
			
			// 解答解説内容 - 図書室名設定
			$data['exam_problem']['answer_explain_contents_book_library_name'] = '';
			if($data['exam_problem']['answer_explain_kind']==3){
				$this->load->model('model_book_library');
				$temp_book_library = $this->model_book_library->get_data(array(
					"school_id"        => $this->libauth->get_school_id(),
					'book_library_id'  => $data['exam_problem']['answer_explain_contents_book_library'],
				));
				if($temp_book_library){
					$data['exam_problem']['answer_explain_contents_book_library_name'] = '[No'.$temp_book_library['book_library_id'].'] '.$temp_book_library['book_library_logic_name'];
				}
			}
			...
			if($copy_newdata_flag==0){
				...
				$auth_exam_problem = $this->_get_auth_exam_problem($data['exam_problem']['exam_problem_id']);
				$data['exam_problem_edit_delete_flag'] = $auth_exam_problem;
				
				//ビュー設定引数設定
				$disp_param = array(
								'view_name'   => 'cms_exam_problem/confirm',
								'submenu_idx' => 3,
								'view_data'   => $data,
								'exam_problem_id'     => $db_data['exam_problem_id'],
							);
				//確認フォーム表示
				$this->_display_view($disp_param);
			}else{
			...
			if($copy_newdata_flag==0){
				...
				//一覧に戻る
				//$this->index();
				header("Location:/cms_exam_problem/");
				exit();
			}else{
				return NULL;
			}
...
//exit();
		...
		$check_exam_problem_lectures = $this->input->post('exam_problem_lectures')?$this->input->post('exam_problem_lectures'):array();

		if(!check_array_data_num(array($check_exam_problem_lectures))){
			$this->lang->load('error');
			...
			if($this->form_validation->run() == FALSE || ($problem_error_msg != "") || ($answer_error_msg != "") || ($answer_explain_error_msg != "") ){
				...
				$data['exam_problem']['update_flg']                     = $this->input->post('update_flg');
				$data['exam_problem']['exam_problem_id']                = $this->input->post('exam_problem_id');
				$data['exam_problem']['exam_problem_name']              = '';
				$data['exam_problem']['teacher_id']                     = $this->input->post('teacher_id');
				$data['exam_problem']['exam_problem_lectures']          = $this->input->post('exam_problem_lectures')?$this->input->post('exam_problem_lectures'):array();
				$data['exam_problem']['problem_kind']                   = $this->input->post('problem_kind');
				$data['exam_problem']['problem_contents_text']          = $this->input->post('problem_contents_text');
				$data['exam_problem']['problem_contents_video']         = $this->input->post('problem_contents_video');
				$data['exam_problem']['problem_contents_book_library']  = $this->input->post('problem_contents_book_library');
				$data['exam_problem']['problem_note']                   = $this->input->post('problem_note');
				$data['exam_problem']['answer_kind']                    = $this->input->post('answer_kind');
				$data['exam_problem']['answer_contents_no']             = $answer_contents_no;
				$data['exam_problem']['answer_contents_word']           = $answer_contents_word;
				$data['exam_problem']['answer_contents_correct']        = $answer_contents_correct;
				$data['exam_problem']['answer_contents_text']           = $this->input->post('answer_contents_text');
				$data['exam_problem']['answer_point']                   = $this->input->post('answer_point');
				$data['exam_problem']['answer_explain_kind']                   = $this->input->post('answer_explain_kind');
				$data['exam_problem']['answer_explain_contents_text']          = $this->input->post('answer_explain_contents_text');
				$data['exam_problem']['answer_explain_contents_video']         = $this->input->post('answer_explain_contents_video');
				$data['exam_problem']['answer_explain_contents_book_library']  = $this->input->post('answer_explain_contents_book_library');
				$data['exam_problem']['answer_explain_note']                   = $this->input->post('answer_explain_note');
				...
				$data['exam_problem']['update_flg']                     = $this->input->post('update_flg');
				$data['exam_problem']['exam_problem_id']                = $this->input->post('exam_problem_id');
				$data['exam_problem']['exam_problem_name']              = $this->input->post('exam_problem_name');
				$data['exam_problem']['teacher_id']                     = $this->input->post('teacher_id');
				$data['exam_problem']['exam_problem_lectures']          = $this->input->post('exam_problem_lectures')?$this->input->post('exam_problem_lectures'):array();
				$data['exam_problem']['problem_kind']                   = $this->input->post('problem_kind');
				$data['exam_problem']['problem_contents_text']          = $this->input->post('problem_contents_text');
				...
				$data['exam_problem']['school_id']                      = $this->libauth->get_school_id();
				...
				$temp_teacher = $this->model_teacher->get_name(array(
					'teacher_id' => $data['exam_problem']['teacher_id'],
				));
				if($temp_teacher){
					$data['exam_problem']['teacher_name'] = $temp_teacher;
				}
				...
				if($data['exam_problem']['problem_kind']==2){
					...
					if($temp_video){
						$data['exam_problem']['problem_contents_video_name'] = $temp_video['video_logic_name'];
					}
				}
				...
				if($data['exam_problem']['problem_kind']==3){
					...
					if($temp_book_library){
						$data['exam_problem']['problem_contents_book_library_name'] = $temp_book_library['book_library_logic_name'];
					}
				}
	...
	function commit(){
		...
		if(isset($edit_form_data['exam_problem_id']) && $edit_form_data['exam_problem_id'] != ""){
			//モデル読み込み
			$this->load->model('model_exam_problem');
			...
			// 試験テーブル更新
			$exam_problem_result = $this->model_exam_problem->update_exam_problem($update_param);
			
			// 試験講座テーブル更新
			$data_param = array(
							'exam_problem_id'  => $exam_problem_result['lastInsertId'],
							'data'             => $update_param['data'],
						);
			$this->model_exam_problem->update_exam_problem_lectures($data_param);
		...
		//	$this->model_exam->update_exam_student_group($data_param);
			...
			//ビュー設定引数設定
			//$data = array();
			$data['exam_problem_id'] = $exam_problem_result['lastInsertId'];
			$disp_param = array(
							'view_name'   => 'cms_exam_problem/commit',
							'submenu_idx' => 3,
							'view_data'   => $data,
						);
			//確認フォーム表示
	...
	//データ削除（論理削除）
	//----------------------------------------------
	function delete_item($exam_problem_id){
		// load language
		$this->lang->load('error');
		...
		$auth_exam_problem = 1;
		$auth_exam_problem = $this->_get_auth_exam_problem($exam_problem_id);
		...
		if($auth_exam_problem == 0){
			...
			//データ削除
			$data = $this->model_exam_problem->delete_item(array(
				'exam_problem_id' => $exam_problem_id,
			));
	...
	function _display_view($param) {
		...
		$param['view_data']['exam_problem_groups_dropdown']  = $this->_get_exam_problem_group_list_array($drop_param);
		...
		if( isset($param['view_data']['exam_problem']) ){
			$temp_cource_id = implode(",", $param['view_data']['exam_problem']['exam_problem_lectures']);
...
//		$data['problem_kind'][3] = $this->lang->line_or_def('common_book_library' , '図書室');
	...
	function _get_select_answer_kind(){
		...
		$data['answer_kind'][3] = $this->lang->line_or_def('common_free_exam_answer', 'フリー解答');
...
//		$data['answer_explain_kind'][3] = $this->lang->line_or_def('common_book_library' , '図書室');
	...
	function _get_course_video_list_array($param){
		...
		$data['problem_contents_video'][-1] = $this->lang->line_or_def('msg_exam_problem_video_select','ビデオを選択してください');
	...
	//----------------------------------------------
	function _get_exam_problem_group_list_array($param){
		//引数設定
		$param = array_merge(
						array(
							'school_id'   => 0,
		...
		//一覧ドロップダウン生成
		$data['exam_problem_group_list'] = $this->model_exam_problem_group->get_exam_problem_dropdown_list($param);
		if( !$data['exam_problem_group_list'] ){
			$data['exam_problem_group_list'] = [];
		}
		$data['exam_problem_groups'] = array();
		if( count($data['exam_problem_group_list']) != 0 ) {
			$data['exam_problem_groups'][0] = '';
			foreach ( $data['exam_problem_group_list'] as $exam_problem_group ) {
				$data['exam_problem_groups'][$exam_problem_group['exam_problem_group_id']] = $exam_problem_group['exam_problem_group_name'];
			}
		}
		
		return $data['exam_problem_groups'];
	}
	...
	//----------------------------------------------
	function _get_auth_exam_problem($exam_problem_id = 0){
		...
		//}elseif( ($work_auth['exam'] == 1) || ($work_auth['exam_no_eval'] == 1) ){
			...
			// 設問の再取得
			$this->load->model('model_exam_problem');
			$exam_data = $this->model_exam_problem->get_exam_problem(array(
							'exam_problem_id' => $exam_problem_id,
						));
			// 課題の管理講師 = ログイン講師
	...
	function _answer_problem($param){
		...
		if( ($param['answer_kind'] == 1) || ($param['answer_kind'] == 2) ){
			...
			$values = array_map('intval', $param['answer_contents_correct']);
			$correct = array_sum($values);  // 正解の数をカウント
	...
	// [Ajax用]設問詳細を取得
	//----------------------------------------------
	function get_exam_problem_detail(){
		
		$this->load->helper('json');
		
		$param = array(
					'school_id'        => $this->libauth->get_school_id(),
					'exam_problem_id'  => ($this->input->post('exam_problem_id')) ? $this->input->post('exam_problem_id') : 0,
					);

		// 設問詳細を取得
		$this->load->model('model_exam_problem');
		$exam_problems = $this->model_exam_problem->get_exam_problem($param);
		// exam_problem.exam_problem_id          -- 設問ID
		// exam_problem.exam_problem_name        -- 設問名
		// exam_problem.problem_kind             -- 設問種類 1:テキスト、2:動画、3:図書室
		// exam_problem.problem_contents         -- 設問内容 テキスト、または、ビデオID・図書室ID
		// exam_problem.problem_note             -- 設問備考
		// exam_problem.answer_kind              -- 解答種類 1:単一形式、2:複数形式、3:フリー回答
		// exam_problem.answer_contents          -- 解答内容 JSON型
		// exam_problem.answer_point             -- 解答配点
		// exam_problem.answer_explain_kind      -- 解答解説種類 1:テキスト、2:動画、3:図書室、9:なし
		// exam_problem.answer_explain_contents  -- 解答解説内容 テキスト、または、ビデオID・図書室ID
		// exam_problem.answer_explain_note      -- 解答解説備考
		// exam_problem.school_id                -- 学校ID
		// exam_problem.teacher_id               -- 講師ID
		// exam_problem.status                   -- 状態 0:有効 9:削除
		// exam_problem.update_at                -- 更新日時
		// 
		...
		if($exam_problems){
			...
			// 設問名・管理講師・解答配点
			$result_data['exam_problems_name']         = $exam_problems['exam_problem_name'];
			$result_data['exam_problems_teacher_name'] = $exam_problems['teacher_name'];
			$result_data['exam_problems_problem_kind'] = $exam_problems['problem_kind'];
			
			// 設問 - 詳細 - 設問内容
			$temp_contents_id = 0;
			$temp_contents    = '';
			if($exam_problems['problem_kind']== 1){
				// 設問種類→1:テキスト
				$temp_contents = str_replace(array("\r\n", "\r", "\n"), array("<br/>", "<br/>", "<br/>"), $exam_problems['problem_contents']);
			}elseif($exam_problems['problem_kind'] == 2){
				// 設問種類 → 2：ビデオ
				$this->load->model('model_video');
				$temp_contents_id = $exam_problems['problem_contents'];
				$db_data          = $this->model_video->get_material(array('video_id' => $temp_contents_id));
				$temp_contents    = $db_data['video_logic_name'];
			}elseif($exam_problems['problem_kind'] == 3){
				// 設問種類 → 3：図書室
				$this->load->model('model_book_library');
				$temp_contents_id = $exam_problems['problem_contents'];
			...
			$result_data['exam_problems_problem_contents_id'] = $temp_contents_id;
			$result_data['exam_problems_problem_contents']    = $temp_contents;

			// 設問 - 詳細 - 解答種類（ID）
			$result_data['exam_problems_answer_kind'] = $exam_problems['answer_kind'];
					
			// 設問 - 詳細 - 解答内容	
			$temp_answer_contents = '';
			if( ($exam_problems['answer_kind']==1) || ($exam_problems['answer_kind']==2) ){
				// 解答種類 → 1：単一形式、2：複数形式
				$array_answer_contents = obj2arr(json_decode( $exam_problems['answer_contents'] )); // JSON型、選択した番号の文字列
			...
			}elseif($exam_problems['answer_kind']==3){
				// 解答種類 → 3：フリー回答
				$array_answer_contents = obj2arr(json_decode( $exam_problems['answer_contents'] )); // JSON型、選択した番号の文字列
				$line_contents         = (array)$array_answer_contents['answer_contents'][0];
				$line_contents['word'] = str_replace(array("\r\n", "\r", "\n"), array("<br/>", "<br/>", "<br/>"), $line_contents['word']);
			...
			$result_data['exam_problems_answer_contents'] = $temp_answer_contents;
		}else{
			$result_data = array();
		}
	...
	// [Ajax用]解答詳細を取得
	//----------------------------------------------
	function get_exam_answer_detail(){
	
		$this->load->helper('json');
		
		$param = array(
					'exam_id'           => ($this->input->post('exam_id'))                 ? $this->input->post('exam_id')                 : 0,
					'student_id'        => ($this->input->post('answer_student_id'))       ? $this->input->post('answer_student_id')       : 0,
					'exam_answer_no'    => ($this->input->post('answer_no'))               ? $this->input->post('answer_no')               : 0,
					'exam_answer_data'  => ($this->input->post('exam_answer_data'))        ? $this->input->post('exam_answer_data')        : "",
				);

		// 解答修正確認画面から戻った際、正誤・解答配点 を編集した状態にする必要がある  例："483/1/23@15/1/23"
		$array_exam_answer_data = array();
		if($param['exam_answer_data']!=""){
			// カンマ区切りで配列変換
			$array_data = explode("@", $param['exam_answer_data'] );
			foreach($array_data as $answer){
				...
				if( (count($array_answer)==3) && ($array_answer[0]>0) && (($array_answer[1]==0) || ($array_answer[1]==1)) && ($array_answer[2]>-1) ){
					$array_exam_answer_data[$array_answer[0]] = array(
						'exam_answer_mark'  => $array_answer[1],       // 正誤
						'exam_answer_point' => $array_answer[2],       // 解答配点
					);
				}
			}
		}
		...
		if(isset($exam_answer_detail)){
			foreach($exam_answer_detail as $exam_answer){
				...
				// 設問ID
				$temp_result_data['exam_problem_id'] = $exam_answer['exam_problem_id'];
				
				// 設問名
				$temp_result_data['exam_problem_name'] = $exam_answer['exam_problem_name'];
				
				// 解答ID
				$temp_result_data['exam_answer_id'] = $exam_answer['exam_answer_id'];
				
				// 解答種類
				$temp_result_data['answer_kind'] = $exam_answer['answer_kind'];
				...
				// 正誤（ID）
				$temp_result_data['exam_answer_mark'] = $exam_answer['exam_answer_mark'];
				if( isset($array_exam_answer_data[ $exam_answer['exam_answer_id'] ]['exam_answer_mark']) ){
					$temp_result_data['exam_answer_mark'] = $array_exam_answer_data[ $exam_answer['exam_answer_id'] ]['exam_answer_mark'];
				}
				
				// 解答配点（解答結果による配点）
				$temp_result_data['exam_answer_point'] = $exam_answer['exam_answer_point'];
				if( isset($array_exam_answer_data[ $exam_answer['exam_answer_id'] ]['exam_answer_point']) ){
					$temp_result_data['exam_answer_point'] = $array_exam_answer_data[ $exam_answer['exam_answer_id'] ]['exam_answer_point'];
				}
				
				// 解答配点（設問に設定されている配点）
				$temp_result_data['answer_point'] = $exam_answer['answer_point'];
				
				array_push($result_data, $temp_result_data);
			}
		}else{
			$temp_result_data = array();
			$temp_result_data['exam_problem_id'] = 0;
			array_push($result_data, $temp_result_data);
		}
