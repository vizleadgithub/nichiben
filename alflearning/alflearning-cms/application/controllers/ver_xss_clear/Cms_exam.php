...
class Cms_exam extends CI_Controller {
	//----------------------------------------------
	//プライベート変数宣言
	//----------------------------------------------
...
//		}
	...
	function index($offset=0){
		...
		if ( !$this->input->post() ){
			$data = $this->session->userdata('exam_search_cond') ?: array(
				's_cource' => '',
				's_free_word' => '',
			);
		} else {
			...
			$this->session->set_userdata('exam_search_cond', $data);
		}

		$exam_list = $this->model_exam->get_exam_list(array(
		...
		//$data['exam_list'] = $exam_list['items'];
		$exam_table_list = $exam_list['items'];
		
		//問題受付状態の取得
		$now = date('YmdHis');
		for($i = 0; $i<count($exam_table_list); $i++){
			if($exam_table_list[$i]['exam_open'] <= $now 
					&& $now <= $exam_table_list[$i]['exam_close']){
				$exam_table_list[$i]["disp_status"] = $this->lang->line_or_def('common_accept_now','受付中');
			} else if( $exam_table_list[$i]['exam_open'] > $now){
				$exam_table_list[$i]["disp_status"] = $this->lang->line_or_def('common_accept_plan','受付予定');
			} else if( $exam_table_list[$i]['exam_close'] < $now){
				$exam_table_list[$i]["disp_status"] = $this->lang->line_or_def('common_accept_end','受付終了');
			} else {
				$exam_table_list[$i]["disp_status"] = "";
			}
		}
		$data['exam_list'] = $exam_table_list;
		
		//ページネーション設定
		$config['base_url']   = base_url().'/cms_exam/index';
		$config['per_page']   = $per_page;
		$config['total_rows'] = $exam_list['cnt'];
		$config['first_link'] = '&lt;&lt;';
		$config['last_link']  = '&gt;&gt;';
	...
	function newdata(){
		...
		//初期表示設定
		$data['exam']['update_flg']           = 0;
		$data['exam']['exam_id']              = 0;
		$data['exam']['exam_name']            = '';
		$data['exam']['exam_caption']         = '';
		$data['exam']['exam_open']            = '1900/01/01 00:00:00';
		$data['exam']['exam_close']           = '2100/12/31 23:59:59';
		$data['exam']['public_flag']          = 0;
		$data['exam']['resubmit_flag']        = 1;
		$data['exam']['display_format']       = 0;
		$data['exam']['marking_public_flag']  = 9;
		$data['exam']['marking_public_kind']  = 1;
		$data['exam']['marking_public_open']  = date('Y/m/d H:i:s', strtotime("+1 month"));  // システム日時 + 一か月先
		$data['exam']['teacher_id']           = $this->libauth->get_teacher_id();
		$data['exam']['teacher_name']         = '';
		$data['exam']['exam_lectures']        = array();  // 所属講座ID
		$data['exam']['exam_students']        = array();  // 選択済み受講者ID
		$data['exam']['exam_problems']        = array();  // 選択済み設問ID
		$data['exam']['exam_remind_time']     = array();
		$data['exam']['exam_remind_unit']     = array();
		$data['exam']['criteria_type']        = 1;
		$data['exam']['criteria_value']       = '';

		// mitemo対応（管理講師を裏側で持つ）
		if( getenv('URL_SERVICE')=='mitemo' ){
			$this->load->model('model_teacher');
			$data['exam']['teacher_id'] = $this->model_teacher->get_teacher_representative_school($this->libauth->get_school_id());
		}

		//ビュー設定引数設定
		$disp_param = array(
						'view_name'   => 'cms_exam/edit',
						'submenu_idx' => 2,
						'view_data'   => $data,
					);
		//ビュー設定
	...
	function edit(){
		...
		if( $edit_form_data['update_flg']!=0){
			$auth_exam = $this->_get_auth_exam($edit_form_data['exam_id']);
		}
		
		// 権限を持たない場合、エラーを返す
		if($auth_exam == 0){
			//戻り先設定
			$data['returnurl']       = site_url('cms_exam');  //site_url('cms_exam').'/detail/'.$edit_form_data['exam_id'].'/';
		...
		elseif(isset($edit_form_data['exam_id']) &&  $edit_form_data['exam_id'] <> ''){
			//画面表示用データ設定
			$data['exam'] = $edit_form_data;
			...
			if( $this->input->post('exam_students') ){
				...
				$param_select_student = array(
								'exam_id'    => $data['exam']['exam_id'],
								'cource_id'  => $data['exam']['exam_lectures'],
							);
				$lectures = $this->model_exam->get_exam_student($param_select_student);
				$idx = -1;
				if(isset($lectures)){
					foreach($lectures as $lecture){
						$idx++;
						$data['exam']['exam_students'][$idx] = intval($lecture['student_id']);
					}
				}
			}
			...
			if( $this->input->post('exam_problems') ){
				...
				$idx = -1;
				if(isset($exam_problems)){
					foreach($exam_problems as $exam_problem){
						$idx++;
						$data['exam']['exam_problems'][$idx] = intval($exam_problem['exam_problem_id']);
					}
				}
			}


			if( !isset($data['exam']['exam_remind_time'])){
				...
				$queue_list = $this->model_queue->get_queue(array(
					'queue_type'          => 'exam',
					'queue_type_id'       => $data['exam']['exam_id'],
					'queue_kind'          => 'mail',
				...
				if($queue_list){
					...
					foreach($queue_list as $queue){
						...
						$data['exam']['exam_remind_time'][$count] = $queue_kind_detail->args->TIME;
						$data['exam']['exam_remind_unit'][$count] = $queue_kind_detail->args->UNIT;
					}
				}
			}
			...
			$disp_param = array(
							'view_name'   => 'cms_exam/edit',
							'submenu_idx' => ($data['exam']['update_flg']==0 ? 2 : 3),
							'view_data'   => $data,
						);
			//確認フォーム表示
			$this->_display_view($disp_param);
	...
	function detail($exam_id){
		...
		foreach($temp_exam_answer_data as $exam_answer_data){
			...
			if($string_exam_answer_data==""){
				$string_exam_answer_data  = $temp[1]."/".$temp[2]."/".$temp[3];
			}else{
				$string_exam_answer_data .= "@".$temp[1]."/".$temp[2]."/".$temp[3];
			}
		}
		...
		//データ取得
		$db_data = $this->model_exam->get_exam($data_param);
		...
		if(count($db_data) > 0){
			...
			//表示用データ設定
			$data['exam']['update_flg']           = 1;
			$data['exam']['exam_id']              = $db_data['exam_id'];
			$data['exam']['exam_name']            = $db_data['exam_name'];
			$data['exam']['exam_caption']         = $db_data['exam_caption'];
			$data['exam']['exam_open']            = $db_data['exam_open'];
			$data['exam']['exam_close']           = $db_data['exam_close'];
			$data['exam']['public_flag']          = $db_data['public_flag'];
			$data['exam']['resubmit_flag']        = $db_data['resubmit_flag'];
			$data['exam']['marking_public_flag']  = $db_data['marking_public_flag'];
			$data['exam']['marking_public_kind']  = $db_data['marking_public_kind'];
			$data['exam']['marking_public_open']  = ($db_data['marking_public_open'])?$db_data['marking_public_open']:date('Y/m/d H:i:s', strtotime("+1 month"));
			$data['exam']['teacher_id']           = $db_data['teacher_id'];
			$data['exam']['teacher_name']         = $db_data['teacher_name'];
			$data['exam']['display_format']       = $db_data['display_format'];
			$data['exam']['criteria_type']        = $db_data['criteria_type'];
			$data['exam']['criteria_value']       = $db_data['criteria_value'];
			
			$data['exam']['string_exam_answer_data'] = $string_exam_answer_data;  // POST値
			
			//所属講座取得
			$data['exam']['exam_lectures']  = array();
			$lectures = $this->model_exam->get_exam_lectures($data_param);
			$idx = -1;
			if(isset($lectures)){
				foreach($lectures as $lecture){
					$idx++;
					$data['exam']['exam_lectures'][$idx] = $lecture['cource_id'];
				}
			}
			
			// セッションへDB取得データを書き込み
			$this->session->set_userdata('edit_form_data',serialize($data['exam']));
			...
			if(isset($lectures)){
				foreach($lectures as $lecture){
					$idx++;
					$data['exam']['exam_students'][$idx] = $lecture['student_id'];
				}
			}

			//選択済み設問ID・設問名・管理講師名・解答配点を取得
			$data['exam']['exam_problems']                  = array();  // 設問ID
			$data['exam']['exam_problems_name']             = array();  // 設問名
			$data['exam']['exam_problems_teacher_name']     = array();  // 管理講師名
			$data['exam']['exam_problems_answer_point']     = array();  // 解答配点
			
			$exam_problems = $this->model_exam->get_exam_problems($data_param);
			
			$idx = -1;
			if(isset($exam_problems)){
				foreach($exam_problems as $exam_problem){
					$idx++;
					$data['exam']['exam_problems'][$idx] = $exam_problem['exam_problem_id'];
					
					$data['exam']['exam_problems_name'][$idx]         = $exam_problem['exam_problem_name'];
					$data['exam']['exam_problems_teacher_name'][$idx] = $exam_problem['teacher_name'];
					$data['exam']['exam_problems_answer_point'][$idx] = $exam_problem['answer_point'];
				}
			}
			
			// 所属講座名設定
			$data['exam']['exam_lectures_name'] = array();
			$this->load->model('model_cource');
			if($data['exam']['exam_lectures']){
				foreach($data['exam']['exam_lectures'] as $idx => $lecture){
					$name_param = array(
									'cource_id' => $lecture,
								);
				//	$data['student']['student_lectures_name'][$idx] = $this->model_cource->get_name($name_param);
					if($this->model_cource->get_name($name_param)){
						$data['exam']['exam_lectures_name'][$idx] = $this->model_cource->get_name($name_param);
					}
				}
				//連想キーと要素との関係を維持しつつ配列をソート
				asort($data['exam']['exam_lectures_name']);
			}
			...
			// 受講者ID取得（問題詳細画面用）
			// データ取得用引数設定
		//	$data['exam']['lecture_students'] = array();
			$exam_students_confirm_list = "0";
			$param_select_student = array(
							'exam_id'    => $data_param['exam_id'],
							'cource_id'  => $data['exam']['exam_lectures'],
						);
			$lectures_confirm = $this->model_exam->get_exam_student_confirm($param_select_student);
			$idx = -1;
			if(isset($lectures_confirm)){
				foreach($lectures_confirm as $lecture){
					$idx++;
					$data['exam']['exam_students_confirm'][$idx] = $lecture['student_id'];
					
					$exam_students_confirm_list .= ",".$lecture['student_id'];
				}
			}
			
/*
			//受講者名設定（問題詳細画面用の受講者IDより取得するように変更）
			$data['exam']['exam_students_name'] = '---';
			$this->load->model('model_student');
			if($data['exam']['exam_students_confirm']){

				$student_list = $this->model_student->get_name_array( array('student_id_list' => $exam_students_confirm_list) );
				
				foreach($student_list as $idx => $student_data){
					if($data['exam']['exam_students_name'] == '---'){
						$data['exam']['exam_students_name'] = '[No'.$student_data['student_id'].']&nbsp;'.$student_data['student_name'].'&nbsp;&lt;'.$student_data['student_email'].'&gt;';
					}else{
						$data['exam']['exam_students_name'] .= '<br/>'.'[No'.$student_data['student_id'].']&nbsp;'.$student_data['student_name'].'&nbsp;&lt;'.$student_data['student_email'].'&gt;';
					}
				}
				//foreach($data['exam']['exam_students_confirm'] as $idx => $lecture){
				//	//データ取得用引数設定
				//	$name_param = array(
				//					'student_id' => $lecture,
				//				);
				...
				//	if($data['exam']['exam_students_name'] == '---'){
				//		$data['exam']['exam_students_name'] = '[No'.$lecture.']&nbsp;'.$temp['student_name'].'&nbsp;&lt;'.$temp['student_email'].'&gt;';
				//	}else{
				//		$data['exam']['exam_students_name'] .= '<br/>'.'[No'.$lecture.']&nbsp;'.$temp['student_name'].'&nbsp;&lt;'.$temp['student_email'].'&gt;';
				//	}
				//}
			}
*/
			...
			// 設問の解答配点の最大値を取得
			$data['exam']['max_answer_point'] = 0;        // 解答配点の合計
			$exam_param = array(
					'school_id'  => $this->libauth->get_school_id(),
					'exam_id'    => $data_param['exam_id'],
					);
			$this->load->model('model_exam_answer');
			$max_answer_point = $this->model_exam_answer->get_max_answer_point($exam_param);
			$data['exam']['max_answer_point'] = $max_answer_point;
			
			// 解答の取得
			//           ['exam_id']                             // 問題ID
			$data['exam']['answer_date']             = array();  // 解答日時
			$data['exam']['answer_student_id']       = array();  // 解答受講者名
			$data['exam']['answer_student_name']     = array();  // 解答受講者名
			$data['exam']['answer_no']               = array();  // 解答回数（何回目の解答か）
			$data['exam']['answer_point_total']      = array();  // 解答配点の合計
			$data['exam']['exam_answer_latest_flag'] = array();  // 最新解答フラグ　設問の解答が最新かどうか　1：最新、0：過去
			// 
			$list_result = $this->model_exam_answer->get_exam_answer_list($exam_param);
			//$exam_answer_list_count = $list_result['cnt'];
			$exam_answer_list = $list_result['items'];
			
			$check_student_id = ',-1,';
			$idx = -1;
			if(isset($exam_answer_list)){
				foreach($exam_answer_list as $exam_answer){
					$idx++;
					
					// 解答日時・(受講者ID)・受講者名・(解答回数)・解答配点
					$data['exam']['answer_date'][$idx]         = date('Y/m/d H:i:s', strtotime($exam_answer['answer_date']));
					$data['exam']['answer_student_id'][$idx]   = $exam_answer['answer_student_id'];
					$data['exam']['answer_student_name'][$idx] = $exam_answer['answer_student_name'];
					$data['exam']['answer_no'][$idx]           = $exam_answer['answer_no'];
					$data['exam']['answer_point_total'][$idx]  = $exam_answer['answer_point_total'];
					
					// 最新解答フラグ（check_student_id にない場合は最新解答とする）
					$data['exam']['exam_answer_latest_flag'][$idx] = 0;
					if( !strpos($check_student_id, ",".trim($exam_answer['answer_student_id']).",") ){
						$data['exam']['exam_answer_latest_flag'][$idx] = 1;
						$check_student_id .= $exam_answer['answer_student_id'].',';
					}
				}
			}
			...
			$auth_exam = 1;
			$auth_exam = $this->_get_auth_exam($data['exam']['exam_id']);
			$data['exam_edit_delete_flag'] = $auth_exam;
	...
	function confirm(){
		...
		$this->form_validation->set_rules('exam_id'       , $this->lang->line_or_def('common_id','ID')                       , 'trim|xss_clean|numeric');
		$this->form_validation->set_rules('exam_name'     , $this->lang->line_or_def('common_exam_name','問題（テスト）名')  , 'trim|xss_clean|required');
		...
		$this->form_validation->set_rules('exam_caption'  , $this->lang->line_or_def('common_caption','説明')                , 'trim|xss_clean');
		$this->form_validation->set_rules('exam_open'     , $this->lang->line_or_def('common_submit_period','提出期間')      , 'trim|xss_clean|required|callback_datetime_check');
		$this->form_validation->set_rules('exam_close'    , $this->lang->line_or_def('common_submit_period','提出期間')      , 'trim|xss_clean|required|callback_datetime_check|callback_period_check[exam_open]');
...
//		$this->form_validation->set_rules('exam_students' , $this->lang->line_or_def('common_student','受講者')              , 'trim|xss_clean|required');
		...
		$check_exam_lectures = $this->input->post('exam_lectures')?$this->input->post('exam_lectures'):array();
		...
		if(!check_array_data_num(array($check_exam_lectures, $check_exam_students, $check_exam_problems))){
			...
			// 個数が1以上、両方の配列個数が同じ場合
			if( (count($temp_exam_remind_time)>0) && (count($temp_exam_remind_time)==count($temp_exam_remind_unit)) ){
				$remind_count      = -1;
				$remind_check_list = '0,';
				foreach($temp_exam_remind_time as $ino => $remind_time){
					...
					if(  ((is_numeric($remind_time)) && ($remind_time>0)) && (($temp_exam_remind_unit[$ino]=='minute') || ($temp_exam_remind_unit[$ino]=='hour') || ($temp_exam_remind_unit[$ino]=='day'))  ){
						...
						if( strpos($remind_check_list,  ','.$remind_time.$temp_exam_remind_unit[$ino].',')!==FALSE ){
							// 重複あり
						}else{
							// 重複なし
							$remind_check_list .= $remind_time.$temp_exam_remind_unit[$ino].',';    // リストに追加
							
							$remind_count = $remind_count + 1;
							$exam_remind_time[$remind_count] = $remind_time;
							$exam_remind_unit[$remind_count] = $temp_exam_remind_unit[$ino];
						}
					}
				}
			}
			...
			if($this->form_validation->run() == FALSE){
				...
				$data['exam']['exam_name']            = '';
				$data['exam']['exam_lectures']        = $this->input->post('exam_lectures')?$this->input->post('exam_lectures'):array();
				$data['exam']['exam_caption']         = '';
				$data['exam']['exam_open']            = '';
				$data['exam']['exam_close']           = '';
				$data['exam']['public_flag']          = '';
				$data['exam']['resubmit_flag']        = '';
				$data['exam']['marking_public_flag']  = '';
				$data['exam']['marking_public_kind']  = '';
				$data['exam']['marking_public_open']  = '';
				$data['exam']['teacher_id']           = $this->input->post('teacher_id');
				$data['exam']['teacher_name']         = '';
				$data['exam']['exam_students']        = $this->input->post('exam_students')?$this->input->post('exam_students'):array();
				$data['exam']['exam_problems']        = $this->input->post('exam_problems')?$this->input->post('exam_problems'):array();
				$data['exam']['exam_remind_time']     = $exam_remind_time;
				$data['exam']['exam_remind_unit']     = $exam_remind_unit;
				$data['exam']['display_format']       = $this->input->post('display_format');
				$data['exam']['criteria_type']        = '';
				$data['exam']['criteria_value']       = '';
				...
				$data['exam']['update_flg']           = $this->input->post('update_flg');
				$data['exam']['exam_id']              = $this->input->post('exam_id');
				$data['exam']['exam_name']            = $this->input->post('exam_name');
				$data['exam']['exam_lectures']        = $this->input->post('exam_lectures')?$this->input->post('exam_lectures'):array();
				$data['exam']['exam_caption']         = $this->input->post('exam_caption');
				$data['exam']['exam_open']            = $this->input->post('exam_open');
				$data['exam']['exam_close']           = $this->input->post('exam_close');
				$data['exam']['public_flag']          = $this->input->post('public_flag');
				$data['exam']['resubmit_flag']        = $this->input->post('resubmit_flag');
				$data['exam']['marking_public_flag']  = $this->input->post('marking_public_flag');
				$data['exam']['marking_public_kind']  = $this->input->post('marking_public_kind');
				$data['exam']['marking_public_open']  = $this->input->post('marking_public_open');
				$data['exam']['school_id']            = $this->libauth->get_school_id();
				$data['exam']['teacher_id']           = $this->input->post('teacher_id');
				$data['exam']['teacher_name']         = '';
				$data['exam']['exam_remind_time']     = $exam_remind_time;
				$data['exam']['exam_remind_unit']     = $exam_remind_unit;
				$data['exam']['display_format']       = $this->input->post('display_format');
				$data['exam']['criteria_type']        = $this->input->post('criteria_type');
				$data['exam']['criteria_value']       = $this->input->post('criteria_value');
				...
				$temp_teacher = $this->model_teacher->get_name(array(
					'teacher_id' => $data['exam']['teacher_id'],
				));
				if($temp_teacher){
					$data['exam']['teacher_name'] = $temp_teacher;
				}
				...
				$data['exam']['exam_students_name'] = '---';
				$this->load->model('model_student');
				if($data['exam']['exam_students']){
					$student_id_list = implode(",", $data['exam']['exam_students']);
				...
				if($data['exam']['exam_problems']){
					foreach($data['exam']['exam_problems'] as $idx => $lecture){
						...
						$data['exam']['exam_problems_answer_point'][$idx] = $data_result['answer_point'];
					}
				}
				...
				$data['exam']['exam_answer_latest_flag'] = array();  // 最新解答フラグ
				$check_student_id = ',-1,';
	...
	function commit(){
		...
		if(isset($edit_form_data['exam_id']) && $edit_form_data['exam_id'] != ""){
			...
			// 試験テーブル更新
			$exam_result = $this->model_exam->update_exam($update_param);
			
			// 試験講座テーブル更新
			$data_param = array(
							'exam_id'  => $exam_result['lastInsertId'],
							'data'     => $update_param['data'],
						);
			$this->model_exam->update_exam_lectures($data_param);
			
			// 試験受講者・試験受講者グループ更新
			$this->model_exam->update_exam_student_group($data_param);
			
			// 試験設問関係テーブル更新
			$this->model_exam->update_rel_exam_problem($data_param);
			


			// 通知登録を行う。
			// 通知機能は未完成のため動作させないように！
		//	$res_remind = $this->exam_queue_remind($data_param);


			//ビュー設定引数設定
			$data = array();
			$disp_param = array(
							'view_name'   => 'cms_exam/commit',
							'submenu_idx' => 3,
							'view_data'   => $data,
						);
			//確認フォーム表示
	...
	function delete_item($exam_id){
		...
		if($auth_exam == 0){
			//戻り先設定
			$data['returnurl']       = site_url('cms_exam');  // site_url('cms_exam').'/detail/'.$exam_id.'/';
			...
			//データ削除
			$data = $this->model_exam->delete_item(array(
				'exam_id' => $exam_id,
			));
		...
		if($auth_exam == 0){
			...
			foreach($temp_exam_answer_data as $exam_answer_data){
				...
				$array_exam_answer_data[$temp[1]] = array(
					'exam_answer_mark'  => $temp[2],     // 正誤
					'exam_answer_point' => $temp[3],     // 解答配点
				);
			}
			...
			if(count($db_data) > 0){
				...
				$data['exam']['answer_point_total']      = array();  // 解答配点の合計
				$data['exam']['exam_answer_latest_flag'] = array();  // 最新解答フラグ　設問の解答が最新かどうか　1：最新、0：過去
				$data['exam']['exam_answer_show_flag']   = array();  // 表示フラグ　1：表示、0：非表示（=変更がない）
				// 
				$list_result = $this->model_exam_answer->get_exam_answer_list($exam_param);
				//$exam_answer_list_count = $list_result['cnt'];
				$exam_answer_list = $list_result['items'];
				...
				if(isset($exam_answer_list)){
					foreach($exam_answer_list as $exam_answer){
						...
						$data['exam']['answer_no'][$idx]              = $exam_answer['answer_no'];
						$data['exam']['answer_point_total'][$idx]     = $exam_answer['answer_point_total'];
						$data['exam']['exam_answer_show_flag'][$idx]  = 1;
						...
						$param = array(
							'exam_id'         => $data_param['exam_id'],
							'student_id'      => $exam_answer['answer_student_id'],
							'exam_answer_no'  => $exam_answer['answer_no'],
						);
						$detail_result = $this->model_exam_answer->get_exam_answer_detail($param);
						//$exam_answer_detail_count = $detail_result['cnt'];
						$exam_answer_detail = $detail_result['items'];
	/*
				...
				//	      ,exam_problem.problem_kind          -- 設問種類  1:テキスト、2:動画、3:図書室
				//	      ,exam_problem.problem_contents      -- 設問内容
					      ,exam_problem.answer_kind           -- 解答種類  1:単一形式、2:複数形式、3:フリー回答
					      ,exam_problem.answer_contents       -- 解答内容 JSON型
					      ,exam_problem.answer_point          -- 解答配点
	*/
						$idx_detail = -1;
						$new_answer_point_total = 0;  // POST側 解答配点合計
						$update_flag            = 0;  // 解答結果・解答配点変更フラグ（1：変更あり）
						if(isset($exam_answer_detail)){
							foreach($exam_answer_detail as $exam_answer){
								// POSTされた解答配点の合計を取得
							//	$new_answer_point_total = $new_answer_point_total + $array_exam_answer_data[ $exam_answer['exam_answer_id'] ]['exam_answer_point'];
								
								$new_answer_point_total = $new_answer_point_total;
								if( isset($array_exam_answer_data[ $exam_answer['exam_answer_id'] ]) ){
									$new_answer_point_total = $new_answer_point_total + $array_exam_answer_data[ $exam_answer['exam_answer_id'] ]['exam_answer_point'];
								}
								...
								// 解答結果の確認
								$new_exam_answer_mark = -1;
							//	if( $array_exam_answer_data[ $exam_answer['exam_answer_id'] ]['exam_answer_mark'] != $exam_answer['exam_answer_mark']){
							//		$new_exam_answer_mark = $array_exam_answer_data[ $exam_answer['exam_answer_id'] ]['exam_answer_mark'];
							//		$update_flag = 1;
							//	}
								if( isset($array_exam_answer_data[ $exam_answer['exam_answer_id'] ]) ){
									if( $array_exam_answer_data[ $exam_answer['exam_answer_id'] ]['exam_answer_mark'] != $exam_answer['exam_answer_mark']){
								...
								// 解答配点の確認
								$new_exam_answer_point = -1;
							//	if( $array_exam_answer_data[ $exam_answer['exam_answer_id'] ]['exam_answer_point'] != $exam_answer['exam_answer_point']){
							//		$new_exam_answer_point = $array_exam_answer_data[ $exam_answer['exam_answer_id'] ]['exam_answer_point'];
							//		$update_flag = 1;
							//	}
								if( isset($array_exam_answer_data[ $exam_answer['exam_answer_id'] ]) ){
									if( $array_exam_answer_data[ $exam_answer['exam_answer_id'] ]['exam_answer_point'] != $exam_answer['exam_answer_point']){
								...
								if( ($new_exam_answer_mark == -1) && ($new_exam_answer_point == -1) ){
									continue;
								}
								...
								// 解答ID
								$data['exam_answer'][$idx]['exam_answer_id'][$idx_detail] = $exam_answer['exam_answer_id'];
								
								// 解答種類
								$data['exam_answer'][$idx]['answer_kind'][$idx_detail] = $exam_answer['answer_kind'];
								
								// 解答内容 --- 解答種類が単一・複数の場合→設問テーブルより内容取得
								//              解答種類がテキストの場合→解答テーブルより内容取得
								$temp ='';
								if( ($exam_answer['answer_kind']==1) || ($exam_answer['answer_kind']==2) ){
									$array_exam_answer_contents = explode(',', $exam_answer['exam_answer_contents']);      // 受講者が選択した番号
									$array_answer_contents      = obj2arr(json_decode( $exam_answer['answer_contents'] )); // JSON型、選択した番号の文字列
								...
								// 解答結果（ID）
								$data['exam_answer'][$idx]['exam_answer_mark'][$idx_detail] = $exam_answer['exam_answer_mark'];
								
								// 解答配点（解答結果による配点）
								$data['exam_answer'][$idx]['exam_answer_point'][$idx_detail] = $exam_answer['exam_answer_point'];
								
								// POST側 解答結果（変更なし[-1]の場合、DB側解答結果を格納）
								$data['exam_answer'][$idx]['new_exam_answer_mark'][$idx_detail] = $new_exam_answer_mark;
								if($new_exam_answer_mark==-1){
									$data['exam_answer'][$idx]['new_exam_answer_mark'][$idx_detail] = $exam_answer['exam_answer_mark'];
								}
								
								// POST側 解答配点（変更なし[-1]の場合、DB側解答配点を格納）
								$data['exam_answer'][$idx]['new_exam_answer_point'][$idx_detail] = $new_exam_answer_point;
								if($new_exam_answer_point==-1){
									$data['exam_answer'][$idx]['new_exam_answer_point'][$idx_detail] = $exam_answer['exam_answer_point'];
								}
								
								// 解答配点（設問に設定されている配点）
								$data['exam_answer'][$idx]['answer_point'][$idx_detail] = $exam_answer['answer_point'];
								
							}
							
							// POST側 解答配点合計
							$data['exam']['new_answer_point_total'][$idx]  = $new_answer_point_total;
							
							if($update_flag == 0){
								$data['exam']['exam_answer_show_flag'][$idx] = 0;
							}
						}
					}
				}
	...
	//exit();
				...
				//ビュー設定引数設定
				$disp_param = array(
								'view_name'   => 'cms_exam/confirm_answer',
								'submenu_idx' => 3,
								'view_data'   => $data,
							);
				//確認フォーム表示
	...
	// 解答修正登録（登録完了画面）
	//----------------------------------------------
	function exam_answer_update_commit(){
		...
		$arr_exam_answer_id = array();

		foreach($array_exam_answer_data as $index => $value){
			$update_param = array(
							'exam_id'            => $exam_id,
							'exam_answer_id'     => $index,
							'exam_answer_mark'   => $value['exam_answer_mark'],
							'exam_answer_point'  => $value['exam_answer_point'],
							'marked_teacher_id'  => $this->libauth->get_teacher_id(),  // 注意：課題管理者ではなく、ログイン講師IDを格納
						);
			
			// 解答テーブルの更新
			$result = $this->model_exam_answer->update_exam_answer($update_param);

			//更新があったexam_answer_idを配列にとっておく(guidance用)
			$arr_exam_answer_id[] = $index;
		}

		//コースで解答済みのログがある場合のみ、合計点をguidance_historyに追加する。
		$this->load->model('model_guidance');
		$this->model_guidance->update_guidance_history_exam($exam_id,$arr_exam_answer_id);

		//完了フォーム表示
		$this->_display_view(array(
			'view_name'   => 'cms_exam/commit_answer',
			'submenu_idx' => 4,
			'view_data'   => array(),
			//'class_id'    => $this->input->post('class_id'),
		));
	...
	function _display_view($param) {
		...
		//設問リスト用一覧取得（edit）
		$this->load->model('model_exam_problem');
		$param['view_data']['exam_problems_list'] = $this->model_exam_problem->get_exam_problem_ul_list($drop_param);
		
		// 解答結果ドロップダウン用データ取得（confirm）
		$param['view_data']['exam_answer_mark_list']  = $this->_get_exam_answer_mark_list();
	...
	// 解答結果、ドロップダウン用データ取得
	//----------------------------------------------
	function _get_exam_answer_mark_list(){
		$this->lang->load('common');
		
		$data['exam_answer_mark'] = array();
		$data['exam_answer_mark'][0] = $this->lang->line_or_def('common_non_correct_answer', '不正解');
		$data['exam_answer_mark'][1] = $this->lang->line_or_def('common_correct_answer', '正解');
		
		return $data['exam_answer_mark'];
	}
	...
	//  別窓にて設問の表示イメージを表示する
	//----------------------------------------------
	function preview_exam_problem($exam_problem_id = 0){
		// load language
		$this->lang->load('common');
		...
		//データ取得
		$db_data = $this->model_exam_problem->get_exam_problem($data_param);
		
		if(count($db_data) > 0){
			// 該当ありの場合
			
			// 設問名の取得
			$exam_problem_name = '[No'.$db_data['exam_problem_id'].'] '.$db_data['exam_problem_name'];
			
			// 設問内容の取得 ---------- ---------- ---------- ---------- ---------- ---------- ---------- ----------
			$problem_contents_title = $this->lang->line_or_def('common_exam_problem','設問');
			...
			// 解答内容の取得 ---------- ---------- ---------- ---------- ---------- ---------- ---------- ----------
			$problem_answer_title = $this->lang->line_or_def('common_exam_answer','解答');
			...
			if( ($db_data['answer_kind'] == 1) || ($db_data['answer_kind'] == 2) ){
				...
				// 3:フリー解答
				$problem_answer_title .= '<br/>['.$this->lang->line_or_def('common_free_exam_answer', 'フリー解答').']';
			...
			//※該当なしの場合
			$exam_problem_name = $this->lang->line_or_def('common_nothing', 'なし');
		...
		$html_data  = '';
		$html_data .= '<html><head>';
		$html_data .= '<title>'.$exam_problem_name.'</title>';
		...
		$html_data .= '<div class="problem_contents_title">'.$this->lang->line_or_def('common_exam_problem_name','設問名').'</div>';
		$html_data .= '<div class="problem_contents_value">'.$exam_problem_name.'</div>';
		...
		$html_data .= '<div style="width:100%;"><hr style="border-top: 1px solid #bbb;"></div>';
		$html_data .= '<div style="display: inline-block;">';
		$html_data .= '<div class="problem_contents_title">'.$this->lang->line_or_def('common_exam_answer_points','解答配点').'</div>';
	...
	//----------------------------------------------
	function answer_csv_download_summary($exam_id = 0, $latest = 0){
		//モデル読み込み
		$this->load->model('model_exam');
		$this->load->model('model_exam_answer');
...
// CSVファイル名の作成 ---------------------------------------------------------------
		...
		if(count($db_data) > 0){
			...
			$post_csv_filename .= '_'.convert_FileName($db_data['exam_name']);
			
		//	$post_csv_filename .= '_'.date('Ymd_His', strtotime($db_data['exam_open']));   // 問題提出期限（前）
		//	$post_csv_filename .= '_'.date('Ymd_His', strtotime($db_data['exam_close']));  // 問題提出期限（後）
		}
		if($latest == 0){
...
//		$csv_header = array('解答日時', '解答回数', '最新解答', '受講者ID', '受講者名', '取得配点', '最大配点');
		$csv_header    = array();
		$csv_header[0] = $this->lang->line_or_def('common_exam_answer_datetime',   '解答日時');
		$csv_header[1] = $this->lang->line_or_def('common_exam_answer_no',         '解答回数');
		$csv_header[2] = $this->lang->line_or_def('common_latest_exam_answer',     '最新解答');
		$csv_header[3] = $this->lang->line_or_def('common_student_id',             '受講者ID');
		$csv_header[4] = $this->lang->line_or_def('common_student_name',           '受講者名');
		$csv_header[5] = $this->lang->line_or_def('common_get_exam_answer_points', '取得配点');
		$csv_header[6] = $this->lang->line_or_def('common_max_exam_answer_points', '最大配点');
		
		// CSV２行目以降、データ取得
		$list_result = $this->model_exam_answer->get_exam_answer_list($data_param);
	...
	//   $latest をオンにすることにより最新解答のみを対象とする
	//----------------------------------------------
	function answer_csv_download_detail($exam_id = 0, $latest = 0){
		//モデル読み込み
		$this->load->model('model_exam');
		$this->load->model('model_exam_answer');
...
// CSVファイル名の作成 ---------------------------------------------------------------
	...
	//	$csv_header = array('解答日時', '解答回数', '最新解答', '受講者ID', '受講者名', '設問ID', '設問名', '設問種類', '設問内容', '解答種類', '解答内容', '解答結果', '解答配点', '正解配点');
		...
		$csv_header[3]  = $this->lang->line_or_def('common_student_id',            '受講者ID');
		$csv_header[4]  = $this->lang->line_or_def('common_student_name',          '受講者名');
		$csv_header[5]  = $this->lang->line_or_def('common_exam_problem_id',       '設問ID');
		$csv_header[6]  = $this->lang->line_or_def('common_exam_problem_name',     '設問名');
		...
		$csv_header[11] = $this->lang->line_or_def('common_exam_answer_result',    '解答結果');
		$csv_header[12] = $this->lang->line_or_def('common_exam_answer_points',    '解答配点');
		...
		foreach($table_data as $table_data_record){
			...
			$param = array(
				...
				'exam_answer_no'  => $table_data_record['answer_no'],
			);
			$detail_result = $this->model_exam_answer->get_exam_answer_detail($param);
			//$exam_answer_detail_count = $detail_result['cnt'];
			$exam_answer_detail = $detail_result['items'];
...
*/
		//	$idx_detail = -1;
			if(isset($exam_answer_detail)){
				...
				//	$idx_detail++;
					...
					// 設問ID
					$csv_data_detail[5] = $exam_answer['exam_problem_id'];
					
					// 設問名
					$csv_data_detail[6] = $exam_answer['exam_problem_name'];
					
					// 設問種類（1:テキスト、2:動画、3:図書室）
					if($exam_answer['problem_kind']==1) $csv_data_detail[7] = $this->lang->line_or_def('common_text'         , 'テキスト');
					if($exam_answer['problem_kind']==2) $csv_data_detail[7] = $this->lang->line_or_def('common_video'        , 'ビデオ');
					if($exam_answer['problem_kind']==3) $csv_data_detail[7] = $this->lang->line_or_def('common_book_library' , '図書室');

					// 設問内容
					$temp = '';
					if($exam_answer['problem_kind'] == 1){
						$temp = $exam_answer['problem_contents'];
					}elseif($exam_answer['problem_kind'] == 2){
						$this->load->model('model_video');
						$temp_id = $exam_answer['problem_contents'];
						$db_data = $this->model_video->get_material(array('video_id' => $temp_id));
						$temp = "[".$temp_id."]".$db_data['video_logic_name'];
					}elseif($exam_answer['problem_kind'] == 3){
						$this->load->model('model_book_library');
						$temp_id = $exam_answer['problem_contents'];
					...
					if($exam_answer['answer_kind']==1) $csv_data_detail[9] = $this->lang->line_or_def('common_single_forms' , '単一形式');
					if($exam_answer['answer_kind']==2) $csv_data_detail[9] = $this->lang->line_or_def('common_plural_forms' , '複数形式');
					if($exam_answer['answer_kind']==3) $csv_data_detail[9] = $this->lang->line_or_def('common_free_exam_answer'  , 'フリー解答');
					...
					// 解答結果（0:不正解 1:正解）
					if($exam_answer['exam_answer_mark']==0) $csv_data_detail[11] = $this->lang->line_or_def('common_non_correct_answer', '不正解');
					if($exam_answer['exam_answer_mark']==1) $csv_data_detail[11] = $this->lang->line_or_def('common_correct_answer'  , '正解');

					// 取得配点（解答結果による配点）
					$csv_data_detail[12]= $exam_answer['exam_answer_point'];

					// 解答配点（設問に設定されている配点）
					$csv_data_detail[13] = $exam_answer['answer_point'];
	...
	// 問題（テスト） - 通知対応
	//----------------------------------------------
	function exam_queue_remind($param) {
		//引数設定
		$param = array_merge(
						array(
							'exam_id' => 0,
		...
		$this->model_queue->update_queue_deleted_type_kind_status(array(
			'queue_type'     => 'exam',
			'queue_type_id'  => $param['exam_id'],
			'queue_kind'     => 'mail',
			'queue_status'   => 'WAITING',
		));

		// 通知時間がある場合のみ、実行
		if( $data['exam_remind_time'] ){
			
			// 通知内URLの構築（開発・ステは常にSSLなし。本番の場合は管理画面URLから判定）
			//   'development' 'testing' 'production'
			$show_url = $this->config->item('domain_follower').'/exam/index/'.$param['exam_id'];
			...
			// 通知時間のループ
			foreach($data['exam_remind_time'] as $ino => $remind_time){
				
				// 送信日時の検出	'Y-m-d H:i:s'
				$string_sub = " -1 hour";
				$show_time  = "1".$this->lang->line_or_def('common_hours_ago','時間前');
				switch ( $data['exam_remind_unit'][$ino] ){
				case 'month':
				...
				//$queue_execute_time = date('Y-m-d H:i:s', strtotime($data['exam_close'] . $string_sub));
				$this->load->helper('string_datetime_helper');
				$queue_execute_time = get_search_datetime($data['exam_close'], $string_sub);

				// 問題（テスト）終了日時の修正
				//$this->load->helper('string_datetime_helper');
				$format  = $this->lang->line_or_def('common_format_year_month_day_week' , '{YEAR}年{MONTH}月{DAY}日（{WEEK}）')."{HOUR}:{MINUTE}:{SECOND}";
				$args_show_close_date = get_format_datetime($data['exam_close'], $format);
				
				// 種類詳細の作成（JSON型）
				$queue_kind_detail_array = array();
				$queue_kind_detail_array['exam_id']                = $param['exam_id'];
				$queue_kind_detail_array['mail_format']            = "exam_reminder_before";
				...
				$queue_kind_detail_array['args']['UNIT']           = $data['exam_remind_unit'][$ino];
	...
	public function check_required_checkbox2($input) {
		...
		if (!is_array($input) || count($input) == 0) {
			$this->form_validation->set_message('check_required_checkbox2', '設問 欄は必須です。');
			return false;
		}
		return true;
	}
