	...
	function cms_user($offset=0){
	  ...
	  //$this->form_validation->set_rules('s_student_group'   , $this->lang->line_or_def('common_group','グループ')                             , 'trim|xss_clean');
		...
		if ( !$this->input->post() ){
			...
			$data['s_sub_auth_ethic_training_on']	= intval(strip_tags($this->input->post('s_sub_auth_ethic_training_on', TRUE) ?? 0));
			$data['s_sub_auth_ethic_training_off']	= intval(strip_tags($this->input->post('s_sub_auth_ethic_training_off', TRUE) ?? 0));
