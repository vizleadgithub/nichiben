...
class Cms_student extends CI_Controller {
	...
	function index($offset=0){
		...
		if ( !$this->input->post() ){
			...
			$data['s_lawyer_number']		= ($this->input->post('s_lawyer_number', TRUE) ?? '');
			$data['s_lawyer_division']		= ($this->input->post('s_lawyer_division', TRUE) ?? '');
			$data['s_email']			= ($this->input->post('s_email', TRUE) ?? '');
			$data['s_bar_association']		= ($this->input->post('s_bar_association', TRUE) ?? '');
			$data['s_free_word']			= ($this->input->post('s_free_word', TRUE) ?? '');
			$data['s_sub_auth_ethic_training_on']	= ($this->input->post('s_sub_auth_ethic_training_on', TRUE) ?? 0);
			$data['s_sub_auth_ethic_training_off']	= ($this->input->post('s_sub_auth_ethic_training_off', TRUE) ?? 0);
			//$data['s_id']				= FALSE;
			//$data['s_cource']			= FALSE;
