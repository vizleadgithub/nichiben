...
class Bat_report_oneoff extends CI_Controller{
	private $nowTime;
	...
	//======================================================================//
	// [2014-04-03] 注意
	//    本バッチは、スマコネ配信サーバのログ設置不具合によるデータ取得エラーに対応するための
	//    ソースであり、常駐バッチではないので注意！
	//======================================================================//
...
//			}
	...
	//- ビデオ授業レポート（日単位）
	//〓  サーバメンテによるサーバ側不具合対応
	//    配信サーバの情報がメンテ後に設置されてなかったため、特定日付の traffic・strage をreport_video テーブルに反映する処理
	//    日付範囲としては、2014/03/19～2014/04/01  （2014/04/02 時点で配信サーバ情報が設置されたため、2014/04/03 定時バッチに2014/04/03が格納されるはず）
	//----------------------------------------------------------------------//
	public function daily_video_oneoff(){

		print "[".date('Y/m/d H:i:s')."] daily_video_oneoff START\n";

		$schools = $this->_getSchools();
		foreach($schools as $school){

print "[".date('Y/m/d H:i:s')."] school_id : ".$school['school_id']." \n";
...
//			}
			...
			//レポート取得日付
			//〓 日付を、2014-03-19 に固定指定。2014-03-19 以降の情報がないため。
			$createReportDate = '2014-03-19';
			//$createReportDate = $this->_getNextReportDate(array(
			//  'school_id'	=> $school['school_id'],
			...
			//見つかったのが今日だったら終了
			//〓 システム日付も固定指定のため、処理不要
			$this_nowTime = strtotime('2014-04-02');
			//if($createReportDate == date('Y-m-d', $this->nowTime)){
			//  return;
			//}
//print $createReportDate."@@@".$this_nowTime."\n";
//exit();
			//月毎
			...
			while(date('Y-m', strtotime($_getReportDate)) != date("Y-m", strtotime(date("Y-m-01", $this_nowTime) . "+1 month"))){
				$reports = $this->_getStreamReport(array(
					'school'	=> $school,
					'ym'		=> date('Y-m',strtotime($_getReportDate)),
				));

//print var_dump($reports);
//exit();
...
//print "[".date('Y/m/d H:i:s')."][school_id:".$school['school_id']."] ".$_log->day." / ".$_log->traffic." / ".$_log->storage." \n";
				...
				//  if($_log->day == $createReportDate){
					...
					// 〓 最新の traffic・strage は通常バッチにより反映しているため、本バッチでは処理しない
					...
					if(strtotime("$_log->day 00:00:00") >= strtotime(date('Y-m-d 00:00:00', $this_nowTime))){	//今日以降は履歴じゃないので取らない
						continue;
					}
					if($_start){
						// 〓
						// 日付毎の traffic・strage を記載する。
						// 今回の場合、すでにレコードがあるため、INSERTではなく、UPDATEとする
						...
						$res_strage = $this->db->query($this->db->update_string('report_video', array(
								'value'		=> ($_log->storage ? $_log->storage : 0),
							),'school_id='.$school['school_id']." AND type='strage' AND date = '".$_log->day."' "
						));

					//	$insert_stream = $this->db->insert_string('report_video_batch_test', array(
					//		'school_id'	=> $school['school_id'],
					//		'type'		=> 'stream',
					//		'date'		=> $_log->day,
					//		'value'		=> ($_log->traffic ? $_log->traffic : 0),
					//	));
					//	$insert_stream = str_replace('INSERT INTO','INSERT IGNORE INTO',$insert_stream);	// INSERT時、重複エラーの場合、無視するSQL文に変更
					//	$this->db->query($insert_stream);
						
					//	$insert_strage = $this->db->insert_string('report_video_batch_test', array(
					//		'school_id'	=> $school['school_id'],
					//		'type'		=> 'strage',
					//		'date'		=> $_log->day,
					//		'value'		=> ($_log->storage ? $_log->storage : 0),
					//	));
					//	$insert_strage = str_replace('INSERT INTO','INSERT IGNORE INTO',$insert_strage);	// INSERT時、重複エラーの場合、無視するSQL文に変更
					//	$this->db->query($insert_strage);
		...
		print "[".date('Y/m/d H:i:s')."] daily_video_oneoff END\n";
	}
