...
//var_dump($product_ids2);
		...
		if( count($exam_ids)>0 ){
			...
			for($i1=0;$i1<count($exam_ids);$i1++){ 
				...
				$sql.= "".$this->db->escape($exam_ids[$i1]["exam2_id"])."";
			}
			$sql.= ") ";
			//--------------------------------------
			if( count($product_ids1)>0 ){
				...
				for($i1=0;$i1<count($product_ids1);$i1++){ 
					...
					$sql.= "".$this->db->escape($product_ids1[$i1])."";
				}
				$sql.= ") ";
			}
			//--------------------------------------
			if( count($product_ids2)>0 ){
				...
				for($i1=0;$i1<count($product_ids2);$i1++){ 
					...
					$sql.= "".$this->db->escape($product_ids2[$i1])."";
				}
				$sql.= ") ";
			}
			//--------------------------------------
...
//var_dump($arr_csv);
			...
			for($i1=0;$i1<count($arr_csv);$i1++){ 
				...
				if ($query->num_rows() > 0){
					...
					$sql = "SELECT student_id,product_id FROM exam2_answer WHERE exam2_id=".$arr_csv[$i1]["exam2_id"]." AND product_id=".$this->db->escape($arr_csv[$i1]["product_id"])." GROUP BY student_id,product_id ORDER BY student_id ASC ";
					$query = $this->db->query($sql);
					if ($query->num_rows() > 0){
						...
						for($i2=0;$i2<count($arr_csv[$i1]["exam2"][0]["student"]);$i2++){ 
							...
							$sql = "SELECT ";
							$sql.= " exam2_problem_id, student_id, max( exam2_answer_no ) as mean , exam2_answer_contents , exam2_answer_contents_old ";
							$sql.= "FROM ";
							$sql.= " exam2_answer ";
							$sql.= "WHERE ";
		...
		if ($query->num_rows() > 0){
			...
			if ($query->num_rows() > 0){
				...
				for($i2=0;$i2<count($arr_exam2[0]["student"]);$i2++){ 
					...
					$sql.= " AND exam2_answer.product_id=".$param["product_id"]." ";
