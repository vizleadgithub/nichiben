...
//	}
	...
	function get_class_tantou_list($param) {
		...
		$sql_limit  = " LIMIT {$this->db->escape_str($param['offset'])}, {$this->db->escape_str($param['rowcount'])}";
...
//		$sql_order = " ORDER BY class.class_id";
	...
	// [ver2.0]授業講師テーブル取得
	//----------------------------------------------
	function get_class_teacher($param){
		//引数設定
		$param = array_merge(
						array(
							'class_id' => 0,
		...
		$query = $this->db->query("
									SELECT
										  rel_class_teacher.class_id    AS class_id
										, rel_class_teacher.teacher_id  AS teacher_id
										, rel_class_teacher.is_creator  AS is_creator
										, teacher.teacher_name          AS teacher_name
									FROM
										rel_class_teacher INNER JOIN teacher ON rel_class_teacher.teacher_id = teacher.teacher_id
									WHERE
										rel_class_teacher.class_id = {$this->db->escape($param['class_id'])}
									ORDER BY
										rel_class_teacher.teacher_id ASC
								");
	...
	// [ver2.0]授業講師テーブル更新
	//----------------------------------------------
	function update_class_teacher($param){
		//引数設定
		$param = array_merge(
						array(
							'data' => array(),
	...
	//	$data = $param['data'];
		...
		// class_id を条件にレコード削除
		$this->db->query("
						DELETE FROM rel_class_teacher
						WHERE
							class_id = {$this->db->escape($data['class_id'])}
						");
		
		// sub_teacher_id 配列に teacher_idを追加
		$all_teacher_id   = $data['sub_teacher_id'];
		$all_teacher_id[] = $data['teacher_id'];

		// teacher_id 分、レコード新規登録
		foreach($all_teacher_id as $teacher_id) {
			$is_creator = 0;
	...
	//  担当する授業であるかの確認
	//----------------------------------------------
	function check_class_teacher($param){
		//引数設定
		$param = array_merge(
						array(
							'class_id'   => 0,
		...
		$query = $this->db->query("
									...
									WHERE
										rel_class_teacher.class_id = {$this->db->escape($param['class_id'])}
									AND
										rel_class_teacher.teacher_id = {$this->db->escape($param['teacher_id'])}
								");
