...
class Model_data_import extends CI_Model
{
	//----------------------------------------------
	// コンテンツの最大登録数(max 10)
	//----------------------------------------------
	...
	function import_delete() {
		...
		// 商品
		$this->db->query("TRUNCATE TABLE tbl_product");
		$this->db->query("TRUNCATE TABLE tbl_product_add");
		$this->db->query("TRUNCATE TABLE tbl_product_elearning");
		$this->db->query("TRUNCATE TABLE tbl_product_live_training");
		$this->db->query("TRUNCATE TABLE tbl_product_ethic_training");
		$this->db->query("TRUNCATE TABLE tbl_product_passport");

		// 商品＆弁護士会とのリレーション
		$this->db->query("TRUNCATE TABLE rel_product_bar_association");
		$this->db->query("TRUNCATE TABLE rel_product_bar_association_branch");

		// 動画
		$this->db->query("TRUNCATE TABLE video");
		$this->db->query("TRUNCATE TABLE video_alfstream_status");
		$this->db->query("TRUNCATE TABLE video_lecture");
		$this->db->query("TRUNCATE TABLE video_search_index");
		$this->db->query("TRUNCATE TABLE video_alfstream_reading_history");

		// 論理研修
		$this->db->query("TRUNCATE TABLE tbl_ethic_group");
		$this->db->query("TRUNCATE TABLE tbl_ethic_question");
		$this->db->query("TRUNCATE TABLE tbl_ethic_branch");
		$this->db->query("TRUNCATE TABLE tbl_ethic_question_history");
		...
		$this->db->query("TRUNCATE TABLE tbl_order_detail");
		$this->db->query("TRUNCATE TABLE tbl_order_no");
		$this->db->query("TRUNCATE TABLE tbl_order_temp");

		// ユーザ
		// $this->db->query("TRUNCATE TABLE student");

		// お気に入り登録
		$this->db->query("TRUNCATE TABLE tbl_favorite");

		// メールマガジン
		$this->db->query("TRUNCATE TABLE tbl_mailmagazine");
		$this->db->query("TRUNCATE TABLE tbl_mailmagazine_sended");

		// その他
		$this->db->query("TRUNCATE TABLE tbl_status_change_ethic");
		$this->db->query("TRUNCATE TABLE tbl_status_change_payment");
	...
	function import_ms_course($file_path, $all_del = 0) {
		...
		while(($values = @fgetcsv($fpr, 0, "\t")) !== false) {
			...
			*/
			...
			// 登録処理
			if ($query->num_rows() == 0) {
				$res = $this->db->query($this->db->insert_string('tbl_product', $param));
				if ($res) {
			...
			// 更新処理
				...
				$res = $this->db->query($this->db->update_string('tbl_product', $param, 'product_id='.$product_id));
				if ($res) {
					$update_cnt++;
			...
			if ($res && $product_id) {
				...
				$this->db->query("DELETE FROM tbl_product_add WHERE product_id = ?", array(
									$product_id
								));

				// INSERT tbl_product_add
				$this->db->query("INSERT INTO tbl_product_add (product_id, product_type_add, all_contents_download, all_contents_download_before) VALUES (?,?,?,?)", array(
									$product_id,
				...
				$this->db->query("DELETE FROM tbl_product_elearning WHERE product_id = ?", array(
									$product_id
								));

				// INSERT tbl_product_elearning
				$this->db->query("INSERT INTO tbl_product_elearning (product_id, product_kind_flg, live_training_product_id, product_flg, search_word) VALUES (?,?,?,?,?)", array(
									$product_id,
			...
			*/
			...
			if (!$parent_flg) {
				...
				if ($res && $product_id) {
					...
					$this->db->query("DELETE FROM tbl_product_live_training WHERE product_id = ?", array(
										$product_id
									));

					// INSERT tbl_product_live_training
					$this->db->query("INSERT INTO tbl_product_live_training (product_id, training_kind_flg, ethic_flg, app_flg, limit_date, memo1, memo2, memo3, memo4, memo5, live_start_date, download_flg, target_flg, sponsor, target) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", array(
										$product_id,
					...
					// DELETE tbl_product_live_training
					$this->db->query("DELETE FROM rel_product_bar_association WHERE product_id = ?", array(
										$product_id
									));
					...
					foreach ($owner1 as $k1 => $v1) {
						...
						$this->db->query("INSERT INTO rel_product_bar_association (product_id, bar_association_id, atype, capacity, hall, receptionist_start_date, receptionist_end_date, contents, update_at, dates, web_flg) VALUES (?,?,?,?,?,?,?,?,?,?,?)", array(
											$product_id,
					...
					$this->db->query("DELETE FROM rel_product_bar_association_branch WHERE product_id = ?", array(
										$product_id
									));
					...
					foreach ($owner1 as $k1 => $v1) {
						...
						foreach ($bar_association_branch_id as $k2 => $v2) {
							...
							// INSERT rel_product_bar_association
							$this->db->query("INSERT INTO rel_product_bar_association_branch (product_id, bar_association_branch_id, capacity, hall, receptionist_start_date, receptionist_end_date, contents, update_at, dates, web_flg) VALUES (?,?,?,?,?,?,?,?,?,?)", array(
												$product_id,
				...
				/*
				// 親がtbl_productマスタに存在するかどうかのチェック
				$query = $this->db->query(
					' SELECT * FROM tbl_product'.
					' WHERE product_code = ?',
					array(
				...
				foreach ($bar_association_branch_id as $k2 => $v2) {
					...
					// 親がrel_product_bar_associationに存在するかどうかのチェック
					$query = $this->db->query(
						' SELECT * FROM rel_product_bar_association_branch'.
						' WHERE product_id = ?'.
					...
					// 更新処理
						...
						$this->db->query("UPDATE rel_product_bar_association_branch SET capacity = ?, hall = ?, receptionist_start_date = ?, receptionist_end_date = ?, contents = ?, update_at = ?, dates = ?, web_flg = ? WHERE product_id = ? AND bar_association_branch_id =?", array(
											$param4['capacity'],
											$param4['hall'],
											$param4['receptionist_start_date'],
											$param4['receptionist_end_date'],
	...
	function import_ms_contents($file_path, $all_del = 0) {
		...
		while(($values = @fgetcsv($fpr, 0, "\t")) !== false) {
			...
			*/
			...
			// すでにvideoマスタに存在するかどうかのチェック
			// （DB取込用ID変換履歴テーブルから）
			$query = $this->db->query(
				' SELECT T1.* FROM video T1 INNER JOIN convert_id_list T2 ON (T1.video_id = T2.new_id)'.
			...
			// 登録処理
			if ($query->num_rows() == 0) {
				$res = $this->db->query($this->db->insert_string('video', $param));
				if ($res) {
			...
			// 更新処理
				...
				$res = $this->db->query($this->db->update_string('video', $param, 'video_id='.$video_id));
				if ($res) {
					$update_cnt++;
			...
			if ($res && $video_id) {
				// DELETE video_alfstream_status
				$this->db->query("DELETE FROM video_alfstream_status WHERE video_id = ?", array(
									$video_id
								));

				// INSERT video_alfstream_status
				$param2 = array(
					"video_id" => $video_id,
				...
				$this->db->query($this->db->insert_string('video_alfstream_status', $param2));

				// DELETE video_lecture
				$this->db->query("DELETE FROM video_lecture WHERE video_id = ?", array(
									$video_id
								));
				...
				$this->db->query($this->db->insert_string('video_lecture', $param2));

				// DELETE video_search_index
				$this->db->query("DELETE FROM video_search_index WHERE video_id = ?", array(
									$video_id
								));
				...
				$this->db->query($this->db->insert_string('video_search_index', $param2));
				...
				// （DB取込用ID変換履歴テーブルから）
				$query = $this->db->query(
					' SELECT T1.* FROM tbl_product T1 INNER JOIN convert_id_list T2 ON (T1.product_id = T2.new_id)'.
				...
				if ($query->num_rows() > 0) {
					...
					if ($param4) {
						$res = $this->db->query($this->db->update_string('tbl_product', $param4, 'product_id='.$product_id));
						if (!$res) {
							// LOGの書き出し
							$this->_output_log(__FUNCTION__."_error_3.log", "[error] id:".$values[0]);
					...
					for ($k = 1; $k <= $this->_max_contents; $k++) {
						if ($rows[0]["contents_contents".$k]) {
							$query2 = $this->db->query(
								' SELECT TIME_TO_SEC(alfstream_duration) AS play_time FROM video_alfstream_status'.
								' WHERE video_id = ?',
								array(
					...
					// play_timeの合計値の設定
					$res = $this->db->query($this->db->update_string('tbl_product', array("play_time" => $play_time), 'product_id='.$product_id));
				}
				else {
					// LOGの書き出し
					$this->_output_log(__FUNCTION__."_error_2.log", "[error] id:".$values[0]);
	...
	function import_tmoral_exam($file_path, $type, $all_del = 0) {
		...
		while(($values = @fgetcsv($fpr, 0, "\t")) !== false) {
			...
			*/
			...
			// 並び順
			$query = $this->db->query("SELECT MAX(rank) AS rank FROM tbl_ethic_group", array());
			if ($query->num_rows() > 0) {
				$rows = $query->result_array();
				$rank = $rows[0]['rank'] + 1;
			}
			...
			*/
			...
			// すでにtbl_ethic_groupマスタに存在するかどうかのチェック
			$query = $this->db->query(
				' SELECT * FROM tbl_ethic_group'.
			...
			// 登録処理
			if ($query->num_rows() == 0) {
				$res = $this->db->query($this->db->insert_string('tbl_ethic_group', $param));
				if ($res) {
					...
					// （DB取込用ID変換履歴テーブルから）
					$query2 = $this->db->query(
						' SELECT T1.*, T2.alfstream_duration FROM (video T1 INNER JOIN video_alfstream_status T2 ON (T1.video_id = T2.video_id)) INNER JOIN convert_id_list T3 ON (T2.video_id = T3.new_id)'.
					...
					// 登録処理
					if ($query->num_rows() == 0) {
						$res = $this->db->query($this->db->insert_string('tbl_product', $param2));
						if ($res) {
							$product_id = $this->db->insert_id();
							// $regist_cnt++;
...
echo "product update\n";
						...
						$res = $this->db->query($this->db->update_string('tbl_product', $param2, 'product_id='.$product_id));
						if ($res) {
							// $update_cnt++;
					...
					if ($res && $product_id) {
						...
						// DELETE tbl_product_ethic_training
						$this->db->query("DELETE FROM tbl_product_ethic_training WHERE product_id = ?", array(
											$product_id
										));

						// INSERT tbl_product_elearning
						$this->db->query("INSERT INTO tbl_product_ethic_training (product_id, ethic_group_id, bar_association_year, publish_flg) VALUES (?,?,?,?)", array(
											$product_id,
			...
			if ($ethic_group_id) {
				// 個数
				$query = $this->db->query(
					' SELECT COUNT(*) AS cnt FROM tbl_ethic_question'.
				...
				$query = $this->db->query(
					' SELECT * FROM tbl_ethic_question'.
					' WHERE ethic_group_id = ?'.
					' AND question = ?',
					array(
						$ethic_group_id,
				...
				$ethic_question_id = "";
				if ($query->num_rows() == 0) {
					$res = $this->db->query($this->db->insert_string('tbl_ethic_question', $param2));
					if ($res) {
				...
				// 更新処理
					...
					$res = $this->db->query($this->db->update_string('tbl_ethic_question', $param2, 'ethic_question_id='.$ethic_question_id));
					if ($res) {
						$update_cnt++;
				...
				if ($ethic_question_id) {
					// DELETE tbl_ethic_branch
					$this->db->query("DELETE FROM tbl_ethic_branch WHERE ethic_question_id = ?", array(
										$ethic_question_id
									));
					...
					for ($j = 1; $j <= 5; $j++) {
						...
						*/
						...
						// INSERT tbl_ethic_branch
						$this->db->query("INSERT INTO tbl_ethic_branch (ethic_question_id, question_branch, answer_flg, rank) VALUES (?,?,?,?)", array(
	...
	// 倫理研修解答結果 インポート
	//----------------------------------------------
	function import_tmoral_result($file_path, $type, $all_del = 0) {
		// テーブルのデータを全削除
		if ($all_del) {
			$this->db->query("TRUNCATE TABLE tbl_ethic_question_history");
		}
		...
		while(($values = @fgetcsv($fpr, 0, "\t")) !== false) {
			...
			*/
			...
			// すでにtbl_ethic_question_historyマスタに存在するかどうかのチェック
			$query = $this->db->query(
				' SELECT * FROM tbl_ethic_question_history'.
				' WHERE student_id = ?'.
				' AND product_id = ?',
				array(
					$student_id,
			...
			// 登録処理
			if ($query->num_rows() == 0) {
				$res = $this->db->query($this->db->insert_string('tbl_ethic_question_history', $param));
				if ($res) {
					$regist_cnt++;
			...
			// 更新処理
				...
				$res = $this->db->query($this->db->update_string('tbl_ethic_question_history', $param, 'student_id='.$student_id.' AND product_id='.$product_id));
				if ($res) {
					$update_cnt++;
	...
	function import_video($file_path, $all_del = 0) {
		// テーブルのデータを全削除
		if ($all_del) {
			$this->db->query("UPDATE video SET idkey = NULL");
		}
			...
			*/
			...
			if ($query->num_rows() > 0) {
				...
				foreach ($rows as $k1 => $v1) {
					$res = $this->db->query($this->db->update_string('video', $param, 'video_id='.$v1["video_id"]));
					if ($res) {
						$update_cnt++;
					}
				}
