...
class Model_exam extends CI_Model  
{
	...
	function get_exam_list($param) {
		...
		$query = $this->db->query(
			...
			'       ,exam.criteria_type    AS criteria_type '.
			'       ,exam.criteria_value   AS criteria_value '.
			'       ,teacher.teacher_name  AS teacher_name '.
			'   FROM exam LEFT JOIN teacher ON teacher.teacher_id = exam.teacher_id '.
			'  WHERE exam.school_id = ? '.
			'    AND exam.status <> 9 '.
			($param['s_free_word'] ?
				' AND ('.
				' 	exam.exam_name LIKE '.'"%'.$this->db->escape_like_str($param['s_free_word']).'%"'.
				'	OR exam.exam_id LIKE '.'"%'.$this->db->escape_like_str($param['s_free_word']).'%"'.
				' )'
				: ''
			).
			($param['s_cource'] > 0 ?
				' AND exam.exam_id IN (SELECT exam_id FROM rel_exam_lecture WHERE cource_id = '.$this->db->escape($param['s_cource']).' )'
				: ''
			).
	...
	function get_exam($param){
		...
		//SQL生成
		$query = $this->db->query(
			' SELECT exam.exam_id               AS exam_id '.
			'       ,exam.exam_name             AS exam_name '.
			'       ,exam.exam_caption          AS exam_caption '.
			"       ,DATE_FORMAT(exam.exam_open  , '%Y/%m/%d %H:%i:%s')  AS exam_open ".
			"       ,DATE_FORMAT(exam.exam_close , '%Y/%m/%d %H:%i:%s')  AS exam_close ".
			'       ,exam.public_flag           AS public_flag '.
			'       ,exam.resubmit_flag         AS resubmit_flag '.
			'       ,exam.marking_public_flag   AS marking_public_flag '.
			'       ,exam.marking_public_kind   AS marking_public_kind '.
			"       ,DATE_FORMAT(exam.marking_public_open , '%Y/%m/%d %H:%i:%s')  AS marking_public_open ".
			'       ,exam.school_id             AS school_id '.
			'       ,exam.teacher_id            AS teacher_id '.
			'       ,exam.display_format        AS display_format '.
			'       ,exam.status                AS status '.
			...
			'       ,teacher.teacher_name       AS teacher_name '.
			'   FROM exam LEFT JOIN teacher ON teacher.teacher_id = exam.teacher_id '.
			"  WHERE exam.exam_id = {$this->db->escape($param['exam_id'])} ".
			'    AND exam.status <> 9 '
		);
	...
	function update_exam($param){
		...
		if ($data['update_flg'] == 0){
			...
			$data = array_merge(array(
					'exam_name'            => 'no value',
					'exam_caption'         => '',
					'exam_open'            => date("Y/m/d H:i:s"),
					'exam_close'           => date("Y/m/d H:i:s"),
					'public_flag'          => 0,
					'resubmit_flag'        => 1,
			...
			$res = $this->db->query($this->db->insert_string('exam', array(
				//	'exam_id'              => '',                            /* 問題ID */
					'exam_name'            => $data['exam_name'],            /* 問題名 */
					'exam_caption'         => $data['exam_caption'],         /* 説明 */
					'exam_open'            => $data['exam_open'],            /* 提出期限開始日時 */
					'exam_close'           => $data['exam_close'],           /* 提出期限終了日時 */
			...
			//修正
			$res = $this->db->query($this->db->update_string('exam', array(
					'exam_name'            => $data['exam_name'],
					'exam_caption'         => $data['exam_caption'],
					'exam_open'            => $data['exam_open'],
					'exam_close'           => $data['exam_close'],
	...
	// 問題講座テーブルから講座ID取得
	//----------------------------------------------
	function get_exam_lectures($param){
		//引数設定
		$param = array_merge(
						array(
							'exam_id' => 0,
	...
	// 問題講座テーブルの登録更新処理
	//----------------------------------------------
	function update_exam_lectures($param){
		//引数設定
		$param = array_merge(
						array(
							'exam_id'  => 0,
		...
		//一旦すべて削除
		$sql = "DELETE FROM rel_exam_lecture
				WHERE
					exam_id = ?
				;";
		$this->db->query($sql, 
		...
		//選択受講講座を登録
		foreach($data['exam_lectures'] as $cource_id) {
			$sql = "INSERT INTO
						rel_exam_lecture
					(
						exam_id,
	...
	//----------------------------------------------
	function get_exam_problems($param){
		//引数設定
		$param = array_merge(
						array(
							'exam_id' => 0,
		...
		//SQL投入
		//$query = $this->db->query("
		//	 SELECT rel_exam_problem.exam_id, rel_exam_problem.exam_no, rel_exam_problem.exam_problem_id 
		//	   FROM rel_exam_problem 
		//	        LEFT JOIN exam_problem ON rel_exam_problem.exam_problem_id = exam_problem.exam_problem_id
		//	  WHERE rel_exam_problem.exam_id = {$this->db->escape($param['exam_id'])} 
		//	    AND exam_problem.status      = 0 
		//	    AND rel_exam_problem.exam_problem_id IN (
		//	        SELECT rel_exam_problem_lecture.exam_problem_id
		//	          FROM rel_exam_problem_lecture
		//	         WHERE rel_exam_problem_lecture.cource_id IN (
		//	               SELECT rel_exam_lecture.cource_id
		//	                 FROM rel_exam_lecture
		//	                WHERE rel_exam_lecture.exam_id = {$this->db->escape($param['exam_id'])} 
		//	         )
		//	    )
		//	  ORDER BY rel_exam_problem.exam_id ASC, rel_exam_problem.exam_no ASC 
		//");
		$query = $this->db->query("
			 SELECT  rel_exam_problem.exam_id          AS exam_id
			        ,rel_exam_problem.exam_no          AS exam_no
			        ,rel_exam_problem.exam_problem_id  AS exam_problem_id
	...
	function get_cource_exam($param) {
		...
		if($cource_list!=''){
			$sql .= " AND exam.exam_id IN (
							SELECT rel_exam_lecture.exam_id 
							  FROM rel_exam_lecture 
							 WHERE rel_exam_lecture.cource_id IN (".$cource_list.")
						) ";
		}
		...
		if($student_group_list!=''){
			$sql .= " AND exam.exam_id IN (
							SELECT rel_exam_student_group.exam_id 
							  FROM rel_exam_student_group 
							 WHERE rel_exam_student_group.student_group_id IN (".$student_group_list.")
						) ";
		}
	...
	function update_exam_student_group($param){
		...
		//選択受講講座を登録
		foreach($data['exam_students'] as $student_id) {
	...
	//問題設問関係テーブル更新
	//----------------------------------------------
	function update_rel_exam_problem ($param){
		//引数設定
		$param = array_merge(
						array(
							'exam_id' => 0,
		...
		//一旦すべて削除
		$sql = "DELETE FROM rel_exam_problem WHERE exam_id = ? ";
		$this->db->query($sql, 
							array(
								$param['exam_id']
							));
		...
		//問題設問関係テーブルを登録
		$exam_no_count = 0;
		foreach($data['exam_problems'] as $exam_problem_id) {
			$exam_no_count = $exam_no_count + 1;
			$sql = "INSERT INTO rel_exam_problem (exam_id, exam_no, exam_problem_id, update_at) VALUES (?, ?, ?, ?) ";
			$this->db->query($sql, 
								array(
									$param['exam_id'],
									$exam_no_count,
	...
	function get_exam_student($param){
		...
		//SQL投入
		$query = $this->db->query("
			SELECT rel_exam_student.student_id 
			  FROM rel_exam_student 
			       LEFT JOIN student ON rel_exam_student.student_id = student.student_id 
			 WHERE rel_exam_student.exam_id = {$this->db->escape($param['exam_id'])} 
			   AND student.status = 0
			 ORDER BY rel_exam_student.student_id
		");
	...
	//   ※受講者画面-詳細画面で使用。
	//----------------------------------------------
	function get_student_from_exam($param){
		//引数設定
		$param = array_merge(
						array(
							'student_id'    => '0',
		...
		$query = $this->db->query("
							 SELECT  rel_exam_student.exam_id 
							        ,exam.exam_name 
							   FROM  rel_exam_student LEFT JOIN exam ON rel_exam_student.exam_id = exam.exam_id
							  WHERE  exam.status = 0 
							    AND  rel_exam_student.student_id IN ({$this->db->escape_str($param['student_id'])})
							  GROUP  BY rel_exam_student.exam_id 
							  ORDER  BY rel_exam_student.exam_id desc
						");
	...
	function get_exam_student_confirm($param){
		...
		$query = $this->db->query("
			SELECT rel_exam_student.student_id 
			  FROM rel_exam_student 
			       LEFT JOIN student ON rel_exam_student.student_id = student.student_id 
			 WHERE rel_exam_student.exam_id = {$this->db->escape($param['exam_id'])}".$sql."
			   AND student.status = 0 
			 ORDER BY rel_exam_student.student_id 
		");
