...
class Model_exam_problem extends CI_Model  
{
	//----------------------------------------------
	//プライベート変数宣言
	//----------------------------------------------
	...
	//試験設問一覧取得
	//----------------------------------------------
	function get_exam_problem_list($param) {
		// load language
		...
		$query = $this->db->query(
			...
			'        exam_problem.exam_problem_id          AS exam_problem_id '.
			'       ,exam_problem.exam_problem_name        AS exam_problem_name '.
		//	'       ,exam_problem.problem_kind             AS problem_kind '.              // 設問種類 1:テキスト、2:動画、3:図書室
		//	'       ,exam_problem.problem_contents         AS problem_contents '.          // 設問内容 テキスト、または、ビデオID・図書室ID
		//	'       ,exam_problem.problem_note             AS problem_note '.              // 設問備考
		//	'       ,exam_problem.answer_kind              AS answer_kind '.               // 解答種類 1:単一形式、2:複数形式、3:フリー回答
		//	'       ,exam_problem.answer_contents          AS answer_contents '.           // 解答内容 JSON型
			'       ,exam_problem.answer_point             AS answer_point '.              // 解答配点 
		//	'       ,exam_problem.answer_explain_kind      AS answer_explain_kind '.       // 解答解説種類 1:テキスト、2:動画、3:図書室、9:なし(初期値）
		//	'       ,exam_problem.answer_explain_contents  AS answer_explain_contents '.   // 解答解説内容 テキスト、または、ビデオID・図書室ID
		//	'       ,exam_problem.answer_explain_note`     AS answer_explain_note '.       // 解答解説備考
			'       ,exam_problem.school_id                AS school_id '.
			'       ,exam_problem.teacher_id               AS teacher_id '.
			'       ,exam_problem.status                   AS status '.
			'       ,exam_problem.update_at                AS update_at '.
			'       ,teacher.teacher_name                  AS teacher_name '.
			'   FROM exam_problem LEFT JOIN teacher ON teacher.teacher_id = exam_problem.teacher_id '.
			'  WHERE exam_problem.school_id = ? '.
			'    AND exam_problem.status <> 9 '.
			($param['s_free_word'] ?
				' AND ('.
				'    exam_problem.exam_problem_name         LIKE '.'"%'.$this->db->escape_like_str($param['s_free_word']).'%"'.
				' OR exam_problem.exam_problem_id          LIKE '.'"%'.$this->db->escape_like_str($param['s_free_word']).'%"'.
				' )'
				: ''
			).
			($param['s_cource'] > 0 ?
				' AND exam_problem.exam_problem_id IN (SELECT exam_problem_id FROM rel_exam_problem_lecture WHERE cource_id = '.$this->db->escape($param['s_cource']).' )'
				: ''
			).
			($param['s_exam_problem_group'] > 0 ?
				' AND exam_problem.exam_problem_id IN (SELECT rel_exam_problem_group.exam_problem_id FROM rel_exam_problem_group WHERE rel_exam_problem_group.exam_problem_group_id = '.$this->db->escape($param['s_exam_problem_group']).' )'
				: ''
			).
			...
			' ORDER BY exam_problem.exam_problem_id DESC'.
			' LIMIT ?, ?',
			array(
				(int) $param['school_id'],
				(int) $param['offset'],
	...
	//一件取得
	//----------------------------------------------
	function get_exam_problem($param){
		//引数設定
		$param = array_merge(
						array(
		...
		//SQL生成
		$query = $this->db->query("
			SELECT  exam_problem.exam_problem_id          AS exam_problem_id 
			       ,exam_problem.exam_problem_name        AS exam_problem_name 
			       ,exam_problem.problem_kind             AS problem_kind 
			       ,exam_problem.problem_contents         AS problem_contents 
			       ,exam_problem.problem_note             AS problem_note 
			       ,exam_problem.answer_kind              AS answer_kind 
			       ,exam_problem.answer_contents          AS answer_contents 
			       ,exam_problem.answer_point             AS answer_point 
			       ,exam_problem.answer_explain_kind      AS answer_explain_kind 
			       ,exam_problem.answer_explain_contents  AS answer_explain_contents 
			       ,exam_problem.answer_explain_note      AS answer_explain_note 
			       ,exam_problem.school_id                AS school_id 
			       ,exam_problem.teacher_id               AS teacher_id 
			       ,exam_problem.status                   AS status 
			       ,exam_problem.update_at                AS update_at 
			       ,teacher.teacher_name                  AS teacher_name 
			   FROM exam_problem LEFT JOIN teacher ON teacher.teacher_id = exam_problem.teacher_id 
			  WHERE exam_problem.exam_problem_id = {$this->db->escape($param['exam_problem_id'])} 
			    AND exam_problem.school_id       = {$this->db->escape($param['school_id'])} 
			    AND exam_problem.status <> 9 
		");
	...
	//新規登録・更新処理
	//----------------------------------------------
	function update_exam_problem($param){
		...
		if ($data['update_flg'] == 0){
			...
			$res = $this->db->query($this->db->insert_string('exam_problem', array(
				//	'exam_problem_id'          => '',                                /* 試験設問ID */
					'exam_problem_name'        => $data['exam_problem_name'],        /* 試験設問名 */
			...
			$res = $this->db->query($this->db->update_string('exam_problem', array(
					'exam_problem_name'        => $data['exam_problem_name'],     /* 試験設問名 */
					'problem_kind'             => $data['problem_kind'],          /* 設問種類 */
			...
			// ),'exam_problem_id='.$param['exam_problem_id']." AND exam_name='name' AND update_at = '".$_log->day."' "
			
			return array(
				'lastInsertId'	=> $data['exam_problem_id'],
			);
		}
	}
	...
	function delete_item($param){
		...
		// ),'exam_id='.$param['exam_id']." AND exam_name='name' AND update_at = '".$_log->day."' "
	...
	// 設問講座テーブルから講座ID取得
	//----------------------------------------------
	function get_exam_problem_lectures($param){
		//引数設定
		$param = array_merge(
						array(
							'exam_problem_id' => 0,
		...
		$query = $this->db->query("
									SELECT
										cource_id
									FROM
										rel_exam_problem_lecture
									WHERE
										exam_problem_id = {$this->db->escape($param['exam_problem_id'])}
									ORDER BY
										cource_id
								");
	...
	// 設問講座テーブルの登録更新処理
	//----------------------------------------------
	function update_exam_problem_lectures($param){
		//引数設定
		$param = array_merge(
						array(
							'exam_problem_id'  => 0,
		...
		//一旦すべて削除
		$sql = "DELETE FROM rel_exam_problem_lecture
				WHERE
					exam_problem_id = ?
				;";
		$this->db->query($sql, 
							array(
								$param['exam_problem_id'],
							));
		
		//選択受講講座を登録
		foreach($data['exam_problem_lectures'] as $cource_id) {
			$sql = "INSERT INTO
						rel_exam_problem_lecture
					(
						exam_problem_id,
	...
	// 設問リスト用一覧取得
	//----------------------------------------------
	function get_exam_problem_ul_list($param) {
		//引数設定
		$param = array_merge(
						array(
							'school_id'   => 0,
		...
		// SQL生成
		$sql = "
			 SELECT  exam_problem.exam_problem_id
			...
			/*      ,exam_problem.school_id         */
			        ,exam_problem.teacher_id
			/*      ,exam_problem.status            */
			/*      ,exam_problem.update_at         */
			        ,teacher.teacher_name AS teacher_name 
			   FROM  exam_problem LEFT JOIN teacher ON exam_problem.teacher_id = teacher.teacher_id
			  WHERE  exam_problem.status = 0
			    AND  exam_problem.school_id = {$this->db->escape($param['school_id'])}
		";
		...
		$cource_list = $this->model_teacher->get_teacher_lecture_string();
		if($cource_list!=''){
			$sql .= " AND exam_problem.exam_problem_id IN (
							SELECT rel_exam_problem_lecture.exam_problem_id 
							  FROM rel_exam_problem_lecture 
							 WHERE rel_exam_problem_lecture.cource_id IN (".$this->db->escape_str($cource_list).")
						) ";
		...
		$sql .= "ORDER BY exam_problem.exam_problem_name ASC ";
	...
	// [Ajax用]学校所属の試験を取得
	//----------------------------------------------
	function get_cource_exam($param) {
		//引数設定
		$param = array_merge(
						array(
							'school_id'	=> 0,
		...
		if (isset($param['free_word']) && $param['free_word'] != '') {
			$sql .= "   AND ( ";
			$sql .= "        exam_name    LIKE '%{$this->db->escape_like_str($param['free_word'])}%' ";
			$sql .= "     OR exam_caption LIKE '%{$this->db->escape_like_str($param['free_word'])}%' ";
			$sql .= "   )";
		}
		...
		$cource_list = $this->model_teacher->get_teacher_lecture_string();
		if($cource_list!=''){
			$sql .= " AND exam.exam_id IN (
							SELECT rel_exam_lecture.exam_id 
							  FROM rel_exam_lecture 
							 WHERE rel_exam_lecture.cource_id IN (".$this->db->escape_str($cource_list).")
						) ";
		}
		...
		if($student_group_list!=''){
			$sql .= " AND exam.exam_id IN (
							SELECT rel_exam_student_group.exam_id 
							  FROM rel_exam_student_group 
							 WHERE rel_exam_student_group.student_group_id IN (".$this->db->escape_str($student_group_list).")
						) ";
		}
	...
	// [Ajax用]学校所属の設問を取得
	//----------------------------------------------
	function get_cource_exam_problem($param) {
		//引数設定
		$param = array_merge(
						array(
							'school_id'   => 0,
		...
		//SQL生成
		$sql  = "";
		$sql .= "SELECT exam_problem.exam_problem_id, exam_problem.exam_problem_name, exam_problem.answer_point, teacher.teacher_name ";
		$sql .= "  FROM exam_problem LEFT JOIN teacher ON exam_problem.teacher_id = teacher.teacher_id ";
		$sql .= " WHERE exam_problem.status = 0 ";
		$sql .= "   AND exam_problem.school_id = {$this->db->escape($param['school_id'])} ";
		...
		if (isset($param['cource_flag']) && $param['cource_flag'] != 0 && $param['cource_id'] != '')  {
			...
			$sql .= "   AND exam_problem.exam_problem_id IN (";
			$sql .= "       SELECT rel_exam_problem_lecture.exam_problem_id FROM rel_exam_problem_lecture WHERE rel_exam_problem_lecture.cource_id IN (".$this->db->escape_str($temp2).") ";
			$sql .= "   ) ";
		}

		if (isset($param['free_word']) && $param['free_word'] != '') {
			$sql .= "   AND ( ";
			$sql .= "        exam_problem.exam_problem_name LIKE '%{$this->db->escape_like_str($param['free_word'])}%' ";
			$sql .= "     OR exam_problem.problem_contents  LIKE '%{$this->db->escape_like_str($param['free_word'])}%' ";
			$sql .= "   )";
		}
		...
		$sql .= " ORDER BY exam_problem.exam_problem_id ASC";
		//var_dump($sql);
		$query = $this->db->query($sql);
	...
	//----------------------------------------------
	function get_exam_student_group($param){
		//引数設定
		$param = array_merge(
						array(
							'exam_id' => 0,
		...
		$query = $this->db->query("
									SELECT
										student_group_id
									FROM
										rel_exam_student_group
									WHERE
										exam_id = {$this->db->escape($param['exam_id'])}
									ORDER BY
										student_group_id
								");
	...
	//----------------------------------------------
	function update_exam_student_group($param){
		//引数設定
		$param = array_merge(
						array(
							'exam_id' => 0,
		...
		//一旦すべて削除
		$sql = "DELETE FROM rel_exam_student WHERE exam_id = ? ";
		$this->db->query($sql, 
							array(
								$param['exam_id']
							));
		...
		foreach($data['lecture_students'] as $student_id) {
			$sql = "INSERT INTO rel_exam_student (exam_id, student_id, update_at) VALUES (?, ?, ?) ";
			$this->db->query($sql, 
								array(
									$param['exam_id'],
									$student_id,
		...
		$sql = "DELETE FROM rel_exam_student_group WHERE exam_id = ? ";
		$this->db->query($sql, 
							array(
								$param['exam_id']
							));
		...
		foreach($temp_student_group_id as $student_group_id){
			$sql = "INSERT INTO rel_exam_student_group (exam_id,student_group_id,update_at) VALUES(?, ?, ?) ";
			$this->db->query($sql, 
								array(
									$param['exam_id'],
									$student_group_id,
	...
	//試験IDから、受講者IDを取得（複数）
	//----------------------------------------------
	function get_exam_student($param){
		//引数設定
		$param = array_merge(
						array(
							'exam_id'    => 0,
	...
	//----------------------------------------------
	function get_exam_student_confirm($param){
		//引数設定
		$param = array_merge(
						array(
							'exam_id'    => 0,
		...
		$cource_list = $this->model_teacher->get_teacher_lecture_string();
		if($cource_list!=''){
			$sql .= " AND rel_exam_student.student_id IN (
		...
		$query = $this->db->query("
									...
									FROM
										rel_exam_student
									WHERE
										rel_exam_student.exam_id = {$this->db->escape($param['exam_id'])}".$sql."
									ORDER BY
										rel_exam_student.student_id
								");
	...
	function get_name($param){
		...
		$query = $this->db->query("
			...
			/*      ,exam_problem.update_at         */
			   ...
			   FROM  exam_problem LEFT JOIN teacher ON exam_problem.teacher_id = teacher.teacher_id
			  WHERE  exam_problem.status = 0
			    AND  exam_problem.exam_problem_id = {$this->db->escape($param['exam_problem_id'])}
		");
	...
	// ※設問インポートで使用
	//----------------------------------------------
	function check_exam_problem_book_library($param){
		//引数設定
		$param = array_merge(
						array(
							'book_library_id'       => 0,
							'exam_problem_lectures' => array(),
							'school_id'             => 0,
						),
						$param
					);

		$cource_id_list = implode(",", $param['exam_problem_lectures']);
	...
	// ※設問インポートで使用
	//----------------------------------------------
	function check_exam_problem_video($param){
		//引数設定
		$param = array_merge(
						array(
							'video_id'              => 0,
	 ...
	 * 設問エクスポートで使用
	 */
	function get_exam_problem_export_data($exam_problem_lectures, $exam_problem_groups){
		$sql = ' SELECT exam_problem.* ';
		$sql.= '   FROM exam_problem LEFT JOIN teacher ON teacher.teacher_id = exam_problem.teacher_id ';
		$sql.= '  WHERE exam_problem.school_id = 1 ';
		$sql.= '    AND exam_problem.status <> 9 ';
		if($exam_problem_lectures){
			$in_epl = '';
			foreach($exam_problem_lectures as $val){
				if(is_numeric($val)){
					if($in_epl ==''){
						$in_epl.= $val;
					} else {
			...
			if($in_epl!=''){
				$sql.= ' AND exam_problem.exam_problem_id IN (SELECT exam_problem_id FROM rel_exam_problem_lecture WHERE cource_id IN( '.$this->db->escape_str($in_epl).' ))';
			}
		}
		if($exam_problem_groups){
			$in_epg = '';
			foreach($exam_problem_groups as $val){
				if(is_numeric($val)){
					if($in_epg ==''){
						$in_epg.= $val;
					} else {
			...
			if($in_epg!=''){
				$sql.= ' AND exam_problem.exam_problem_id IN (SELECT rel_exam_problem_group.exam_problem_id FROM rel_exam_problem_group WHERE rel_exam_problem_group.exam_problem_group_id IN( '.$this->db->escape_str($in_epg).' ))';
			}
		}
		$sql.= ' ORDER BY exam_problem.exam_problem_id ASC';
