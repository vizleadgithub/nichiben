				...
				<?=form_open_multipart("cms_video/commit_moviecut")?>
					...
					<table class="form">
						...
						<tr>
							...
							<td>
								[ビデオID:<?= htmlspecialchars( $video['video_id'], ENT_QUOTES, 'UTF-8', false) ?>] <?= htmlspecialchars( $video['video_logic_name'], ENT_QUOTES, 'UTF-8', false) ?> を使用
										...
										if( isset($lecture_cources) ) { 
											foreach( $lecture_cources as $cource ){ ?>
												...
												<input type="checkbox" name="video_lectures[]" id="lectures_<?=$cource['cource_id']?>" value=<?=$cource['cource_id']?>
													...
													?>
													>
												<label for="lectures_<?=$cource['cource_id']?>"><?= htmlspecialchars( $cource['cource_name'], ENT_QUOTES, 'UTF-8', false) ?></label>
												</li>
										<?php 
										}
									}else{ ?>
						...
						<tr><!-- moviecut -->
							<th style="vertical-align:top;">
								<?= $this->lang->line_or_def('common_beginning_to_cut_range','切り出し範囲') ?>
							</th>
							<td>
								<div style="margin-bottom: 5px;">
									<div style="float:left;line-height : 28px;width: 80px;">
										<select style="width: 50px;" id="cut_start_hour" name="cut_start_hour">
											<?php for($temp = 0; $temp <= intval($video['max_hour']); $temp++): ?>
												<?php $selected = ($temp == $video['cut_start_hour'] ? 'selected="selected"' : ''); ?>
												<option <?= $selected; ?> value="<?= sprintf('%02d', $temp); ?>"><?= sprintf('%02d', $temp); ?></option>
											<?php endfor; ?>
										</select>
										<?= $this->lang->line_or_def('common_js_hourText','時') ?>
									</div>
									<div style="float:left;line-height : 28px;width: 80px;">
										<select style="width: 50px;" id="cut_start_minute" name="cut_start_minute">
											<?php for($temp = 0; $temp <= intval($video['max_minute']); $temp++): ?>
												<?php $selected = ($temp == $video['cut_start_minute'] ? 'selected="selected"' : ''); ?>
												<option <?= $selected; ?> value="<?= sprintf('%02d', $temp); ?>"><?= sprintf('%02d', $temp); ?></option>
											<?php endfor; ?>
										</select>
									...
									<div style="float:left;line-height : 28px;width: 80px;">
										<select style="width: 50px;" id="cut_start_second" name="cut_start_second">
											<?php for($temp = 0; $temp <= intval($video['max_second']); $temp++): ?>
												<?php $selected = ($temp == $video['cut_start_second'] ? 'selected="selected"' : ''); ?>
												<option <?= $selected; ?> value="<?= sprintf('%02d', $temp); ?>"><?= sprintf('%02d', $temp); ?></option>
											<?php endfor; ?>
										</select>
									...
									<div style="float:left;line-height : 28px;">
										<?= $this->lang->line_or_def('common_start','開始') ?> 
									</div>

									<div style="float:left;line-height : 28px;margin-left: 50px;">
										<?= $this->lang->line_or_def('common_reproduction_time','再生時間') ?> / <?= htmlspecialchars( $video['video_time'], ENT_QUOTES, 'UTF-8', false) ?>
									</div>
								...
								<div style="margin-bottom: 5px;">
									<div style="float:left;line-height : 28px;width: 80px;">
										<select style="width: 50px;" id="cut_end_hour" name="cut_end_hour">
											<?php for($temp = 0; $temp <= intval($video['max_hour']); $temp++): ?>
												<?php $selected = ($temp == $video['cut_end_hour'] ? 'selected="selected"' : ''); ?>
												<option <?= $selected; ?> value="<?= sprintf('%02d', $temp); ?>"><?= sprintf('%02d', $temp); ?></option>
											<?php endfor; ?>
										</select>
										<?= $this->lang->line_or_def('common_js_hourText','時') ?>
									...
									<div style="float:left;line-height : 28px;width: 80px;">
										<select style="width: 50px;" id="cut_end_minute" name="cut_end_minute">
											<?php for($temp = 0; $temp <= intval($video['max_minute']); $temp++): ?>
												<?php $selected = ($temp == $video['cut_end_minute'] ? 'selected="selected"' : ''); ?>
												<option <?= $selected; ?> value="<?= sprintf('%02d', $temp); ?>"><?= sprintf('%02d', $temp); ?></option>
											<?php endfor; ?>
										</select>
										<?= $this->lang->line_or_def('common_js_minuteText','分') ?>
									...
									<div style="float:left;line-height : 28px;width: 80px;">
										<select style="width: 50px;" id="cut_end_second" name="cut_end_second">
											<?php for($temp = 0; $temp <= intval($video['max_second']); $temp++): ?>
												<?php $selected = ($temp == $video['cut_end_second'] ? 'selected="selected"' : ''); ?>
												<option <?= $selected; ?> value="<?= sprintf('%02d', $temp); ?>"><?= sprintf('%02d', $temp); ?></option>
											<?php endfor; ?>
										</select>
										<?= $this->lang->line_or_def('common_js_secondText','秒') ?>
									...
									<div style="float:left;line-height : 28px;">
										<?= $this->lang->line_or_def('common_end','終了') ?> 
									</div>
								</div>
								<div style="clear : both"></div>
							</td>
