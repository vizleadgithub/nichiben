...
<?php
	...
	<script type="text/javascript">
		...
		//	// Ajax読み込みに変更に伴い不要
		//	$("*[name='exam2_answer_mark[]']").bind("change keyup",function(){
		//		var exam2_answer_mark = $(this).val();
		//		var answer_point     = $(this).next("*[name='answer_point[]']").val();
		//		if(exam2_answer_mark==0){
		//			// 不正解
		//			$(this).parents(".confirm_ul_li_line_dotted").find("*[name='exam2_answer_point[]']").val(0);
		//		}else{
		//			// 正解
		//			$(this).parents(".confirm_ul_li_line_dotted").find("*[name='exam2_answer_point[]']").val(answer_point);
		//		}
		//	});
		...
		function delete_item(id, msg){
			if(window.confirm( msg )){
				location.href = "<?=base_url()?>cms_exam2/delete_item/" + id ;
			}
		}
		//**************************************************
		//詳細確認画面　修正ボタン押下
		...
		function edit_item(){
			...
			$("*[name='exam2_students[]']").map(function() {
				...
				input.setAttribute('name',  'exam2_students[]');
				input.setAttribute('value', $(this).val() );
				form.appendChild(input);
			});
			$("*[name='exam2_problems[]']").map(function() {
				var input = document.createElement('input');
				input.setAttribute('type',  'hidden');
				input.setAttribute('name',  'exam2_problems[]');
				input.setAttribute('value', $(this).val() );
				form.appendChild(input);
			});
		...
		//詳細確認画面　設問詳細表示ボタン押下
		//**************************************************
		function show_exam2_problem_detail(id, exam2_problem_id){
			if( $("#"+id).css("display") == 'none' ){
				...
				// Ajax により都度取得
				$.ajax({
					url: "/cms_exam2_problem/get_exam2_problem_detail",
					type: "POST",
					data: "exam2_problem_id="+exam2_problem_id,
					
					success: function(response) {
						if(response){
							$("#"+id+" ul").remove();
							
							var problem_kind      = "<?= $this->lang->line_or_def('common_exam2_problem','設問'); ?>";
							var problem_kind_link = '';
							if(response['exam2_problems_problem_kind']==1){
								problem_kind = problem_kind + "[<?= $this->lang->line_or_def('common_text','テキスト'); ?>]";
							}else if(response['exam2_problems_problem_kind']==2){
								problem_kind      = problem_kind + "[<?= $this->lang->line_or_def('common_video','ビデオ'); ?>]";
								problem_kind_link = '/cms_video/detail/'+response['exam2_problems_problem_contents_id']+'/';
							}else if(response['exam2_problems_problem_kind']==3){
								problem_kind      = problem_kind + "[<?= $this->lang->line_or_def('common_book_library','図書室'); ?>]";
								problem_kind_link = '/cms_book_library/detail/'+response['exam2_problems_problem_contents_id']+'/';
							}
							
							var problem_kind_detail = response['exam2_problems_problem_contents'];
							if(problem_kind_link != ''){
								problem_kind_detail = '[No'+response['exam2_problems_problem_contents_id']+'] '+'<a style="text-decoration: none;" href="'+problem_kind_link+'" target="_blank" rel="noopener noreferrer">'+response['exam2_problems_problem_contents']+'</a>';
							}
							
							var answer_kind = "<?= $this->lang->line_or_def('common_exam2_answer','解答'); ?>";
							if(response['exam2_problems_answer_kind']==1){
								answer_kind = answer_kind + "[<?= $this->lang->line_or_def('common_single_forms','単一形式'); ?>]";
							}else if(response['exam2_problems_answer_kind']==2){
								answer_kind = answer_kind + "[<?= $this->lang->line_or_def('common_plural_forms','複数形式'); ?>]";
							}else if(response['exam2_problems_answer_kind']==3){
								answer_kind = answer_kind + "[<?= $this->lang->line_or_def('common_free_exam2_answer','フリー解答'); ?>]";
							}
							...
							$("#"+id).append(
								$('<ul>').append(
									$('<li class="confirm_ul_li_line_dotted">').append(
									   ...
									   +'<div class="detail_values">'+response['exam2_problems_answer_contents']+'</div>'
							...
							// 対象を表示、それ以外を非表示
							$("[id^=exam2_problem_detail_]").slideUp();
							$("#"+id).slideDown();
						}
					},
					error: function(data) {
		...
		//詳細確認画面　解答詳細表示ボタン押下
		//**************************************************
		function show_exam2_answer_detail(id, exam2_id, answer_student_id, answer_no){
			if( $("#"+id).css("display") == 'none' ){
				var exam2_answer_data = $("#string_exam2_answer_data").val()+"";
				var post_data        = {exam2_id:exam2_id, answer_student_id:answer_student_id, answer_no:answer_no, exam2_answer_data:exam2_answer_data};
				
				$.ajax({
					url: "/cms_exam2_problem/get_exam2_answer_detail",
					type: "POST",
					data: post_data,
					success: function(response) {
						if(response){
							...
							for (var i=0; i< response.length; i++) {
								...
								if(response[i]['exam2_answer_mark']==0){
									answer_mark_non_correct = 'selected="selected"';
									answer_mark_correct     = '';
								}
								
								var answer_index = "answer_index_"+response[i]['exam2_answer_id'];
									...
									$('<li class="confirm_ul_li_line_dotted" id="'+answer_index+'">').append(
										 ...
										 +'<div class="detail_answer">'
										   ...
										   +'<div class="exam2_answer_point">'
										     +"<?= $this->lang->line_or_def('common_exam2_answer_points','解答配点') ?>&nbsp;:&nbsp;"+'<input type=text name="exam2_answer_point[]" maxlength="5" value='+response[i]['exam2_answer_point']+'>'
										   +'</div>'
										 +'</div>'
										 +'<div style="clear:both;"></div>'
									)
							...
							// 対象を表示、それ以外を非表示
							$("[id^=exam2_answer_detail_]").slideUp();
							$("#"+id).slideDown();
						}
					},
					error: function(data) {
		...
		//**************************************************
		function change_exam2_answer_mark(id){
			var exam2_answer_mark = $("#"+id).find("*[name='exam2_answer_mark[]']").val();
			var answer_point     = $("#"+id).find("*[name='answer_point[]']").val();

			if(exam2_answer_mark==0){
				// 不正解
				$("#"+id).find("*[name='exam2_answer_point[]']").val(0);
			}else{
				// 正解
				$("#"+id).find("*[name='exam2_answer_point[]']").val(answer_point);
			}
		}

		//**************************************************
		//詳細確認画面　解答修正登録ボタン押下（アンケートID）
		//**************************************************
		function exam2_answer_update(id){
			// フォームの生成
			var form = document.createElement("form");
			form.setAttribute("action", "<?=base_url()?>cms_exam2/exam2_answer_update_confirm/" + id);
			form.setAttribute("method", "post");
			form.style.display = "none";
			document.body.appendChild(form);
			...
			$("#exam2_answer_ul LI").map(function() {
				...
				var exam2_answer_mark  = '';  // 正誤（正解:1、不正解:0）
				var exam2_answer_point = '';  // 解答配点
				var li_kind_flag      = 0;
				...
				if( $(this).attr('class') == 'confirm_ul_li_line_dotted'){
					// 解答の詳細の方
					exam2_answer_id    = '' + $(this).children("[name='exam2_answer_id[]']").val();  // 解答ID
					exam2_answer_mark  = '' + $(this).find("[name='exam2_answer_mark[]']").val();    // 正誤（正解:1、不正解:0）
					exam2_answer_point = '' + $(this).find("[name='exam2_answer_point[]']").val();   // 解答配点
					if(exam2_answer_point==''){
				...
				if(li_kind_flag==1){
					...
					input.setAttribute('value', exam2_answer_id+'/'+exam2_answer_mark+'/'+exam2_answer_point);
					form.appendChild(input);
				}
			});
			form.submit();
		...
		function csv_download_latest_summary(id){
			...
			location.href ="<?=base_url()?>cms_exam2/answer_csv_download_summary/" + id + "/1";
		...
		function csv_download_summary(id){
			...
			location.href ="<?=base_url()?>cms_exam2/answer_csv_download_summary/" + id;
		...
		function csv_download_detail(id){
			...
			location.href ="<?=base_url()?>cms_exam2/answer_csv_download_detail/" + id;
	...
	<div id="wrapper">
		...
		<div id="main">
			...
			<div id="contents_main">
				...
				<h2>
					<?php 
						if( $exam2['update_flg'] == 0 ){
							print $this->lang->line_or_def('msg_exam2_confirm','アンケート情報の確認');
						}else{
							switch($btn_kirikae_flg){
								...
								case 2://詳細画面
									print $this->lang->line_or_def('msg_exam2_detail','アンケート情報の詳細');
									break;
							}
						}
					?>
				...
				<?=form_open("cms_exam2/commit")?>
					<input type="hidden" name="exam2_id" value='<?= $exam2['exam2_id']; ?>' />
					
					<?php if( isset($exam2['exam2_students']) ): ?>
						<?php foreach( $exam2['exam2_students'] as $student): ?>
							<input type="hidden" name="exam2_students[]" value='<?= $student; ?>' />
						<?php endforeach; ?>
					<?php endif; ?>
					
					<?php if( isset($exam2['exam2_problems']) ): ?>
						<?php foreach( $exam2['exam2_problems'] as $student): ?>
							<input type="hidden" name="exam2_problems[]" value='<?= $student; ?>' />
						<?php endforeach; ?>
					<?php endif; ?>
...
<?php } ?>
						...
						<tr>
							...
							<td >
								<?php
									$flg = FALSE;
									if( isset($exam2['exam2_lectures_name']) ) {
										foreach( $exam2['exam2_lectures_name'] as $name) { 
											if($flg){	?>
												,
											<?php } ?>
											<?=$name?>
...
<?php if(false){ ?>
						<tr>
							...
							<td ><?= $exam2['exam2_students_name']; ?></td>
						</tr>
<?php } ?>
						<tr>
							<th><?= $this->lang->line_or_def('common_caption','説明') ?></th>
							<td style="word-break: break-all;"><?= nl2br($exam2['exam2_caption']); ?>
							</td>
						</tr>
						<tr>
							<th><?= $this->lang->line_or_def('common_submit_period','提出期間') ?></th>
							<td >
								<?= $exam2['exam2_open'] ?><?= $this->lang->line_or_def('common_range','～') ?><?= $exam2['exam2_close'] ?>
							</td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td >
								<?php if($exam2['resubmit_flag'] == 0){ ?>
									<?= $this->lang->line_or_def('common_impossible','不可'); ?>
								<?php }elseif($exam2['resubmit_flag'] == 1){ ?>
...
<!--
						<tr>
							...
							<td >
								<?php if($exam2['marking_public_flag'] == 0): // 0:公開 ?>
									<?= $this->lang->line_or_def('common_public', '公開'); ?>
<?php if(false){ ?>
...
-->
						<tr>
							...
							<td >
								<?php if(!empty($exam2['exam2_problems'][0])): ?>
									<ul>
										<li>
											<div class="confirm_ul_li_div" style="width:280px;"><?= $this->lang->line_or_def('common_exam2_problem_name','設問名') ?></div>
										...
										<ul>
											<?php foreach($exam2['exam2_problems'] as $id => $exam2_problem): ?>
												<li class="confirm_ul_li_line">
													<input type="hidden" name="exam2_problem_id[]" value='<?= $exam2['exam2_problems'][$id]; ?>' />
													<div class="confirm_ul_li_div_detail" style="width:280px;">[設問<?= $id+1 ?>]&nbsp;<?= $exam2['exam2_problems_name'][$id] ?></div>
													<div class="confirm_ul_li_div_detail" style="width:100px;">
														<? if( getenv('URL_SERVICE')!='mitemo' ): ?>
															<?= $exam2['exam2_problems_teacher_name'][$id] ?>
														<? endif; ?>
													</div>
													<div class="confirm_ul_li_div_detail" style="width: 70px;"><?= $exam2['exam2_problems_answer_point'][$id] ?></div>
													<div class="confirm_ul_li_div_detail" style="width: 70px;">
														<? if($btn_kirikae_flg==2): ?>
															<input type="button" value="<?= $this->lang->line_or_def('common_exam2_problem_detail','設問詳細') ?>" onClick="show_exam2_problem_detail('exam2_problem_detail_<?= $id; ?>',<?= $exam2['exam2_problems'][$id] ?>);return false;" />
														<? endif; ?>
													</div>
													<div style="clear:both;"></div>
													<?php // ajaxによる表示設定 ?>
													<div id="exam2_problem_detail_<?= $id; ?>" class="detail_area">
														<ul><li></li></ul>
													</div>
												</li>
											<?php endforeach; ?>
							...
							<th style="text-align: left;vertical-align: top;">
								...
								<?php if(!empty($exam2['answer_date'][0])): ?>
									...
									<div class="div_download">
										<a href='#' onClick='csv_download_summary(<?= $exam2['exam2_id']?>);return false;' id="csv_link_summary"><?= $this->lang->line_or_def('common_summary','概要') ?> Excel-csv DL</a>
										<div id="csv_link_summary_none"></div>
									</div>
									<div class="div_download">
										<a href='#' onClick='csv_download_detail(<?= $exam2['exam2_id']?>);return false;' id="csv_link_detail"><?= $this->lang->line_or_def('common_detail','詳細') ?> Excel-csv DL</a>
									...
									<div class="div_download"><?= $this->lang->line_or_def('msg_exam2_answer_download_2','※解答が多い場合、ダウンロードに時間がかかる場合があります') ?></div>
								<?php endif; ?>
							</th>
							<td>
								<?php if(!empty($exam2['answer_date'][0])): ?>
								...
								<input type="hidden" id="string_exam2_answer_data" value='<?= $string_exam2_answer_data; ?>' />
								...
								<ul>
									<li>
										...
										<div class="confirm_ul_li_div" style="width: 70px;"><?= $this->lang->line_or_def('common_exam2_answer_points','解答配点') ?></div>
										<div class="confirm_ul_li_div" style="width: 70px;"></div>
										<div style="clear:both;"></div>
									</li>
								</ul>
									...
									<ul id="exam2_answer_ul">
										<?php foreach($exam2['answer_date'] as $id => $answer_date): ?>
											<li class="confirm_ul_li_line" <?= ($exam2['exam2_answer_latest_flag'][$id]==0) ? 'name="not_latest"' : 'name="latest"' ; ?>>
												...
												<div class="confirm_ul_li_div_detail" style="width: 70px;"><?= $exam2['answer_point_total'][$id]; ?>/<?= $exam2['max_answer_point']; ?></div>
												<div class="confirm_ul_li_div_detail" style="width: 70px;">
													<? if($btn_kirikae_flg==2): ?>
													<input type="button" value="<?= $this->lang->line_or_def('common_exam2_answer_detail','解答詳細') ?>" onClick="show_exam2_answer_detail('exam2_answer_detail_<?= $id; ?>',<?= $exam2['exam2_id']; ?>,<?= $exam2['answer_student_id'][$id] ?>,<?= $exam2['answer_no'][$id] ?>);return false;" />
													<? endif; ?>
												</div>
												<div style="clear:both;"></div>
								...
								<?php else: ?>
									<?= $this->lang->line_or_def('common_no_exam2_answer','解答なし') ?>
								<?php endif; ?>
							</td>
						</tr>
<?php } ?>
					...
					<div class="submit">
						<?php
							switch($btn_kirikae_flg){
								...
								case 2://詳細画面
									print "<input type='image' src='/static/image/btn_back.png' onClick='location.href = \"".site_url('cms_exam2')."\";return false;' />";
									
									$visible_issue_edit = ' style="visibility: hidden;" ';
									if($exam2_edit_delete_flag > 0) $visible_issue_edit = ' ';
									
									print "<input type='image' src='/static/image/btn_revise.png' onClick='edit_item();return false;' ".$visible_issue_edit."/>";
									print "<input type='image' src='/static/image/btn_delete.png' onClick='delete_item(".$exam2['exam2_id'].',"'.$this->lang->line_or_def('msg_delete','本当に削除してもよろしいですか？').'"'.");return false;' ".$visible_issue_edit."/>";
if(false){
									if(!empty($exam2['answer_date'][0])){
										print "<a id='btn_blue_long_button' onclick='exam2_answer_update(".$exam2['exam2_id'].");return false;' href='#' ".$visible_issue_edit.">".$this->lang->line_or_def('common_exam2_answer_update_regist','解答修正登録')."</a>";
									}
}
									break;
							}
