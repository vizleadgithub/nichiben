...
<?php
	...
	<style type="text/css">
		...
		/* 受講者 - 左側 */
		#student_group_list, #exam2_problem_group_list{
			display : none;
		}
			#student_group_list DIV, #exam2_problem_group_list DIV{
				width      : 160px;
				margin-top : 10px;
				display    : block;
				word-wrap  : break-word;
			}
				#student_group_list DIV A, #exam2_problem_group_list DIV A{
		...
		/* 受講者 - 左側 - メッセージ */
		#student_group_list_msg, #exam2_problem_group_list_msg{
			margin-top    : 20px;
			margin-bottom : 10px;
			display       : none;
			text-indent   : -1em; 
...
.problem_del_link {
	...
	<script type="text/javascript"><!--
		...
		$(function(){
			/* datetimepicker設定 */
			$('#exam2_open' ).datetimepicker(datetimepickeroption);
			$('#exam2_close').datetimepicker(datetimepickeroption);
			...
			$('.cource_list').click(function (){
				...
				ajax_search_cource_exam2_problems(class_lectures_list);
		...
		function li_drop(target){
			//【name="exam2_problem_id[]"】を【name="exam2_problems[]"】に変更
			target.children("*[name='exam2_problem_id[]']").attr("name","exam2_problems[]");
		...
		function add_link_preview(){
			$('#problem_drag_area .preview_button').click(function(e){
				var exam2_problem_id = $(this).closest('LI').children("*[name='exam2_problem_id[]']").val();
				var url = "/cms_exam2/preview_exam2_problem/" + exam2_problem_id + "";
		...
		*/
		$(function () {
			...
			$('#problem_drag_area, #problem_drop_area').on('click', '.preview_button', function (e) {
				...
				const url = "/cms_exam2/preview_exam2_problem/" + exam_problem_id;
				window.open(url, 'mywindow6', 'width=636,height=418,menubar=no,toolbar=no,scrollbars=yes');
			});
		});
		...
		*/
		...
		// [ajax]講座所属の設問取得
		//**************************************************
		function ajax_search_cource_exam2_problems(cource_id){
			var select_exam2_problem_id = "0";
			...
			$.ajax({
				...
				success: function(response) {
					...
					$("#ul_problem_drop_area").find("LI").each(function(idx, elem) {
						var target_val = $(elem).children("*[name='exam2_problems[]']").val();
						var delete_flg = 1;
						for (var i=0; i< response.length; i++) {
							if(target_val == response[i]['exam2_problem_id']){
								delete_flg = 0;
								break;
							}
						}
					...
					if(response){
						for (var i=0; i< response.length; i++) {
							...
							select_exam2_problem_id = ""+select_exam2_problem_id+","+response[i]['exam2_problem_id']+""; 
							
							// 取得したデータを行に入れる
							$("#ul_problem_drag_area").append(
								$('<li>').append(
									$('<input type="hidden" name="exam2_problem_id[]" value='+response[i]['exam2_problem_id']+' />')

								).append(
									$('<div class="problem_name">').text(
										'[No'+response[i]['exam2_problem_id']+'] '+response[i]['exam2_problem_name']+' [<?= $this->lang->line_or_def('common_management_teacher','管理講師') ?>:'+response[i]['teacher_name']+'] [<?= $this->lang->line_or_def('common_exam2_answer_points','解答配点') ?>:'+response[i]['answer_point']+']'
									)
								).append(
									$('<div class="problem_preview_link">').html(
										'<input class="preview_button" type="button" value="<?= $this->lang->line_or_def('common_preview','プレビュー') ?>" onclick="" data-epid="' + response[i]['exam2_problem_id'] + '" />'
									)
								).append(
									$('<div class="problem_del_link">').html(
										'<input class="delete_button" type="button" value="<?= $this->lang->line_or_def('common_deletion','削除') ?>" onclick="" data-epid="' + response[i]['exam2_problem_id'] + '" />'
									)
								).append(
						...
						$.ajax({
							url: "/cms_exam2_problem_group/get_cource_exam2_problem_group",
							type: "POST",
							data: "select_exam2_problem_id="+select_exam2_problem_id,
							
							success: function(response) {
								// 元にあるリンクを削除
								$("#exam2_problem_group_list DIV").remove();
								if(response){
									// 取得したデータを行に入れる
									for (var keyString in response) {
										$("#exam2_problem_group_list").append(
											$('<div>').append(
												$('<a href="#" onclick="select_exam2_problem_group('+"'"+response[keyString]+"'"+');return false;">'+keyString+'</a>')
											)
										);
									}
									$("#exam2_problem_group_list").css('display','block');
									$("#exam2_problem_group_list_msg").css('display','block');
								}else{
									$("#exam2_problem_group_list").css('display','none');
									$("#exam2_problem_group_list_msg").css('display','none');
								}
							}
						});
					}else{
				...
				success: function(response) {
					...
					if(response){
						...
						for (var i=0; i< response.length; i++) {
							...
							$("#student_ul").append(
								$('<li></li>').append(
									$('<input type="checkbox" name="exam2_students[]" id="student_'+response[i]['student_id']+'" value='+response[i]['student_id']+' '+check_flag+'>')
								).append(
		...
		// 設問 - 設問グループ名リンク押下時の処理
		//**************************************************
		function select_exam2_problem_group(exam2_problem_id_list){
			// 設問IDがない場合は終了
			if(exam2_problem_id_list.length == 0){
				return false;
			}
			// 設問IDを配列変換
			var resArray = exam2_problem_id_list.split(",");
			...
			$('#ul_problem_drag_area').find('li').each(function(idx, item) {
				...
				var elem = $(item).clone();
				// LI から 設問IDを取得
				var target_val = elem.children("*[name='exam2_problem_id[]']").val();
	...
	<div id="wrapper">
		...
		<div id="main">
			...
			<div id="contents_main">
				...
				<h2><?= $this->lang->line_or_def('msg_exam2_input','アンケートの情報を入力してください') ?></h2>
...
<?php if(false){ ?>
						<? if( getenv('URL_SERVICE')=='mitemo' ): ?>
							<input type="hidden" name="teacher_id"   value='<?=set_value('teacher_id',   $exam2['teacher_id'])?>'>
						<? else: ?>
						<tr>
...
<?php } ?>
						...
						<tr>
							...
							<td >
								<input type="text" name="exam2_open" size="19" value="<?=set_value('exam2_open',$exam2['exam2_open'])?>" id="exam2_open" readonly>
								<?= $this->lang->line_or_def('common_range','～') ?>
								<input type="text" name="exam2_close" size="19" value="<?=set_value('exam2_close',$exam2['exam2_close'])?>" id="exam2_close" readonly>
							</td>
						</tr>
						<tr>
...
<?php if(false){ ?>
								<div id="marking_public_kind_area" style="<?=(set_value('marking_public_flag',$exam2['marking_public_flag'])==9) ? 'display:none;' : ''?> ">
									<div>
										<input type="radio" onclick="marking_public_kind_checkradio('none')" value="1" id="marking_public_kind" name="marking_public_kind" <?=(set_value('marking_public_kind',$exam2['marking_public_kind'])==1) ? 'checked' : ''?> />
										<label for="marking_public_kind_now"><?= $this->lang->line_or_def('common_spot_public','即時公開') ?></label>
									</div>
									<div>
										<input type="radio" onclick="marking_public_kind_checkradio('block')" value="2" id="marking_public_kind" name="marking_public_kind" <?=(set_value('marking_public_kind',$exam2['marking_public_kind'])==2) ? 'checked' : ''?> />
										<label for="marking_public_kind_time"><?= $this->lang->line_or_def('common_timed_public','時限式公開') ?></label>
										&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
										<input type="text" name="marking_public_open" size="19" value="<?=set_value('marking_public_open',$exam2['marking_public_open'])?>" id="marking_public_open" readonly style="<?=(set_value('marking_public_kind',$exam2['marking_public_kind'])==1) ? 'display:none;' : ''?>" >
									</div>
								</div>
<?php } ?>
							</td>
...
-->
						<input type="hidden" name="marking_public_flag" value="9">
							...
							<th>
								...
								&nbsp;&nbsp;<a class="tooltip_bottom" href="#" onclick="tooltip_message_bottom();return false;"><img src="/static/image/question.png" alt="question" />
												<span class="tooltip_message_bottom">
													...
													<?= $this->lang->line_or_def('msg_tooltip_full_screen_collectively_scoring_format' , '１画面に全てのアンケートが表示され、解答欄に全て入力した上で一括採点となる形式'); ?>
													<br/><br/>
													<?= $this->lang->line_or_def('common_q_and_a_format_each_time_scoring'      , '一問一答形式（都度採点）'); ?><br />&nbsp;&nbsp;
													<?= $this->lang->line_or_def('msg_tooltip_q_and_a_format_each_time_scoring' , '１問ごとに１画面が表示され、アンケートと解答入力をし、１問ごとに１画面で解答解説を閲覧することを繰り返す形式'); ?>
													<br/><br/>
													<?= $this->lang->line_or_def('common_q_and_a_format_collectively_scoring'      , '一問一答形式（一括採点）'); ?><br />&nbsp;&nbsp;
													<?= $this->lang->line_or_def('msg_tooltip_q_and_a_format_collectively_scoring' , '１問ごとに１画面が表示され、アンケートと解答入力を繰り返し、最後に一括で解答解説を閲覧する形式'); ?>
												</span></a>
							</th>
							<td >
								<input type="radio" value="0" id="display_format" name="display_format" <?=(set_value('display_format',$exam2['display_format'])==0) ? 'checked' : ''?> /><?= $this->lang->line_or_def('common_full_screen_collectively_scoring_format','全画面一括採点形式'); ?>&nbsp;
								<input type="radio" value="1" id="display_format" name="display_format" <?=(set_value('display_format',$exam2['display_format'])==1) ? 'checked' : ''?> /><?= $this->lang->line_or_def('common_q_and_a_format_each_time_scoring','一問一答形式（都度採点）'); ?>
								<input type="radio" value="2" id="display_format" name="display_format" <?=(set_value('display_format',$exam2['display_format'])==2) ? 'checked' : ''?> /><?= $this->lang->line_or_def('common_q_and_a_format_collectively_scoring','一問一答形式（一括採点）'); ?>
							</td>
						</tr>
-->
						<input type="hidden" name="display_format" value="0">
						<tr>
							<th style="vertical-align: top;">
								<?= $this->lang->line_or_def('common_exam2_problem','設問') ?>

								<div id="exam2_problem_group_list"></div>
								<div id="exam2_problem_group_list_msg">
									<?= $this->lang->line_or_def('msg_select_exam2_problem_group_caption','※上記設問グループ名選択により、同グループ設問が選択されます') ?>
								</div>
							</th>
							<td>
								<div id="problem_list">
									<div id="problem_drop_area">
										<ul id="ul_problem_drop_area">
											<?php if( (isset($exam2_problems_list)) && (isset($exam2['exam2_problems'])) ): ?>
												<?php foreach( $exam2['exam2_problems'] as $exam2_problem_id ): ?>
									...
									<input id="all_delete_button" type="button" value="<?= $this->lang->line_or_def('common_exam2_problem_all_delete','設問一括削除') ?>" onclick="">
								</div>
								<br>
								<div id="problem_choices_list">
									...
									<input id="all_select_button" type="button" value="<?= $this->lang->line_or_def('common_exam2_problem_all_add','設問一括追加') ?>" onclick="">
								</div>
							</td>
						</tr>
<!--
...
-->
						<input type="hidden" name="criteria_type" value="1">
						<input type="hidden" name="criteria_value" value="0">
					</table>
					<div class="submit">
