...
<?php
	...
	<script type="text/javascript"><!--
		...
		function get_book_library_list() {
			...
			$.ajax({
				url: "/cms_exam_problem/get_cource_book_library",
				type: "POST",
		...
		function get_video_list() {
			...
			$.ajax({
				url: "/cms_exam_problem/get_cource_video",
				type: "POST",
				data: "cource_id="+AllVals.join(","),

				success: function(response) {
					$("*[name='problem_contents_video'] option").remove();
					$("*[name='problem_contents_video']").append('<option selected="selected" value="-1"><?= $this->lang->line_or_def('msg_exam_problem_video_select','ビデオを選択してください') ?></option>');
	...
	<div id="wrapper">
		...
		<div id="main">
			...
			<div id="contents_main">
				...
				<h2><?= $this->lang->line_or_def('msg_exam_problem_input','設問の情報を入力してください') ?></h2>
				...
				<?=form_open_multipart("cms_exam_problem/confirm")?>
					...
					<table class="form">
						<tr>
							...
							<td>
								<input type=text name="exam_problem_name" maxlength="256" size="30" value='<?= htmlspecialchars(set_value('exam_problem_name', $exam_problem['exam_problem_name'], FALSE), ENT_QUOTES, 'UTF-8', false) ?>'>
							</td>
						</tr>
...
<?php } ?>
									...
									<?php 
										if( isset($lecture_cources) ) { 
											foreach( $lecture_cources as $cource ){ ?>
												<li>
												<input type="checkbox" name="exam_problem_lectures[]" id="lectures_<?= htmlspecialchars( $cource['cource_id'], ENT_QUOTES, 'UTF-8', false) ?>" value=<?= htmlspecialchars( $cource['cource_id'], ENT_QUOTES, 'UTF-8', false) ?>
													<?php 
						...
						<tr>
							...
							<td >
								<?=form_dropdown('problem_kind', $problem_kind_list, set_value('problem_kind', $exam_problem['problem_kind']));?>
							</td>
						</tr>
						<tr>
							...
							<td>
								<? // 設問種類に応じて表示切替 ?>
								<textarea name="problem_contents_text" id="problem_contents_text" ><?= htmlspecialchars(set_value('problem_contents_text', $exam_problem['problem_contents_text'], FALSE), ENT_QUOTES, 'UTF-8', false) ?></textarea>
								<?= form_dropdown('problem_contents_book_library',$problem_contents_book_library, set_value('problem_contents_book_library', $exam_problem['problem_contents_book_library']), 'name="problem_contents_book_library" class="problem_contents_book_library" '); ?>
								<?= form_dropdown('problem_contents_video',$problem_contents_video, set_value('problem_contents_video', $exam_problem['problem_contents_video']), 'name="problem_contents_video" class="problem_contents_video" '); ?>
						...
						<tr>
							...
							<td>
								<textarea name="problem_note" id="problem_note" style="height: 80px; width: 65%;"><?= htmlspecialchars(set_value('problem_note', $exam_problem['problem_note'], FALSE), ENT_QUOTES, 'UTF-8', false) ?></textarea>
							</td>
						</tr>
						...
						<tr>
							...
							<td >
								<?=form_dropdown('answer_kind', $answer_kind_list, set_value('answer_kind', $exam_problem['answer_kind']));?>
							</td>
						</tr>
						...
						<tr>
							...
							<td>
								<div id="answer_contents_choice" name="answer_contents_choice">
									<ul id="answer_contents_choice_area">
										...
										<?php foreach($exam_problem['answer_contents_no'] as $ino => $answer_contents_no): ?>
											...
											<input type="hidden" name="answer_contents_no[]" value='<?=htmlspecialchars((string)$exam_problem['answer_contents_no'][$ino], ENT_QUOTES, 'UTF-8', false)?>' />
											<div class="div_word">
												<textarea name="answer_contents_word[]" class="answer_contents_word"><?=htmlspecialchars((string)$exam_problem['answer_contents_word'][$ino], ENT_QUOTES, 'UTF-8', false)?></textarea>
												</div>
												
												<div class="div_correct">
													<?php if($exam_problem['answer_contents_correct'][$ino] == 1): ?>
														<input type="checkbox" value="<?= htmlspecialchars( $answer_contents_no, ENT_QUOTES, 'UTF-8', false) ?>" name="answer_contents_correct[]" checked>
													<?php else: ?>
														<input type="checkbox" value="<?= htmlspecialchars( $answer_contents_no, ENT_QUOTES, 'UTF-8', false) ?>" name="answer_contents_correct[]" >
													<?php endif; ?>
								...
								<textarea name="answer_contents_text" id="answer_contents_text"><?= htmlspecialchars(set_value('answer_contents_text', $exam_problem['answer_contents_text'], FALSE), ENT_QUOTES, 'UTF-8', false) ?></textarea>
							</td>
						</tr>
						
						<tr>
							<th width="160"><?= $this->lang->line_or_def('common_exam_answer_points','解答配点') ?></th>
							<td>
								<input type=text name="answer_point" id="answer_point" value='<?=set_value('answer_point',$exam_problem['answer_point'])?>'>
							</td>
						</tr>
						...
						<tr>
							...
							<td >
								<?=form_dropdown('answer_explain_kind', $answer_explain_kind_list, set_value('answer_explain_kind', $exam_problem['answer_explain_kind']));?>
							</td>
						</tr>
						
						<tr name="tr_answer_explain_contents" style="<?= ($exam_problem['answer_explain_kind'] == 9) ? "display:none" : ""; ?>;">
							<th width="160" style="vertical-align: top;"><?= $this->lang->line_or_def('common_answer_explain_contents','解答解説内容') ?></th>
							<td>
								<? // 設問種類に応じて表示切替 ?>
								<textarea name="answer_explain_contents_text" id="answer_explain_contents_text" ><?= htmlspecialchars(set_value('answer_explain_contents_text', $exam_problem['answer_explain_contents_text'], FALSE), ENT_QUOTES, 'UTF-8', false) ?></textarea>
								<?= form_dropdown('answer_explain_contents_book_library',$problem_contents_book_library, set_value('answer_explain_contents_book_library', $exam_problem['answer_explain_contents_book_library']), 'name="answer_explain_contents_book_library" class="answer_explain_contents_book_library" '); ?>
								<?= form_dropdown('answer_explain_contents_video',$problem_contents_video, set_value('answer_explain_contents_video', $exam_problem['answer_explain_contents_video']), 'name="answer_explain_contents_video" class="answer_explain_contents_video" '); ?>
								 <img name="answer_explain_contents_thum" id="answer_explain_contents_thum" style="" src="">
							</td>
						</tr>
						
						<tr name="tr_answer_explain_note" style="<?= ($exam_problem['answer_explain_kind'] == 9) ? "display:none" : ""; ?>">
							<th width="160" style="vertical-align: top;"><?= $this->lang->line_or_def('common_answer_explain_note','解答解説備考') ?></th>
							<td>
								<textarea name="answer_explain_note" id="answer_explain_note" style="height: 80px; width: 65%;"><?= htmlspecialchars(set_value('answer_explain_note', $exam_problem['answer_explain_note'], FALSE), ENT_QUOTES, 'UTF-8', false) ?></textarea>
							</td>
						</tr>
