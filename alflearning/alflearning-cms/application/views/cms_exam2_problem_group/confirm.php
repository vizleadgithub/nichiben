	...
	<script type="text/javascript">
		...
		function delete_item(id, msg){
			if(window.confirm( msg )){
				location.href = "<?=base_url()?>cms_exam2_problem_group/delete_item/" + id ;
			}
		}
		//--------------------------------------------------
		//詳細確認画面　修正ボタン押下
		//--------------------------------------------------
		function edit_item(){
			location.href ="<?=base_url()?>cms_exam2_problem_group/edit/";
		}
	</script>
	<style type="text/css">
		/* グレーボタン */
	...
	<div id="wrapper">
		...
		<div id="main">
			...
			<div id="contents_main">
				...
				<h2>
					<?php 
						if( $exam2_problem_group['update_flg'] == 0 ){
							print $this->lang->line_or_def('msg_exam2_problem_group_detail','設問グループ情報の確認');
						}else{
							switch($btn_kirikae_flg){
								case 1://修正画面
									print $this->lang->line_or_def('msg_exam2_problem_group_confirm','設問グループ情報の確認');
									break;
									
								case 2://詳細画面
									print $this->lang->line_or_def('msg_exam2_problem_group_detail','設問グループ情報の詳細');
									break;
							}
						}
					?>
				...
				<?=form_open("cms_exam2_problem_group/commit")?>
					<table class="form">
						<tr>
							<th width="160"><?= $this->lang->line_or_def('common_exam2_problem_group_name','設問グループ名') ?></th>
							<td>
								<?= htmlspecialchars( $exam2_problem_group['exam2_problem_group_name'], ENT_QUOTES, 'UTF-8', false) ?>
							</td>
						</tr>
						<tr>
							<th><?= $this->lang->line_or_def('common_caption','説明') ?></th>
							<td style="word-break: break-all;"><?= nl2br( htmlspecialchars( $exam2_problem_group['exam2_problem_group_caption'], ENT_QUOTES, 'UTF-8', false) ) ?></td>
						</tr>
						<tr>
							<th>
					...
					<div class="submit">
						<?php
							switch($btn_kirikae_flg){
								...
								case 2://詳細画面
									print "<input type='image' src='/static/image/btn_back.png' onClick='location.href = \"".site_url('cms_exam2_problem_group')."\";return false;' />";
									print "<input type='image' src='/static/image/btn_revise.png' onClick='edit_item();return false;' />";
									print "<input type='image' src='/static/image/btn_delete.png' onClick='delete_item(".$exam2_problem_group['exam2_problem_group_id'].',"'.$this->lang->line_or_def('msg_delete','本当に削除してもよろしいですか？').'"'.");return false;' />";
									break;
							}
						?>
					</div>
