					...
					<?php if(isset($information_list)) { ?>
						<?php foreach($information_list as $information) { ?>
							<tr style=<?= ($information['school_id']==0) ? "background-color:#D3F5EA;" : ""; ?> >
								<td class="tdc"><a href="/cms_information/detail/<?= $information['information_id'] ?>"><?=$information['information_id']?>
								</td>
								<td class="tdc"><?=$information['information_date']?></td>
								<td class="tdc"><?=$information['information_title']?></td>
								<td class="tdc">
								...
								<td class="tdc">
									<?=$information['disp_status']?>&nbsp;<?= ($information['school_id']==0) ? '('.$this->lang->line_or_def('common_all_school','全学校').')' : ""; ?>
								</td>
							</tr>
						<?php } ?>
					<?php } ?>
