				...
				<?=form_open("cms_information/commit")?>
					<table class="form">
						<tr>
							...
							<td>
								<?php print( date("Y/m/d", strtotime($information['information_date'])) ); ?>
							</td>
						</tr>
						<?php /*
						<tr>
							...
							<td >
									<?= ($information['information_type']==0 ? 'お知らせ' : '更新情報'); ?>
							</td>
						</tr>
						*/ ?>
						<tr>
						...
						<tr>
							...
							<td><?= nl2br( htmlspecialchars( $information['information_caption'], ENT_QUOTES, 'UTF-8', false) ) ?>
							</td>
						</tr>
						<tr>
							<th width="160"><?= $this->lang->line_or_def('common_url','URL') ?></th>
							<td>
								<?= htmlspecialchars( $information['information_url'], ENT_QUOTES, 'UTF-8', false) ?>
							</td>
						</tr>
						<tr>
							<th><?= $this->lang->line_or_def('common_public_period','公開期間') ?></th>
							<td >
								<?php print( date("Y/m/d H:i:s", strtotime($information['information_open'])) ); ?>
								<?= $this->lang->line_or_def('common_range','～') ?>
								<?php print( date("Y/m/d H:i:s", strtotime($information['information_close'])) ); ?>
							</td>
						</tr>
						<tr>
							<th><?= $this->lang->line_or_def('common_information_topfit','先頭枠への固定表示') ?></th>
							<td >
									<?= ($information['information_topfit']==0 ? 'しない' : 'する'); ?>
							</td>
						</tr>
