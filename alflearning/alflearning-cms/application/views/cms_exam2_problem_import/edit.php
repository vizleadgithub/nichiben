...
<!--
				<h2>
					<?= $this->lang->line_or_def('msg_exam2_problem_import_input','設問にインポートするファイルを入力してください') ?>
					<?= str_repeat("&nbsp;", 30);?><?= $this->lang->line_or_def('msg_exam2_problem_import_sample','インポート用ファイルのサンプル') ?>&nbsp;&gt;&gt;&nbsp;<a href="/static/psd/exam2_problem_import.zip">Download</a>
				</h2>
 -->
				<h2>
					<div id="h2_title"><!--<?= $this->lang->line_or_def('msg_exam2_problem_import_input','設問にインポートするファイルを入力してください') ?>-->設問のインポート/エクスポートが行えます</div>
					<div id="h2_result"><?= $this->lang->line_or_def('msg_exam2_problem_import_sample','インポート用ファイルのサンプル') ?>&nbsp;&gt;&gt;&nbsp;<a href="/static/psd/exam2_problem_import.zip">Download</a></div>
					<div style="clear:both;"></div>
				</h2>
 
 
				<?=form_open_multipart("cms_exam2_problem_import/confirm")?>
...
<?php } ?>
									...
									<?php 
										if( isset($lecture_cources) ) { 
											foreach( $lecture_cources as $cource ){ ?>
												<li>
												<input type="checkbox" name="exam2_problem_lectures[]" id="lectures_<?=$cource['cource_id']?>" value=<?=$cource['cource_id']?>
													<?php 
						...
						<tr>
							<th >
								<?= $this->lang->line_or_def('common_exam2_problem_group','設問グループ') ?>
									...
									<?php if( isset($exam2_problem_groups) ): ?>
										<?php foreach( $exam2_problem_groups as $exam2_problem_group ): ?>
											<li>
												...
												?>
												<input type="checkbox" name="exam2_problem_groups[]" id="groups_<?=$exam2_problem_group['exam2_problem_group_id']?>" value=<?=$exam2_problem_group['exam2_problem_group_id']?> <?= $checked_flag ?> />
												<label for="groups_<?=$exam2_problem_group['exam2_problem_group_id']?>">
													<?=$exam2_problem_group['exam2_problem_group_name']?>&nbsp;&nbsp;[<?= $this->lang->line_or_def('common_position_exam2_problem_count','所属設問数') ?>&nbsp;:&nbsp;<?=$exam2_problem_group['exam2_problem_count'];?>]
												</label>
											</li>
										<?php endforeach; ?>
									<?php else: ?>
						...
						<tr>
							...
							<td>
								<input type="file" name="local_file" size="30" value="<?=set_value('local_file',$exam2_problem['local_file'])?>">
				...
				<?=form_open_multipart("cms_exam2_problem_import/export")?>
					<table class="form">
						<tr>
									...
									<?php if( isset($exam2_problem_groups) ): ?>
										<?php foreach( $exam2_problem_groups as $exam2_problem_group ): ?>
											<li>
												...
												?>
												<input type="checkbox" name="exam2_problem_groups_ex[]" id="groups_<?=$exam2_problem_group['exam2_problem_group_id']?>_ex" value=<?=$exam2_problem_group['exam2_problem_group_id']?> <?= $checked_flag ?> />
												<label for="groups_<?=$exam2_problem_group['exam2_problem_group_id']?>_ex">
													<?=$exam2_problem_group['exam2_problem_group_name']?>&nbsp;&nbsp;[<?= $this->lang->line_or_def('common_position_exam2_problem_count','所属設問数') ?>&nbsp;:&nbsp;<?=$exam2_problem_group['exam2_problem_count'];?>]
												</label>
											</li>
										<?php endforeach; ?>
